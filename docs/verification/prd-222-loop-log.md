...
| F15 | **The backend is not the owner, and the desktop protocol was discarding the wrong thing.** Dawn against wgpu-native on the same Bayview bundle, interleaved, unprofiled, `:0`: **11.85 ms against 11.51 ms `render.p50` — flat.** Chrome runs Dawn, so swapping in Chrome's own backend answers the hypothesis directly. Two by-catch findings outrank the arm itself: (a) the **first two whole runs of a session are ~2× slow** (26.05 ms, then 11.4–12.0 ms for every run after, same binary and bundle, load ruled out) — every protocol so far discarded only *window 1 within a run*, so a three-run A/B mixed a 26 ms sample into a 12 ms population, which is the ±100% spread that made Levers A and C undecidable; `substeps.mean` 1.92 → 1.01 tracks it as a **symptom** (slow frame → more catch-up substeps → slower frame), not a cause. (b) Warmed, this lane's within-arm spread is **0.6 ms**, so it resolves a ~1 ms lever — better than the ±15% the fix plan assigned — while `fps` is dead at 59.6–59.8 vsync-capped, extending F11 from Xvfb to `:0`. | `docs/verification/prd-226-a1-backend-swap-2026-08-27.md` | **Strikes the backend option from PRD-226 Phase 5.** The warm-up and vsync rules become binding on every future arm. Cross-session absolutes are also void: a sibling lane priced the *same host revision* ~2.3× cheaper today, and the bundle was rebuilt by another lane (`update.mean` 1.5 ms against a baseline-era 4.0–4.9), so the recorded 22.2 ms desktop baseline does not reproduce |
| F16 | **The frame is measured, and JavaScript plus the bridge own 83% of it.** Three arms, one session, one meter, on the same scene: native control **11.21 ms** `render.p50`, native with the backend command entry points no-oped **9.26 ms**, Chrome on the same machine **4.05 ms** (real NVIDIA Turing adapter, matching `update.mean`). So backend recording plus the GPU work it causes is **1.95 ms (17%)** and JavaScript plus the bridge is **9.26 ms (83%)** — native's JS/bridge term alone is **2.3× Chrome's entire render phase**. Of the 7.16 ms excess over Chrome, at most 1.95 ms can be backend and GPU, so **≥5.2 ms (73%) is JS/bridge**. Validity: the A0 arm read 11.21 against 11.36 measured quiet earlier the same day (0.15 ms apart, inside F15's 0.6 ms spread); `frame.p50` fell 12.62 → 10.64 (Δ 1.98 against the render Δ 1.95) and `hostGap.p50` rose 3.0–4.3 → 5.1–6.2, both consistent with a genuine work reduction. | `docs/verification/prd-226-budget-a0-a2-a5-2026-08-27.md` | **This is the attribution five levers failed to produce, and it retires the whole backend line of enquiry** (A1 struck the implementation, A2 struck the presence). The open question narrows to one: does the 9.26 ms belong to V8 executing three.js or to the bridge? That is arm **A3**, and it selects the architectural option. **A4 is owed** — until the no-JS replay runs, `T0 − A2` holds by construction rather than by cross-check |
