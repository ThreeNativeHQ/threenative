...
| L0-02 | Judge | `pnpm bench:engines --arm tn-desktop --ladder 10000 --modes L1,L3 --repeats 3 --source-sha <sha>` for matrix attribution; Midway native via `pnpm profile:production --target desktop --project <Midway> --scenario native-playtests/launch.playtest.json --render-size 1280x720`, and browser via `--target web --scenario playtests/flight.playtest.json` with the same project and render size; scaffolded platformer via the judge default. Midway's web flight click is unsupported by the desktop runner, so it cannot use `desktop-pair` with that scenario. Exact flags and scenario hashes remain to be frozen after live preflight. |
