...
This record covers the donor-animation port from
`/home/joao/Downloads/threenative-vfx-niagara-46-effects (1).zip`.
...
| Desktop VFX row | `sh scripts/xvfb.sh node packages/runtime-native/conformance/run-conformance.mjs --target desktop --only-tests vfx-compute --out vfx-desktop --reference artifacts/conformance/web` | Selected row PASS; report summary is 1 pass, 0 fail, 91 intentionally blocked rows; pixel mismatch `0.05164279513888889` <= `0.06`; perceptual delta `4.928133090433908` <= `9`; GPU validation errors empty; native temporal captures rendered. The generated local report was moved to `/tmp/threenative-vfx-desktop-20260902/` after capture so it is not committed. |
