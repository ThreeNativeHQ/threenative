...
| **Total** | **72,678** |  | `pnpm budgets` current measurement |  | **No area rejected.** |

The 2026-08-16 device-preflight change accounts for the current `tests/` and `scripts/` rows. The
native review trigger remains visible at 72,678 / 50,000; no native source was removed. Physical
...
| --- | --- | --- |
| `pnpm budgets` | PASS, exit `0` | 7 framework packages, 5 example workspaces, 15,293/15,000 framework LOC, 72,678/50,000 native runtime LOC, 12 PRD files, largest template 1,997 LOC; both review triggers remain visible. |

- Root Vitest: 156 files, 1,369 passed, 35 skipped.
- Runtime-native Vitest: 46 files, 271 passed, 39 skipped.
