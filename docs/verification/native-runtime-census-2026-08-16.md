...
| --- | ---: | --- | --- | --- | --- |
| `src/` | 39,275 | PRD-045, PRD-047, PRD-048, PRD-050, PRD-053, PRD-116, PRD-143, PRD-153, PRD-154, PRD-155 | `src/physics/native_bindings.cpp:586`; desktop V8 and native runtime commands | Move host shims into each game or delete the native host | **KEEP** — this is the owned native host and its physics boundary. |
...
| Root `CMakeLists.txt` | 1,689 | PRD-047, PRD-048, PRD-050 | `pnpm native:build`; explicit binding target build | Make every game own native dependency discovery and linking | **KEEP** — opt-in host build configuration. |
...
| **Total** | **81,179** |  | `pnpm budgets` current measurement |  | **No area rejected.** |
