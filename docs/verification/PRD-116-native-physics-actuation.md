...
| `pnpm budgets` | PASS, exit `0` | 7 framework packages, 5 example workspaces, 15,294/15,000 framework LOC, 75,739/50,000 native runtime LOC, largest template 1,997 LOC. Both review triggers remained visible. The native figure was 71,053 when this lane ran; it is refreshed here because `pnpm budgets` and `packages/physics/__tests__/actuation.spec.ts` both read this row as the live census and fail closed when it drifts. See the dated notes under the census table for the changes that moved it. |
...
| **Total** | **75,739** |  | `pnpm budgets` post-integration output |  | **No area rejected.** |
