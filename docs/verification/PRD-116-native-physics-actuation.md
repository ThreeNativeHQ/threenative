...
| `pnpm budgets` | PASS, exit `0` | 7 framework packages, 5 example workspaces, 15,294/15,000 framework LOC, 75,753/50,000 native runtime LOC, largest template 1,997 LOC. Both review triggers remained visible. The native figure was 71,053 when this lane ran; it is refreshed here because `pnpm budgets` and `packages/physics/__tests__/actuation.spec.ts` both read this row as the live census and fail closed when it drifts. See the dated notes under the census table for the changes that moved it. |
...
| `tests/` | 8,850 | PRD-045, PRD-046, PRD-048, PRD-049, PRD-050, PRD-053, PRD-054, PRD-055, PRD-076, PRD-116, PRD-127, PRD-131 | `tests/physics_actuation_bindings_test.cpp:141`; runtime-native Vitest and native tests | Delete fail-closed tests to reduce the trigger | **KEEP** — removal would conceal regressions. |
| `scripts/` | 11,605 | PRD-045, PRD-048, PRD-049, PRD-050, PRD-053, PRD-054, PRD-116, PRD-127, PRD-131 | `scripts/verify-desktop-physics.mjs:206-228`; build, platform, device-condition, and physical-qualification verifiers | Make every game own packaging, device, and verifier orchestration | **KEEP** — no smaller shared alternative preserves the proof. |
...
| **Total** | **75,753** |  | `pnpm budgets` post-integration output |  | **No area rejected.** |

**Reconciled 2026-08-17 (emulator performance canary), 75,739 → 75,753.** No new counted area.
`scripts/` +14 is one opt-in option on the shared device preflight, `allowEmulator`, which the
benchmark sets and every qualification lane leaves alone.

**Kill-switch pass: keep.** It is the only way an Android performance regression can be caught
without a phone attached — CI has no device, so before this there was no gate at all. It is fourteen
lines and it does not weaken the emulator refusal that PRD-070 requires: the default is unchanged and
a test asserts a qualification lane still refuses.

**Reconciled 2026-08-16 (PRD-130 Phases 3-4), 75,581 → 75,739.** No new counted area. `android/`
+63 is per-ABI APK splits and an engine-aware prebuilt contract that refuses a directory populated
for the other engine; `scripts/` +47 is the V8 prebuilt asset set; `tests/` +48 covers both.

**Kill-switch pass: keep.** Phase 4 is the difference between a default its author receives and a
default users receive: before it, `android/prebuilt/` shipped no V8, so a project assembled from a
release artifact got QuickJS whatever the default said. The splits exist because every APK this
repository built was universal, so its size answered no question anyone has — and the first honest
per-ABI comparison immediately caught a 25.5 MB packaging artifact in a number this PRD had already
published.

**Reconciled 2026-08-16 (PRD-130 Phases 5-6), 75,433 → 75,581.** No new counted area. `tests/` +80
is the engine-identity assertion's unit tests and the guard that all three Android engine-default
sites agree; `scripts/` +57 is `--expect-engine`, which reads the engine the running process reported
instead of trusting the build flag; `CMakeLists.txt` +8 and `android/` +3 are the default flip itself.

**Kill-switch pass: keep.** The engine assertion is the smallest thing that prevents the failure
PRD-118 section 2 already paid for once — a build flag accepted, silently ignored, and every
measurement afterwards describing the other engine. The three-sites guard replaces nothing, because
nothing checked that the three declarations of one default agreed.

**Reconciled 2026-08-16 (PRD-130 Phases 1-2), 75,280 → 75,433.** No new counted area. `scripts/`
+101 is the `v8-android` entry in `download-deps.mjs` and the extraction that unwinds its three-deep
archive; `android/` +32 is per-ABI V8 snapshot staging in `build.gradle.kts`; `src/` +13 is the
ABI-keyed snapshot path in `android_main.cpp`; `tests/` +7 tightens the contract guard that pins
the Android dependency list, which caught this change and now also requires the checksum and the
ABI set.

**Kill-switch pass: keep.** All three close defects that exist whether or not the engine default ever
changes — an APK slice shipping another ABI's snapshot, a dependency outside the only supported
reconstruction path, and a runtime reading a path that assumed one ABI. There is no smaller version:
the provisioning entry replaces a directory somebody placed by hand, and the staging replaces a
single copy that could not be right for both slices.
