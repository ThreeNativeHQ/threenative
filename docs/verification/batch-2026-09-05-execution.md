...
## Final integration rerun — 2026-09-06

After the observer documentation correction, the supported native coverage
refresh exited 0, the CMake host rebuild exited 0, the four focused Vitest
files passed 41/41, evaluator validation passed 32/32, and collector flow
passed its correlated, truncated, foreign, and missing-response cases. The
root verification rerun passed `pnpm typecheck`, `pnpm lint` (600 warnings,
0 errors), `pnpm test` (391 files / 4,291 tests passed; 2 files / 7 tests
skipped), `pnpm budgets`, `pnpm check:docs` (1,480 links), and
`pnpm sync:agents` (19 mirrors already synchronized). The retention index was
regenerated by `scripts/generate-retention-index.ts`; its generated byte count
is authoritative and was checked by `pnpm budgets`.

The observer spans exactly four implementation/test files, below the Phase 1
cap of five. The dependency-cache symlink
`packages/runtime-native/third_party` remains untracked and unstaged; it is not
part of this PR. Physical Android, PRD-361's strict aggregate LOC requirement,
and PRD-362's physical-device acceptance remain open.
