...
| 2 | **`golden-path` stops being non-deterministic** | ❌ `stomp` and `stomp-rise` pass and fail run to run with **identical tick counts** — span 117, frames 115 every time. What varies is `firstTick` (27, 48, 63), the ticks elapsed before the scenario's first step, which tracks boot time; `Patrol.update` has been walking the enemy since load, so a stomp lands on a target in a different phase each run. The fix is `setup.place[]` + `frozen: true` from `packages/playtest/AGENTS.md`, not longer waits — padding cannot fix a phase offset. Diagnosis and the do-nots: [FOLLOW-UP-2026-08-30](../PRDs/batch-2026-08-29/FOLLOW-UP-2026-08-30.md) §2 |
