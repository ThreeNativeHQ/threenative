...
| 5 | A user with no C++ toolchain ships a native game from published artifacts | ❌ **there are no published artifacts.** Ten release tags produced zero surviving releases; the lane deletes its own release when the consumer proof fails ([PRD-078](../PRDs/production-readiness/PRD-078-toolchain-free-consumer-proof.md)). Track A step 4 is its precondition |
