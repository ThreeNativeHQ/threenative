...
useless value ledger. **Standing total: 69/100** (was 67 on 2026-08-12: axis 1 +1, axis 5 +1), on a scale that is not
comparable to the ~60/100 the old axes reported.
...
| 5 | **An honest asset-MCP win** — a rerun where the MCP arm beats the no-MCP control | Axis 2 is capped at 10/20 until then | A rerun of PRD-032's live-agent gate |
...
([PRD-064](../PRDs/PRD-064-tier-1-native-reliability.md)), and a controlled engine benchmark with
