...
const coinPositions: Array<[number, number, number]> = [
  [0, 1.45, 3], [0, 1.45, -0.5], [-1.3, 1.5, -9.7], [1.3, 1.5, -13.1],
  [1.8, 3.7, -20.7], [3.5, 3.7, -24.2], [-1.5, 2.3, -34], [0, 2.8, -44.5],
];
...
function tick(dt = 1 / 60) {
  ...
  hazard.position.x = -2.2 + Math.sin(elapsed * 1.35) * 2.1;
  hazard.position.y = 0.43 + Math.abs(Math.sin(elapsed * 2.7)) * 0.13;
  hazard.rotation.y = Math.sin(elapsed * 1.35) > 0 ? -0.5 : 0.5;
  if (player.position.distanceTo(hazard.position) < 1.18) respawn("Ouch! Mushroom patrol.");
  ...
  for (let index = 0; index < coins.length; index += 1) {
    ...
    if (!coin.collected) {
      ...
      if (player.position.distanceTo(coin.object.position) < 1.05) {
        coin.collected = true;
