...
export function createSpringArm(
  ...
  // Behind, above, and a little to the left of the run direction — the framing
  // in the reference, where the route reads diagonally into the screen.
  const offset = options.offset ?? new Vec3(-2.7, 2.2, 5.8);
  const lookAhead = options.lookAhead ?? new Vec3(2.0, 1.0, -0.8);
