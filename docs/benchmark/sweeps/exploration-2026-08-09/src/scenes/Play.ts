...
import { createForestHub } from "../render/forestHub.js";
...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  override enter(ctx: GameCtx): SceneFrame<GameState, PhysicsContext> {
    ...
    ctx.add(createForestHub());
    const springArm = createSpringArm(ctx.camera as PerspectiveCamera, {
      offset: new Vector3(0, 6.2, 10.2),
      lookAhead: new Vector3(0, 0.7, -2.6),
    });

    const materials = createMaterials();
    const sculptureMesh = ball(0.22, materials.player);
    sculptureMesh.name = "sculpture";
    sculptureMesh.position.set(-2, 1.4, -0.4);
    ctx.add(sculptureMesh);
    const levelX = ctx.random.range(-1, 1);
    const pickupX = 1.2 + makeRandom(Math.round((levelX + 1) * 1000))() * 0.8;
    const floorMesh = new Mesh(roundedBox(18, 0.2, 32, 0.08), materials.floor);
