...
const COIN_POSITIONS = [
  [-6.3, 1.45, 0], [-5, 1.25, 0],
  [-3.6, 1.8, 0], [-2.3, 2.2, 0], [-0.8, 2.45, 0],
...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  enter(ctx: GameCtx): void {
    ...
    ctx.camera.position.set(-3, 5.2, 9.3);
  ...
  #buildCoins(ctx: GameCtx): void {
    ...
    COIN_POSITIONS.forEach(([x, y, z], index) => {
      ...
      const area = new Area3D({ physics: ctx.physics, position, shape: CollisionShape3D.sphere(1.5) });
  ...
  #updateCamera(ctx: GameCtx, dt: number): void {
    ...
    const targetX = x + (this.#finished ? 0.9 : 4.2);
    ...
    ctx.camera.position.z += ((this.#finished ? 7.5 : 9.3) - ctx.camera.position.z) * Math.min(1, dt * 2.2);
