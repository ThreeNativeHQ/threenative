...
camera.position.set(5.4, 5.43, 16.5);
...
const cameraOffset = new THREE.Vector3(5.4, 5, 7.5);
const cameraLookOffset = new THREE.Vector3(0, 1.5, -4.8);
const maxCameraDistance = 11.5;

function updateCamera(dt: number) {
  desiredCamera.copy(player.position).add(cameraOffset);
  camera.position.lerp(desiredCamera, 1 - Math.exp(-10 * dt));
  cameraTarget.copy(camera.position).sub(player.position);
  if (cameraTarget.lengthSq() > maxCameraDistance ** 2) {
    camera.position.copy(player.position).add(cameraTarget.setLength(maxCameraDistance));
  }
  cameraTarget.copy(player.position).add(cameraLookOffset);
