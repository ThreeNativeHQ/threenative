...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  override enter(ctx: GameCtx): SceneFrame<GameState, PhysicsContext> {
    ...
    return (frameCtx, dt) => {
      ...
      const directionalInputActive = Math.hypot(move.x, move.y) > 0.08;
      if (!directionalInputActive) transitionInputHeld = false;
      ...
      if (!transitionInputHeld && directionalInputActive) {
