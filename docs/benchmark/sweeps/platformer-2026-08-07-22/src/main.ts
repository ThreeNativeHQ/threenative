  ...
  plugins: [
    ...
    (ctx) =>
      ctx.state.subscribe((state) => {
        ...
        if (status !== null) status.textContent = state.goalReached && state.coins === state.total ? `Trail complete! ${state.coins}/${state.total} stars • press R to replay` : state.goalReached ? `Flag reached — ${state.coins}/${state.total} stars • collect the rest` : state.status;
