...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  #updateCamera(ctx: GameCtx, dt: number): void {
    ...
    const targetX = x + (this.#finished ? 0.9 : 2.7);
    ctx.camera.position.x += (targetX + (this.#finished ? 0.8 : 1.8) - ctx.camera.position.x) * Math.min(1, dt * 3.2);
    ctx.camera.position.y += ((this.#finished ? 4.8 : 5.5) - ctx.camera.position.y) * Math.min(1, dt * 2.2);
