...
class EndlessWorld {
  ...
  update(dt: number, speed: number, player: Player): { crashed: boolean; collected: boolean } {
    const travel = speed * dt;
    const roadSpan = ROAD_LENGTH * ROAD_COUNT;
    for (const segment of this.road) {
      segment.position.z += travel;
      if (segment.position.z > 15) segment.position.z -= roadSpan;
    }
    ...
    for (const obstacle of this.obstacles) {
      ...
      if (!obstacle.passed && obstacle.group.position.z > PLAYER_Z + 0.85) {
        obstacle.passed = true;
        this.#passed += 1;
      }
      if (!obstacle.hit && Math.abs(obstacle.group.position.z - PLAYER_Z) < 0.72) {
      ...
      if (obstacle.group.position.z > 14) this.#recycleObstacle(obstacle);
    }
    ...
    for (const item of this.collectibles) {
      ...
      if (
        item.active &&
        Math.abs(item.group.position.z - PLAYER_Z) < 0.75 &&
      ...
      if (item.group.position.z > 14) this.#recycleCollectible(item);
    }
...
export class Play extends Scene<GameState> {
  ...
  override enter(ctx: GameCtx): SceneFrame<GameState> {
    ...
    return (frameCtx, dt) => {
      ...
      const result = world.update(dt, speed, player);
      if (result.crashed) {
        status = "crashed";
