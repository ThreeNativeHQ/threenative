...
const PLATFORMS: readonly Platform[] = [
  ...
  { x: -1.5, y: 0.8, width: 4.5, depth: 4.4 },
  { x: 3.5, y: 0.2, width: 4.5, depth: 4.4 },
  { x: 8.6, y: 1, width: 4.4, depth: 4.4 },
...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  enter(ctx: GameCtx): void {
    ...
    ctx.camera.position.set(-3, 5.5, 10.5);
    ctx.camera.lookAt(0, 0.9, 0);
    ctx.state.set({ score: 0, total: COIN_POSITIONS.length, status: "Collect the stars and reach the flag", deaths: 0 });
  }
  ...
  #buildHazard(ctx: GameCtx): void {
    ...
    this.#unsubscribe.push(area.on("bodyEntered", (body) => {
      if (body === this.#player?.body) this.#hurt = true;
    }));
  }
  ...
  #buildCoins(ctx: GameCtx): void {
    ...
    COIN_POSITIONS.forEach(([x, y, z], index) => {
      ...
      this.#unsubscribe.push(area.on("bodyEntered", (body) => {
        ...
        ctx.state.set((state) => ({ score: state.score + 1, status: `Great run! ${state.score + 1}/${state.total} stars collected` }));
      }));
    });
  }
  ...
  #buildGoal(ctx: GameCtx): void {
    ...
    this.#unsubscribe.push(this.#goalArea.on("bodyEntered", (body) => {
      ...
      this.#finished = true;
      ctx.state.set((state) => ({ status: `Trail complete! ${state.score}/${state.total} stars • press R to replay` }));
    }));
  }
  ...
  #updateCamera(ctx: GameCtx, dt: number): void {
    ...
    const targetX = x + 2.7;
    ctx.camera.position.x += (targetX + 3.4 - ctx.camera.position.x) * Math.min(1, dt * 3.2);
  ...
  #restartAfterFall(ctx: GameCtx): void {
    this.#hurt = false;
    this.#player?.reset();
    ctx.state.set((state) => ({ deaths: state.deaths + 1, status: "Careful! Back to the trail…" }));
  }
  ...
  #restart(ctx: GameCtx): void {
    ...
    ctx.state.set({ score: 0, status: "Collect the stars and reach the flag", playerX: -9 });
  }
