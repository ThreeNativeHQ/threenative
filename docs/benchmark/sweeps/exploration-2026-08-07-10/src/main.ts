...
let areaTransitionLatched = false;
let isMoving = false;
...
const routeGates = {
  north: { crossingZ: -8.2, returnZ: -6.7, arrivalZ: -20 },
  south: { crossingZ: 8.2, returnZ: 6.7, arrivalZ: 17 },
};
...
function updateArea(): void {
  ...
  if (previous === "hub") {
    if (player.position.z < routeGates.north.crossingZ) next = "north";
    else if (player.position.z > routeGates.south.crossingZ) next = "south";
  } else if (previous === "north" && player.position.z > routeGates.north.returnZ) {
    next = "hub";
  } else if (previous === "south" && player.position.z < routeGates.south.returnZ) {
    next = "hub";
  }
  if (next === previous) return;
  if (next === "hub" && previous !== "hub") state.returns += 1;
  state.area = next;
  areaTransitionLatched = true;
  if (next === "north") player.position.z = routeGates.north.arrivalZ;
  if (next === "south") player.position.z = routeGates.south.arrivalZ;
  setSceneTone(next);
...
function simulate(dt: number): void {
  ...
  const hasDirectionalInput = directionalKeys.some((key) => input.has(key));
  if (!hasDirectionalInput) areaTransitionLatched = false;
  if (!areaTransitionLatched) {
...
function handleKeyUp(event: KeyboardEvent): void {
  ...
  if (!directionalKeys.some((key) => input.has(key))) areaTransitionLatched = false;
  isMoving = directionalKeys.some((key) => input.has(key)) && !areaTransitionLatched;
}
...
installThreePlaytestBridge({
  ...
  diagnostics: () => [],
  entities: [
    { id: "camera.main", object: camera, path: "camera.main" },
    { id: "player", object: player, path: "player" },
  ],
