...
const AREA_META: Record<AreaId, { index: string; label: string }> = {
  ...
  north: { index: "01", label: "Northern Archive" },
  south: { index: "02", label: "Southern Lantern Grove" },
};
...
export function Hud({ game }: { game: Game<GameState, PhysicsContext> }) {
  ...
  const inspectedPoints = useGameState(game, (state) => state.inspectedPoints);
    ...
    <div className="hud-layer pointer-events-none">
      ...
      <aside className={`journal-panel ${journalOpen ? "is-open" : "is-collapsed"} pointer-events-auto`}>
        ...
        <div className="journal-progress">
          <div className="progress-copy">
            <span>{inspectedPoints.length} of 3 notes logged</span>
            <span className="progress-value">{Math.round((inspectedPoints.length / 3) * 100)}%</span>
          </div>
          <div className="progress-track" aria-hidden="true">
            <span style={{ width: `${(inspectedPoints.length / 3) * 100}%` }} />
          </div>
        </div>
          ...
          <>
            ...
            <ol className="journal-list">
              {(Object.keys(POINT_META) as PointId[]).map((pointId) => {
                ...
                const found = inspectedPoints.includes(pointId);
                return (
      ...
      <div className="objective-ribbon">
        ...
        <div className="objective-line">
          ...
          <span>{objectiveComplete ? "Return to the Starwell" : "Map the three listening points"}</span>
        </div>
      </div>
