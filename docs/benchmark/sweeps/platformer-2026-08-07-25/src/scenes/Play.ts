...
const COIN_POSITIONS = [
  [-8, 1.2, 0], [-6, 1.2, 0], [-2.5, 1.2, 0], [1.6, 1.35, 0],
] as const;
...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  enter(ctx: GameCtx): void {
    ...
    ctx.camera.position.set(-3, 5.2, 8.2);
  ...
  #buildPlatforms(ctx: GameCtx): void {
    ...
    this.#addTree(14.1, 0.25, -1.5, 0.82);
  ...
  #buildHazard(ctx: GameCtx): void {
    ...
    const sign = new Mesh(new BoxGeometry(0.12, 0.9, 0.12), standardMaterial(palette.wood));
    sign.position.set(3.35, 0.72, 0.2);
    hazard.add(sign);
    const warning = new Mesh(new ConeGeometry(0.35, 0.1, 4), standardMaterial(palette.gold));
  ...
  #buildCoins(ctx: GameCtx): void {
    ...
    COIN_POSITIONS.forEach(([x, y, z], index) => {
      ...
      const area = new Area3D({ physics: ctx.physics, position, shape: CollisionShape3D.sphere(0.65) });
  ...
  #buildGoal(ctx: GameCtx): void {
    ...
    this.#add(goal);
    const goalArea = new Area3D({ physics: ctx.physics, position: { x: 15.05, y: 1.25, z: 0 }, shape: CollisionShape3D.box(1.4, 2.3, 2) });
  ...
  #updateCamera(ctx: GameCtx, dt: number): void {
    ...
    const targetX = x + (this.#finished ? 0.9 : 5.6);
    ctx.camera.position.x += (targetX + (this.#finished ? 0.8 : 0.5) - ctx.camera.position.x) * Math.min(1, dt * 3.2);
    ctx.camera.position.y += ((this.#finished ? 4.8 : 5.2) - ctx.camera.position.y) * Math.min(1, dt * 2.2);
    ctx.camera.position.z += ((this.#finished ? 7.5 : 8.2) - ctx.camera.position.z) * Math.min(1, dt * 2.2);
