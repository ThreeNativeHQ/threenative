...
const platformSpecs = [
  ...
  { x: 0, z: 0, width: 7, depth: 6, top: 0.65 },
  { x: 0, z: -4.6, width: 5.2, depth: 2.6, top: 0.95 },
  { x: 0, z: -7.4, width: 5.2, depth: 2.6, top: 1.25 },
  { x: 0, z: -11.4, width: 8, depth: 5, top: 1.55 },
] as const;
...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  update(ctx: GameContext, dt: number): void {
    ...
    const routeInput = MathUtils.clamp(move.x + move.y, -1, 1);
    player.update(step, 0, -routeInput, ctx.input.justPressed("jump"));
    updateFollowCamera(ctx.camera as PerspectiveCamera, player.mesh.position);
  ...
  debug(): Record<string, unknown> {
    return {
      coins: this.coins.filter((coin) => coin.collected).length,
  ...
  private animateWorld(dt: number): void {
    ...
    if (this.enemy) {
      this.enemy.rotation.y = Math.sin(this.time * 2.2) * 0.18;
      this.enemy.position.y = 0.8 + Math.sin(this.time * 5) * 0.04;
    }
    if (this.goal) {
      this.goalPulse += dt;
  ...
  private checkHazards(ctx: GameContext): void {
    ...
    const enemyHit = !this.hazardSpent && this.enemy
      ...
      : false;
    if (enemyHit) {
      this.hazardSpent = true;
      if (this.enemy) this.enemy.visible = false;
      this.respawn(ctx, "The snail sprung its trap once—now the trail is clear!");
    } else if (fell) {
      this.respawn(ctx, "Splash! Back to the trail.");
    }
  }
  ...
  private checkGoal(ctx: GameContext): void {
    ...
    const atGoal = Math.hypot(player.mesh.position.x, player.mesh.position.z + 12.5) < 1.35;
    if (!atGoal) return;
    if (this.coins.every((coin) => coin.collected)) {
      this.goalReached = true;
      ctx.state.set({ goalReached: true, status: "won", message: "Every sun coin is home!" });
    } else {
  ...
  private reset(ctx: GameContext, countRespawn: boolean): void {
    ...
    this.hazardSpent = false;
    this.goalReached = false;
    if (this.enemy) this.enemy.visible = true;
    this.player?.teleport(this.spawn);
    for (const coin of this.coins) {
    ...
    updateFollowCamera(ctx.camera as PerspectiveCamera, this.spawn);
  }
}
