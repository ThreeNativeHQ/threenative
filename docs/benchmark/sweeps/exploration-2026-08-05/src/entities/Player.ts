import type { Ctx } from "@threenative/core";
import { CharacterBody3D, CollisionShape3D, type PhysicsContext } from "@threenative/physics";
import { Group, type Material, Mesh } from "three";
import { ball, roundedBox, tube } from "../render/shapes.js";
import type { GameState } from "../state.js";

type GameCtx = Ctx<GameState, PhysicsContext>;

// Tune these two timers for jump feel; they forgive a late or early button press.
const COYOTE_TIME = 0.12;
const JUMP_BUFFER = 0.14;
const JUMP_SPEED = 5;
const MOVE_SPEED = 2;
const SPAWN = { x: -2, y: 0.5, z: 0 } as const;

export type PlayerMaterials = { readonly accent: Material; readonly body: Material };

export class Player {
  readonly mesh: Mesh;
  readonly body: CharacterBody3D;
  #coyoteTime = 0;
  #jumpBuffer = 0;
  #jumps = 0;
  #coyoteJumps = 0;

  #time = 0;
  #visor: Mesh;

  constructor(ctx: GameCtx, materials: PlayerMaterials) {
    this.mesh = new Mesh(roundedBox(0.6, 0.9, 0.6), materials.body);
    this.mesh.position.set(SPAWN.x, SPAWN.y, SPAWN.z);
    this.mesh.castShadow = true;
    const collar = tube(0.22, 0.28, 0.12, materials.accent);
    collar.position.y = 0.34;
    this.#visor = ball(0.14, materials.accent);
    this.#visor.scale.set(1.3, 0.68, 0.7);
    this.#visor.position.set(0, 0.18, -0.28);
    const antenna = new Group();
    const stem = tube(0.035, 0.035, 0.25, materials.accent);
    stem.position.y = 0.64;
    const tip = ball(0.07, materials.accent);
    tip.position.y = 0.79;
    antenna.add(stem, tip);
    this.mesh.add(collar, this.#visor, antenna);
    ctx.add(this.mesh);
    this.body = new CharacterBody3D({
      autostep: { maxHeight: 0.4, minWidth: 0.2 },
      object: this.mesh,
      physics: ctx.physics,
      shape: CollisionShape3D.capsule(0.2, 0.3),
    });
  }

  update(ctx: GameCtx, dt: number): void {
    const grounded = this.body.grounded;
    this.#coyoteTime = Math.max(0, this.#coyoteTime - dt);
    this.#jumpBuffer = Math.max(0, this.#jumpBuffer - dt);
    if (grounded) this.#coyoteTime = COYOTE_TIME;
    if (ctx.input.justPressed("jump")) this.#jumpBuffer = JUMP_BUFFER;
    if (this.#jumpBuffer > 0 && this.#coyoteTime > 0) {
      this.body.velocity.y = JUMP_SPEED;
      this.#jumpBuffer = 0;
      this.#coyoteTime = 0;
      this.#jumps += 1;
      if (!grounded) this.#coyoteJumps += 1;
    }
    const move = ctx.input.vector("move");
    this.body.velocity.x = move.x * MOVE_SPEED;
    // input.vector("move").y is +up; world-space forward is negative z.
    this.body.velocity.z = -move.y * MOVE_SPEED;
    this.body.moveAndSlide(dt);
    this.#time += dt;
    this.#visor.rotation.y = Math.sin(this.#time * 2.4) * 0.12;
    this.mesh.rotation.y = Math.sin(this.#time * 1.8) * 0.045;
  }

  respawn(): void {
    this.body.teleport(SPAWN);
    this.#coyoteTime = 0;
    this.#jumpBuffer = 0;
  }

  debug(): { coyoteJumps: number; grounded: boolean; jumps: number; position: number[] } {
    return {
      coyoteJumps: this.#coyoteJumps,
      grounded: this.body.grounded,
      jumps: this.#jumps,
      position: this.mesh.position.toArray(),
    };
  }

  dispose(): void {
    this.body.dispose();
    this.mesh.removeFromParent();
  }
}
