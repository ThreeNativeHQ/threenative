...
export class Player {
  ...
  constructor(ctx: GameCtx) {
    ...
    const bodyMaterial = standardMaterial(0xff8b38, 0.55);
    const suitMaterial = standardMaterial(0x2b83b6, 0.48);
    const creamMaterial = standardMaterial(0xfff0c9, 0.68);
    const darkMaterial = standardMaterial(0x2b2631);
    ...
    for (const side of [-1, 1]) {
      ...
      this.#visual.add(ear, eye, foot);
    }

    const scarf = new Mesh(
      new CapsuleGeometry(0.08, 0.25, 5, 8),
      standardMaterial(0xf3d34a),
    );
