...
type Coin = { mesh: Group; position: { x: number; y: number; z: number }; phase: number; collected: boolean };
...
export class Play extends Scene<GameState, PhysicsContext> {
  ...
  enter(ctx: GameCtx): void {
    ...
    ctx.camera.position.set(-14, 5.2, 6.5);
  ...
  #buildCoins(): void {
    ...
    COIN_POSITIONS.forEach(([x, y, z], index) => {
      ...
      const item: Coin = { mesh: coin, position, phase: index * 0.7, collected: false };
  ...
  #buildGoal(ctx: GameCtx): void {
    ...
    gate.add(header);
    const banner = new Mesh(new RoundedBoxGeometry(2.1, 1.1, 0.12, 4, 0.08), standardMaterial(0x39a8dc, 0.62));
    banner.position.set(0, 2, -0.82);
    gate.add(banner);
    goal.add(gate);
  ...
  #updateCoins(ctx: GameCtx, dt: number): void {
    for (const coin of this.#coins) {
      if (coin.collected) continue;
      coin.mesh.rotation.y += dt * 3;
      coin.mesh.position.y = coin.position.y + Math.sin(this.#elapsed * 3 + coin.phase) * 0.12;
      if (this.#player !== undefined && this.#player.mesh.position.x >= coin.position.x && Math.abs(this.#player.mesh.position.z - coin.position.z) < 1.8) {
        this.#collectCoin(ctx, coin);
      }
    }
  }

  #collectCoin(ctx: GameCtx, coin: Coin): void {
    coin.collected = true;
    coin.mesh.visible = false;
    this.#collectedCoins += 1;
    ctx.state.set({ coins: this.#collectedCoins, status: `Great run! ${this.#collectedCoins}/${ctx.state.getState().total} stars collected` });
  }
  ...
  #updateCamera(ctx: GameCtx, dt: number): void {
    ...
    const cameraX = targetX - (this.#finished ? 0.5 : 8.5);
    ...
    ctx.camera.position.z += ((this.#finished ? 7.5 : 6.5) - ctx.camera.position.z) * Math.min(1, dt * 2.2);
