...
[PRD-295](../done/PRD-295-the-native-platform-lane-has-never-been-green.md) — this PRD leaves
...
### Reconciliation with PRD-364

PRD-364 phase 6 adds a later scheduling policy for this workflow: pull requests whose complete
merge-base diff is limited to inert Markdown may omit the native platform jobs, while shared source,
configuration, consumed Markdown and every non-pull-request event still select the full lane. The
workflow keeps this PRD's `native` label conditions for the parity jobs, plus `workflow_call`,
`workflow_dispatch` and the nightly trigger. This is a scheduling exemption, not a change to the
platform coverage or to this PRD's unfinished status; PRD-295 still owns whether the advisory lane
can become required.
