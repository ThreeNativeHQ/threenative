...
**Depends on:** PRD-398 (the native UI cadence it builds on; its commits are still local-only on
`fix/native-perf-followups`)
...
- [ ] AC-1 [local; actor: agent]: A core unit test, red then green: a visible mesh under a hidden
  ...
  again after its ancestor moved. Visible matrices equal three's `updateMatrixWorld` output. —
  Evidence: pending.
