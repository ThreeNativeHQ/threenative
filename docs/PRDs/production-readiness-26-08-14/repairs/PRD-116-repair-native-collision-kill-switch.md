...
- [ ] A native character pushes a mutually included dynamic body and does not push a body excluded
  by its collision groups; web proves the same authored semantics.
- [ ] Removing the native groups assignment makes the focused native regression red while web stays
  green.
- [ ] Push-disabled remains an absolute no-motion control.
- [ ] The native host-boundary record distinguishes Rust/C ABI, desktop V8 binding, aggregate
  desktop wrapper, Android/iOS, simulator, and hardware execution; nothing unrun is claimed.
- [ ] Every counted absorbed-runtime area has a current line count, owner, live proof/caller,
  alternative, and KEEP/DELETE verdict; any rejected area triggers re-scope rather than concealment.
- [ ] `LIMITS.nativeRuntimeLoc` remains 50,000, its warning remains visible, and final/residual LOC
  plus repair delta are recorded.
- [ ] Every verification test count equals final raw command output.
- [ ] Caller census, revert checks, focused gates, and
...
This is planned work. Execution replaces expected-red specifications with exact displacements,
group values, commands/exits, engine name, host limitations, caller lines, LOC attribution and
residual, kill-switch verdicts, final test counts, and restored-green output. Compile-only evidence
does not prove character behavior.
