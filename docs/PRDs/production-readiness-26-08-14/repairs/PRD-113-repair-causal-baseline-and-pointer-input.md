...
**Status: PLANNED, 2026-08-15.** Fresh repair after review 2 of
...
- [ ] Anonymous movement selects a candidate only when it has valid input-on movement and every
...
- [ ] Invalid or missing interval duration remains fail-closed through the existing movement path.
- [ ] The browser runner records a step with `pointerPosition` as input-driven for that step even
  when the pointer has no held button; release-only samples remain input-off.
- [ ] The Android/iOS runner records the same buttonless `pointerPosition` step as input-driven;
  its existing pointer release behavior and held-key parity remain unchanged.
- [ ] No public scenario/assertion/protocol field, gameplay identifier, or package is added.

**Wiring (the phase is not done without this):**

- [ ] Caller edited: `packages/playtest/src/runner/runner.ts:857` consumes the strict causal baseline.
- [ ] Browser input path edited: `packages/playtest/src/runner/runner.ts:588` uses the same explicit
  pointer-position semantics as the scenario helper.
- [ ] Native input path edited: `packages/playtest/src/runner/androidRunner.ts:229` matches browser
  classification for buttonless pointer movement.
- [ ] Existing four-file scope remains exact; no source PRD, archive, verification document,
...
- [ ] Anonymous movement passes only when the selected entity has positive input-on movement and
  zero displacement in every valid input-off interval recorded for that entity.
- [ ] An autonomous entity moving during any input-off interval cannot satisfy anonymous movement,
  even when its input-on displacement/rate is larger.
- [ ] Missing, invalid, or incomparable causal samples fail closed through the existing movement
  assertion result.
- [ ] Browser and Android/iOS classify a buttonless `pointerPosition` step as input-driven for its
  interval; release-only and no-input intervals remain input-off.
- [ ] Existing omitted-press held-state parity, anonymous selector omission, render-time duration,
  and positive input-sensitive movement behavior remain intact.
- [ ] All new and preserved red controls fail before restoration; focused tests, the full chained
  gate, and budgets pass after restoration.
- [ ] Only the four scoped files change, and the separate physics-puzzle archive evidence remains
  `1/6`.
