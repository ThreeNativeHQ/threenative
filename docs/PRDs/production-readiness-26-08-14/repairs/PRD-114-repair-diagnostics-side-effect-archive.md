...
- [ ] Both verification documents say framework seed fail/diagnostics pass and vanilla seed
  pass/diagnostics fail, exactly matching committed `proof.json` assertion rows.
- [ ] The round remains VOID; no functional, visual, parity, fresh-builder, native, or deletion
  conclusion is promoted.
- [ ] `archiveSandbox` rejects a missing sibling referenced only by a static side-effect relative
  import, names the source/specifier, and removes the partial archive.
- [ ] Present siblings, bound imports, single quotes, and double quotes retain coverage.
- [ ] Caller census and revert checks show the guard is live in `pnpm sweep:archive`.
- [ ] Focused tests and `pnpm typecheck && pnpm lint && pnpm test` pass after controls are restored.

## Verification Evidence

Contract conformance: prd_contract: v1

This is planned work. Execution records the exact raw assertion rows, document diff, focused test
count, observed-red archive error, partial-directory census, caller lines, revert failure, and final
gates. No rerun or platform execution is claimed.
