...
| [`requires-charged-device/`](requires-charged-device/) | [PRD-155](requires-charged-device/PRD-155-rendering-defect-sweep-and-native-visual-evidence.md) | A Pixel 8 charged to 50%, thermal `NONE`, and running on battery. Phases 1–4 are complete; only the D4 shadow-cost measurement is waiting, and no code change is waiting on it. Running the game draws more than USB supplies, so the phone has to be charged while idle |
