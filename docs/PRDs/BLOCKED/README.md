...
| [`requires-release-credentials/`](requires-release-credentials/) | [PRD-445](requires-release-credentials/PRD-445-public-release-hygiene.md) | The owner-set `CLOUDFLARE_API_TOKEN` on the `site-production` environment (a `site` run on `main` then goes green), an upstream `threenative-sculpt-mcp` release that moves its `sharp` pin off `0.35.3`, and three owner calls on tracked repository junk. Every box in it is ticked; only the `## Blocked on` list is open. Filed here 2026-09-25 under R6 |
