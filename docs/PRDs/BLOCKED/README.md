...
| [`requires-release-credentials/`](requires-release-credentials/) | [PRD-060](requires-release-credentials/PRD-060-promoted-consumer-distribution.md), [PRD-196](../production-readiness/critical/PRD-196-published-install-is-functional.md) | Candidate-cohort preparation, release, registry, and platform-signing dependencies. PRD-196 was filed here on 2026-09-08: its gates and install plumbing are implemented and green, but eight published package versions are immutable and need a fresh coherent cohort before the `npm publish` and matching `runtime-native-v*` release can happen |
