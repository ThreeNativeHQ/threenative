...
| 4 | Deleting an imported sibling from an archive fails the archive step — the instrument's own control, observed red | **yes** — run deliberately on the framework re-seal sandbox. Removing `threenative.config.ts`, which `src/` imports, makes `pnpm sweep:archive` exit `1` with `Refusing to archive an unbootable project; src/ imports files the archive does not carry`. Restoring it archives green. The same control fired unprompted when that archive was first built, which is how the missing file was found |
