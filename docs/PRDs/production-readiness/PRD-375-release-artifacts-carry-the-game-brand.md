...
- [x] Execute the three real macOS packager/decoder regressions and a real branded-container macOS lane.
      Run `35184227755` on `9ed461042ff77b141ca468d2315b1ff26808b5a3`: **38/38** macOS icon tests passed; across the three verifier suites **140 passed / 0 failed / 1 platform-specific skip**. Run `35184231475`, macOS job `105083173664`, also passed the actual native release lane with packaged/relocated container brand inspection and launch. The current PR candidate is still being revalidated by normal CI; those runs prove the Apple-tool behavior, not final merge readiness.
