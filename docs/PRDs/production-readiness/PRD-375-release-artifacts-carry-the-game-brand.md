...
confirmed this by passing a file whose whole content was a plain-text string. That historical macOS icon-content gap is closed by the follow-up below: the verifier now runs real `sips`/`iconutil` decoding on macOS and compares decoded RGBA. The earlier provenance-only claim remains historical evidence, not the current verifier contract. PRD-365's writer emits no `loading` record, so a
...
- [ ] User verification performed on the named platform — owner/human inspection pending
      **linux-x64 done and owner-inspectable; Windows/macOS machine acceptance has run in CI; owner inspection of the Linux artifact is still pending.** A real PRD-365 container was built from a starter branded only in game files (`orbit-brand.tar.gz` sha256 `1da75994…`) and the shipped CLI passed on it end to end: `starter desktop gate passed: 300 frames, 21910 colors, 337 asset pixels, brand Orbit Brand verified`, exit 0, with a non-blank 1280x720 capture (`e099b09d…`) the implementing agent viewed as the drawn starter scene, not a person. The real `.desktop` launcher entry is `desktop-file-validate`-clean and names `Orbit Brand`; three negative controls fire on the real artifact, the name control in 0.17 s, before any launch. Rebuilt since, with `bootSplash` **kept** — the arm a real scaffolded project produces, which only became possible once the container recorded its loading sequence — and with authored art a person can look at: a 256x256 amber ringed planet (`48b3782a…`) and a matching 1024x576 splash (`86b5578f…`), both viewed by the implementing agent rather than by a person, replacing the first run's 1x1 PNG. Container `f93e0953…`; `--brand-only` exit 0 and the full launching gate exit 0, both against the stock config. The owner's launch command is in the evidence record. Windows and macOS are now wired into the existing `native-platforms` `desktop` matrix job through `--brand-only`, which needs no display and therefore also runs on the Windows leg that cannot launch. The box stays open because the project still requires owner/human inspection on the operator-backed Linux path; Windows/macOS are CI-only by the owner's recorded policy. Exact-candidate platform revalidation for this follow-up is tracked below.
- [x] Hosted macOS machine acceptance at exact PR head `97a33e5b` (machine evidence only, no person looked)
      Run `35383735799` (`pull_request`, `head_sha 97a33e5b51bcd14ee065fdf48852d347764e8443`; container `candidate.sha fd3bc8239bd3538d1c37d0c720933c184cc35b26`), job `native-platforms / macOS desktop core` `105771487546`: steps "Inspect the release container's game brand" and "Verify the relocated release container with the installed verifier" both `success`. Artifact `10563452446` (`native-desktop-release-macOS`, sha256 `99a9f4b8251d519c5de985b2ade769fc7f6ecfed6204246f231c82ea12f4aa59`): `release-container-macOS/brand.txt` = `container brand verified: threenative-starter-native`; manifest icon ICNS sha256 `f8f3071ffc5df09af19cd1954858792a822c7a3fa46f963fa768567c81608db4` from source `48b3782a7fc5731f088eef3ecf17464cfd41a2219a2758d9f21d2da4ab89d907`, decoded by `iconutil-decoded-rgba` across 10 representations; `Info.plist` name `threenative-starter-native`; `loading.bootSplash` `#0d1b2a`; archived container sha256 `ed404c50d9b5df579998cb7c779b66e416b5f71e02899880d42d9cd160fe7da5`; retained `starter-container.png` sha256 `19ea34ec8402f1eb470ce1128965b233d9702809d4e771d7284008d999e637b9` (1024x653, 20741 colours, cyan 284 / magenta 431 authored asset pixels, 300 frames, `pass: true`). No person has seen this capture.
...
- [x] Execute the three real macOS packager/decoder regressions and a real branded-container macOS lane.
      Run `35184227755` on `9ed461042ff77b141ca468d2315b1ff26808b5a3`: **38/38** macOS icon tests passed; across the three verifier suites **140 passed / 0 failed / 1 platform-specific skip**. Run `35184231475`, macOS job `105083173664`, also passed the actual native release lane with packaged/relocated container brand inspection and launch. Exact-candidate run `35381012258`, macOS job `105718001998`, then passed the same brand inspection, installed-verifier check, retained capture/report and clean-toolchain launch against merge candidate `63c8145beb93666e1206a58d927be7804d8a57d5`. Exact PR head `97a33e5b` repeated the hosted macOS lane in run `35383735799` (job `105771487546`; artifacts `10563452446` and `10564100488`); the same run's `native-platforms / Windows desktop core` job `105771488549` also reached and passed its packaging/brand/capture steps (artifact `10563924933`).
- [x] Run full-workspace typecheck, lint, test and budget gates on this candidate.
      Exact-head CI run `35381012258` on branch head `a32a548243aeb4245b7e5c23bc551e92d27e55a7` / merge candidate `63c8145beb93666e1206a58d927be7804d8a57d5` passed `typecheck`, `lint`, `test`, all three `test-unit` shards, `budgets`, `test-browser`, `test-playtest`, `test-native`, `build`, `benchmark`, `performance-contracts`, `supply-chain`, `website` and artifact-build gates. The same run's first Windows native-platform attempt later failed in the pre-existing `threenative-worker-production-test` by the generic 120 s watchdog; the exact `develop` base's CI run `35212649736` passed that contract. That platform retry is tracked separately and does not erase the completed workspace gate.
...
- [ ] Loading and playable-frame handoff remain nonblank and responsive within actual safe areas; progress is measured rather than fabricated.
      **The progress half is now closed and was a real finding.** `ctx.startup.progress` was documented and commented as "monotonic and honest" and was not monotonic: requesting a second asset after the first settled took the reported value from 0.7 to 0.35, and the file-count-to-byte branch switch could drop it further. It was never fabricated — it is tied to settled/requested bytes or files, with no wall-clock term — so the finding is the false monotonicity claim, not a fake bar. Fixed as an enforced high-water mark over the measured state (`packages/core/src/game.ts`), red-green at `packages/core/__tests__/startup-progress-honesty.spec.ts`. The old Windows launch blocker is resolved: PR #258 (`a3a2f221d06d0b08ffb5fe4a943d9c3ae8f898a4`, merged 2026-09-16) moved the release game to sidecar `game.bundle`, re-enabled the Windows verifier/launch steps, and CI run `35122481605` passed. Issue #264 is closed. Exact-candidate macOS revalidation passed in run `35381012258` (job `105718001998`), and exact PR head `97a33e5b` repeated it in run `35383735799` (job `105771487546`, artifact `10564100488` `native-desktop-macOS` sha256 `603fa0d228018c69c373ffcf1a7d2a87289477dbfe0d5383a365fa9e6c1eba31`: `loading-proof.json` `pass: true` — `TN_LOADING_PROOF_OVERLAY_VISIBLE` → `..._DISMISSED`, presents 60/60 to frame 1860, startup/mid-stall `loadingPixels 913920` vs settled `0`; `desktop-darwin-report.json` `pass: true`, 300 frames, screenshot sha256 `d8fcb3921d4e969879dbb7680893ecc7cfc9882f7f7dfd0719a9769ca97ca67e`). This is machine capture evidence, not human inspection, and does not by itself prove safe-area appearance, so the box stays open. The earlier run `35381012258` Windows attempt had stopped in an unrelated worker-production contract timeout before packaging/brand/capture; at `97a33e5b` the same run's `native-platforms / Windows desktop core` job `105771488549` passed those steps (artifact `10563924933`, brand `container brand verified: threenative-starter-native`, capture sha256 `2b27c883cce36cbfbeb4da886787582da33dfa8e12a5bfa03cf89be7a38554f0`), so that specific blocker is gone; the human safe-area/responsiveness inspection remains open.
- [ ] Every target capture is retained against the exact candidate SHA and accepted: by human inspection on the platforms this project has an operator for (web, Android, Linux desktop), and on Windows — which this project verifies only through CI — by the named machine assertions in `inspectStarterScreenshot` (non-blank, above the unrendered-frame colour floor, authored proof asset present and localized by channel margin, bounds fraction and density) together with the retained artifact. Those assertions prove a drawn frame containing the authored asset; **they do not prove the scene looks correct**, and no Windows capture is claimed to have been seen by a person. The macOS half is recorded as its own checked box below. Same-artifact playback proof and iOS historical status are unchanged.
      **No human inspection has happened on any platform.** The Linux capture and the authored art were viewed by the implementing agent before shipping — a look at the pixels rather than trust in a hash — which is recorded in the evidence file and explicitly does **not** satisfy this criterion's human-inspection half; the owner has not yet run the launch command he was given. Same-artifact playback proof *is* satisfied, by the artifact path, container hash and capture sha256 recorded in the container report. The former Windows blocker (#264) is resolved by merged PR #258; this box now remains open for exact-candidate capture acceptance plus the required human inspection on operator-backed targets.
      **Split for precision (2026-09-22):** the exact-head macOS machine half is recorded as its own box below; this combined box stays open for the Windows half and the human-inspection half.
...
- [x] Exact-head macOS capture retained and machine-accepted at PR head `97a33e5b` (machine assertion half only; no person has seen it)
      Run `35383735799` (`head_sha 97a33e5b51bcd14ee065fdf48852d347764e8443`), artifact `10563452446` (`native-desktop-release-macOS`, sha256 `99a9f4b8251d519c5de985b2ade769fc7f6ecfed6204246f231c82ea12f4aa59`): retained `release-container-macOS/starter-container.png` sha256 `19ea34ec8402f1eb470ce1128965b233d9702809d4e771d7284008d999e637b9`, 1024x653, 20741 colours, cyan 284 / magenta 431 authored asset pixels, 300 frames, `pass: true`, brand `container brand verified: threenative-starter-native`; container archive sha256 `ed404c50d9b5df579998cb7c779b66e416b5f71e02899880d42d9cd160fe7da5` plus the container's own `candidate.sha fd3bc8239bd3538d1c37d0c720933c184cc35b26` and capture hash give same-artifact playback. These are the `inspectStarterScreenshot` machine assertions; they do not assert the scene looks correct and no person has seen the capture.
