...
- [ ] The `native-platforms` `desktop` matrix job runs `--brand-only` for this candidate.
      proof: CI job `native-platforms` (`macOS desktop core`, `Windows desktop core`) — the same
      inspection that ran green on `3790d0908` (run 35055978321), re-run for this candidate.
      **linux-x64 done and owner-inspectable; Windows and macOS wired to CI.** A real PRD-365 container was built from a starter branded only in game files (`orbit-brand.tar.gz` sha256 `1da75994…`) and the shipped CLI passed on it end to end: `starter desktop gate passed: 300 frames, 21910 colors, 337 asset pixels, brand Orbit Brand verified`, exit 0, with a non-blank 1280x720 capture (`e099b09d…`) the implementing agent viewed as the drawn starter scene, not a person. The real `.desktop` launcher entry is `desktop-file-validate`-clean and names `Orbit Brand`; three negative controls fire on the real artifact, the name control in 0.17 s, before any launch. Rebuilt since, with `bootSplash` **kept** — the arm a real scaffolded project produces, which only became possible once the container recorded its loading sequence — and with authored art a person can look at: a 256x256 amber ringed planet (`48b3782a…`) and a matching 1024x576 splash (`86b5578f…`), both viewed by the implementing agent rather than by a person, replacing the first run's 1x1 PNG. Container `f93e0953…`; `--brand-only` exit 0 and the full launching gate exit 0, both against the stock config. The owner's launch command is in the evidence record. Windows and macOS run in the existing `native-platforms` `desktop` matrix job (`macOS desktop core`, `Windows desktop core`) through `--brand-only`, which needs no display and therefore also runs on the Windows leg that cannot launch; that inspection already ran green on `3790d0908` (run 35055978321) against a real `rcedit`-written PE resource directory and a real `.app`. The box stays open until that job runs `--brand-only` for this candidate, and because the owner has not yet opened the Linux container from a real file manager himself.
...
- [ ] Loading and playable-frame handoff remain nonblank and responsive within actual safe areas;
      progress is measured rather than fabricated. proof: CI job `native-platforms`
      (`macOS desktop core`, `Windows desktop core`) collecting `starter-container.png` and
      `starter-container-report.json` for this candidate; the progress half is already red-green at
      `packages/core/__tests__/startup-progress-honesty.spec.ts`.
      **The progress half is now closed and was a real finding.** `ctx.startup.progress` was documented and commented as "monotonic and honest" and was not monotonic: requesting a second asset after the first settled took the reported value from 0.7 to 0.35, and the file-count-to-byte branch switch could drop it further. It was never fabricated — it is tied to settled/requested bytes or files, with no wall-clock term — so the finding is the false monotonicity claim, not a fake bar. Fixed as an enforced high-water mark over the measured state (`packages/core/src/game.ts`), red-green at `packages/core/__tests__/startup-progress-honesty.spec.ts`. The handoff half: Linux done; macOS and Windows are CI-only in the `native-platforms` `desktop` matrix job (`macOS desktop core`, `Windows desktop core`), which runs the installed verifier against the relocated container and collects `starter-container.png`/`starter-container-report.json`. Windows's blocker [#264](https://github.com/ThreeNativeHQ/threenative/issues/264) is fixed — the game now travels as `game.bundle` beside the executable, so `rcedit` rewriting the PE cannot discard it — and PRD-365's acceptance record shows the Windows container launching. The box stays open until that job runs for this candidate.
- [ ] Every target capture is retained against the exact candidate SHA and passes the machine
      assertions in `inspectStarterScreenshot` (non-blank, above the unrendered-frame colour floor,
      authored proof asset present and localized by channel margin, bounds fraction and density).
      proof: CI job `native-platforms` uploading `starter-container.png` plus the retained report
      for each `desktop` leg. Those assertions prove a drawn frame containing the authored asset;
      **they do not prove the scene looks correct** — the human half is under *Blocked on*.

## Blocked on

- **A person looking at the captures** (web, Android, Linux desktop, and the owner's own launch of
  the Linux container from a real file manager): unblocked when the owner inspects them. The
  implementing agent viewing the pixels is a look, not a person, and does not close this.
...
- **2026-09-25 (owner, R3) — human inspection moved out of the boxes.** The machine assertions
  stay a box (CI job `native-platforms`); the person looking at the captures is now a *Blocked on*
  line, because no agent can close it.
