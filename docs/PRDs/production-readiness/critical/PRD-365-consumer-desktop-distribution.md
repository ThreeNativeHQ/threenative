...
- [ ] User verification performed on the named platform
  - NOT RUN locally; now CI-only, per the owner's 2026-09-23 decision that each developer signs their own game and `threenative build --mode release` signs with the developer's credential, proven with test credentials on Windows and macOS CI. The `native-platforms` reusable workflow's `desktop` matrix job carries both halves: `Windows desktop core` builds the release container with `THREENATIVE_DESKTOP_SIGN_SUBJECT=ThreeNative CI Signing Proof` (`signtool /n`, read back with `Get-AuthenticodeSignature`), and `macOS desktop core` now generates a self-signed codeSigning identity on the runner, builds with `THREENATIVE_DESKTOP_CODESIGN_IDENTITY='ThreeNative CI Signing Proof'`, and reads it back with `codesign --verify --strict --deep` and `codesign -dv` (`TN_DESKTOP_SIGNING_PROOF_SCHEME:codesign`). The box stays open until that run exists for this candidate; both certificates are self-signed and discarded, so public Authenticode trust and Apple notarization stay PENDING under PRD-060.
  - 2026-09-24 CI fix (PR #302, run `35982709001`, job `107578626210` red at `Create a self-signed code-signing certificate for the macOS proof`): the throwaway PKCS#12 was exported with OpenSSL 3's modern default (AES-256-CBC, SHA-256 MAC), which exports cleanly but `security import` rejects with `SecKeychainItemImport: MAC verification failed during PKCS12 import (wrong password?)`; the `-legacy` fallback never ran because the modern export succeeded. The step now leads with `openssl pkcs12 -export -legacy` (3DES/SHA1, the format macOS Keychain accepts) and falls back to the modern default only where `-legacy` is unavailable. Locally on OpenSSL 3.6.4 the `-legacy` export produces `MAC: sha1, pbeWithSHA1And3-KeyTripleDES-CBC`; the box stays open until the green macOS `desktop core` run is observed for this candidate.
