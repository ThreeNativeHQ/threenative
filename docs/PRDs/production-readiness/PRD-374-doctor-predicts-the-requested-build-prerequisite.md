...
- [x] Callers wired and building: `packages/create-threenative/src/threenative.ts`, `packages/create-threenative/src/doctor.ts`, `packages/create-threenative/__tests__/doctor.spec.ts` (+1 more) — commits `9d50cb878` (wiring; was `c9dd6288a` before the branch was rebuilt on current `main`, same net diff), `c037860f2` (four defects from the first independent review, as its own subject says; review 1's remaining two findings — the stale retention index and a dead SHA — are `811eae6ef`) and the second review's blocker fix in this commit; `pnpm typecheck` exit 0, `pnpm lint` exit 0, and `pnpm budgets` exit 0 after the retention index was regenerated (it was red — the reviewer found it, and regenerating `docs/benchmark/SCREENSHOT-RETENTION.md` fixed it).
- [x] Required test green: `packages/create-threenative/__tests__/doctor.spec.ts` — **87 passed** (68 when the phase first landed; the rest are the five review rounds' regression tests), plus `cli.spec.ts` 5 passed for the argument validation. Whole package **668 passed across 38 files**. Measured at this commit: an earlier edit recorded 84/665 and a test landed after it, which the fifth review caught.
...
- [x] Required test green: `packages/create-threenative/__tests__/doctor.spec.ts` — **87 passed** (75 when the phase first landed, 68 before it), and the whole package **668 passed across 38 files**. Both named cases exist: conversion unavailable when the Blender MCP starts with Blender missing, and a malformed host config preserved while its exact path is reported.
