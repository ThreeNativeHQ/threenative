...
- [ ] Observed red — the hosted lane reached and fixed two real failures (run 34725179598: the full non-iOS cohort was demanded while `build-android` uploaded nothing; run 34730868090: the consumer build's config fell through to a repo-relative fixture), then went green on run 34736214113. The Windows *toolchain* controls now run inline in the job. Run 34743159341 failed the mask control because a bare `cl` from Git Bash is not resolved through PATHEXT (it returned 127, not the shim's 97); fixed by invoking `cmd.exe //c "cl /?"`. Run 34745227091 then passed that control and falsified the helper control's premise: deleting the installed helper does **not** fail the build — it is re-fetched from the served release with no compiler invoked. The control and the acceptance criterion are corrected to that measured behaviour; the hosted result on the next run closes this box.
...
- [ ] Removing the installed helper makes the build fail naming `prebuilt/win32-x64/mystral-tools.exe`, with no silent source-build fallback. — **FALSIFIED by run 34745227091**: deleting the helper does not fail the build. The build re-provisions it from the served release (`install-prebuilt.mjs`'s `isVerifiedInstall` returns false and the install re-fetches), the build completes, and the toolchain log stays empty. Replaced by the corrected criterion below.
