...
**Status:** PARTIAL — phase 1's local-tarball browser gameplay proof is recorded below; a public-registry cohort install still waits on PRD-196. Phase 2's cross-target scenario policy was corrected in `d10375e500df5d82cf93fe3487e7d70124f1a157`; the bounded evidence-hardening checkpoint below passes 42 isolated Node regressions, but repository gates, real distributed desktop/Android consumer runs and independent approval remain unverified for this repair. The 2026-09-15 native-consumer repair (portable `KeyR` restart, platform-library noise not counted as game console errors) passes its focused lanes — playtest red 3 failed → 28 passed, runtime-native 73 passed, scaffold 61 passed, both package typechecks — and its first real distributed consumer rows now execute locally: a starter scaffolded from the candidate's 11 packed tarballs qualifies on **Linux x64 desktop** (exit 0, `pass: true`, 5 real assertions, 0 failures, host built from this candidate) and on **Android emulator** (exit 0, `pass: true`, 5 assertions, APK built through the `--allow-source-build` + QuickJS rollback), both carrying the same scenario hash and application id. Phase 2's user-verification box is ticked for those two targets; macOS/Windows, PRD-365 release containers and arm64-v8a Android stay unrun and are named, not claimed. A physical Pixel 8 run fails closed on Android 17 (`diagnostics` not evaluated) — a real phase-3 finding. Phase 3 physical Android qualification remains open, as does independent review. Revised 2026-09-08; phase 1 worked 2026-09-12; phase 2 contract and repair worked 2026-09-15.
...
- [x] User verification performed on the named platform
      **Desktop (Linux x64) and Android (emulator) both qualify with real rows; scope stated below.** The earlier "environmentally blocked (host GBM buffer creation)" and "`fetch failed`, no APK exists" notes are both superseded. The candidate's own desktop host was built in this worktree (`mystral` `3bd25e2dabe163fb…`, 405/405 targets) because the primary checkout's Sep-9 binary predates `e36887a05`; the starter was scaffolded from the 11 packed candidate tarballs and installed, and the generated game's own `test:native` script — the exact command CI's `starter-linux` job runs — exited 0: container gate `300 frames, 21951 colors, 336 asset pixels`, then `consumer gameplay qualified on desktop: 5 assertions, artifact b0f7d4e28e11`. Android needed the packager's documented `THREENATIVE_RUNTIME_SOURCE=… --allow-source-build` hatch plus the `-PthreenativeJsEngine=quickjs` rollback (`v8-android` has no usable payload and falls through to a ~3.5 h Chromium compile); `BUILD SUCCESSFUL in 2m 34s` produced a 47.6 MB APK, 16 KB-page clean on both ABIs, and it qualified on `emulator-5556`: `5 assertions, artifact 3f24116fb41a`. Both rows carry `pass: true`, `failures: []`, 5 real assertions and the **same** `scenarioHash` `4edb52f1fb8d6ded…` and `applicationId` `com.threenative.starternative` with different artifact hashes — one game, one scenario, two distributed targets. Assertions read real observations on both: score 1→0 and entityCount 3→4 across restart, 0 console errors with `runtimeReady`, `-z` movement delta 3.99997 (≥ 0.5), player visibility 3699.6 projected px (≥ 20). The C1 network waiver is closed by execution, with its reason recorded rather than the assertion skipped. **Not covered by this box:** macOS and Windows rows (produced by the same `test:native` script on the hosted `desktop` matrix job, unrun here); PRD-365 release containers (this row is the `dist-native` executable, not a relocated container); and arm64-v8a Android — the passing Android row is x86_64 emulator. A physical Pixel 8 run of the same APK **fails closed** (`exit 1`, `TN_STARTER_CONSUMER_NO_ASSERTIONS: assertion 'diagnostics' was not evaluated`, `pass: false`, `assertions: 0`, `arm64-v8a`, `17 (API 37)`), which proves the fail-closed contract on hardware and opens a real phase-3 finding: the diagnostics channel is not evaluated on API 37 while it is on API 35. Commands, identities and the full assertion tables: the phase evidence.
