...
- [ ] User verification performed on the named platform
      **Desktop (Linux x64) VERIFIED; Android still open, so the box stays open.** The earlier "environmentally blocked (host GBM buffer creation)" note is superseded: no GBM error occurred. The candidate's own host was built in this worktree (`mystral` `3bd25e2dabe163fb…`, 405/405 targets) because the primary checkout's Sep-9 binary predates `e36887a05`; the starter was scaffolded from the 11 packed candidate tarballs, installed, and the generated game's own `test:native` script — the exact command CI's `starter-linux` job runs — exited 0: container gate `300 frames, 21951 colors, 336 asset pixels`, then `consumer gameplay qualified on desktop: 5 assertions, artifact b0f7d4e28e11, app com.threenative.starternative`. The row records `pass: true`, `failures: []`, artifact `b0f7d4e28e115b6a422eec851a2b4525c953c672088482e74d01d97e8b5fd1bf`, scenario hash `4edb52f1fb8d6ded…`, `linux`/`7.2.3-1-cachyos`/`x64`/`wayland`, tick 985. All five assertions read real observations: score 1→0, entityCount 3→4 across restart, 0 console errors with `runtimeReady`, `-z` movement delta 3.99997 (≥ 0.5), player visibility 3699.6 projected px (≥ 20). The C1 network-waiver is closed by execution, with the reason recorded in the report rather than the assertion skipped. macOS and Windows rows are named, not claimed. **Android:** the recorded `fetch failed` blocker is partly stale — the packager's documented `THREENATIVE_RUNTIME_SOURCE=… --allow-source-build` hatch gets past the fetch, and `adb devices -l` shows two emulators plus a physical Pixel 8. The real gate is `v8-android`, whose provisioning has no usable payload and falls through to compiling V8 from Chromium source (3519 ninja steps); no APK and no Android row were produced. Commands, identities and the full assertion table: the phase evidence.
