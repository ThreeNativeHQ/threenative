...
| Conditional | [368 — Compiled pipelines survive a relaunch](../BLOCKED/requires-pipeline-cache-api/PRD-368-compiled-pipelines-survive-a-relaunch.md) | Revisit if the graph result leaves worthwhile repeat-launch savings. | Prove a version-pinned patch can expose the required native cache operations and keep its build/test burden bounded; no persistent cache implementation is accepted yet. |
