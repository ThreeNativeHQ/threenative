...
| [253](../BLOCKED/requires-portable-native-residency-consumer/PRD-253-content-residency-and-screen-space-hlod.md) **BLOCKED** | Browser Bistro proved a real 2.34 GB decoded / 4.37M-visible-triangle bound, but the root-relative asset URL failed on native desktop before the playtest bridge. No product code shipped; PRD-251 must not invent a second residency scheduler while this is blocked. | [`3DTilesRendererJS`](https://github.com/NASA-AMMOS/3DTilesRendererJS) mechanisms + existing `meshoptimizer` tooling | 10 → HIGH |
...
tarballs. Neither occupies the active queue. PRD-251 must consume the eventual generic residency
scheduler rather than inventing a second one. 243 and 246 remain behind the already-shipped compute
