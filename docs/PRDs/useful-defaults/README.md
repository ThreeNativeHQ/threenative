...
**Status: OPEN — integrated 2026-09-01 against the current main tree. PRD-289 through PRD-292 are
done and archived; the other fourteen keep the status they arrived with.**
...
The PRD-289 through PRD-292 recovery was inventoried, ordered, and given a safe single-worktree
procedure in [RESUME-2026-09-01.md](./RESUME-2026-09-01.md); their completion is recorded in
`docs/verification/` and their individual PRDs are archived in `docs/PRDs/done/`.
...
| --- | --- |
| [289 — the conventions the templates document also run in them](../done/PRD-289-the-conventions-the-templates-document-also-run-in-them.md) | `GroundSnap`, `normaliseToMetres`, `attachToBone` and `AnimationPlayer` now have live generated-template call sites or reasoned N/A rows, with a drift gate. |
| [290 — a game that fails to start says why, on the screen](../done/PRD-290-a-game-that-fails-to-start-says-why-on-the-screen.md) | Boot failures now dismiss the launch surface and render an assertable, styled message on the browser path; the evidence record names native limitations honestly. |
| [291 — a template is playable with the input the device has](../done/PRD-291-a-template-is-playable-with-the-input-the-device-has.md) | Templates now ship portable touch controls and desktop visibility proof; the evidence record names the unexecuted physical-device lane. |
| [292 — a fast body does not pass through a wall](../done/PRD-292-a-fast-body-does-not-pass-through-a-wall.md) | Dynamic bodies now default to measured continuous collision on web and native, with a named opt-out and effective-setting report. |
...
| 8 | [291 — a template is playable with the input the device has](../done/PRD-291-a-template-is-playable-with-the-input-the-device-has.md) | DONE | Touch controls are generated in each applicable template, with browser proof and honest native-device evidence. |
| 9 | [292 — a fast body does not pass through a wall](../done/PRD-292-a-fast-body-does-not-pass-through-a-wall.md) | DONE | The framework owns the cross-backend CCD seam, measured default, opt-out, reporting, and web/native projectile proof. |
