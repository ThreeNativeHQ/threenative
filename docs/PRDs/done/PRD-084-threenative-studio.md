...
**Status: DONE, 2026-08-12 — Phases 0–3 passed.** The owner decision is recorded at the foot
of this file. The spike measured preserved state, a 16.994 second sentence-to-proof loop, and
220 lines of disposable glue, with both required controls observed red. The packed Studio
consumer then proved browser chat, bounded source editing, HMR, three playtest assertions,
and ordinary-git checkpoint/restore. See
...
evidence, not a new argument.** So this PRD does not argue. **The owner reopened it on
2026-08-12** and admitted the package; what is still owed is the evidence. Phase 0 is a
throwaway spike that produces three numbers, and Phase 1 is designed against those numbers
rather than against a guess — a build order, not a permission slip. The spike may still
conclude *do not build this*, and that conclusion is binding on its own evidence.
...
**Phase 1 starts once Phase 0 has produced its three numbers — that is a technical dependency
on the measurement, not a wait on anyone.**
...
PRD-080 (removed) rather than after it. **Unblocks:** the
Studio surface in
...
argument above holds. **The owner admitted it as a workspace package on 2026-08-12.** The
standing alternative, if the boundary ever stops holding, is a separate repository on its own
release lane like `threenative-asset-mcp` — never shrinking it into an existing package.
...
- [x] **No IR, no scene format, no operation registry, no mutation contract.** The agent edits
      files. Any phase that starts building `authoring` is stopped, not scoped down.
- [x] **No property inspector that writes a scene.** Reading and displaying scene state is
...
- [x] **The agent is a subprocess the user already has, not a model we own.** No API keys held
...
- [x] **The project on disk is a normal scaffolded ThreeNative project.** Closing Studio leaves
      a repository that `pnpm dev`, `pnpm test` and `git log` work on. **There is no Studio
      project format**, so there is no dead end.
- [x] **History is git.** Not `project-history`. A checkpoint is a commit; restore is a
...
|---|---|---|---|---|---|
| 1 | Spike result: does the loop hold together at all | `docs/spikes/studio-loop-2026-08-12.md`, read by Phase 1's design in §7 | nothing — the question has never been measured here | n/a | the spike's own reload assertion inverted → it must fail |
...
sentence.
**Pre-existing files edited:** `pnpm-lock.yaml`; the existing `packages/*` workspace wildcard
and root recursive scripts discovered Studio without edits. No package source outside
`packages/studio/`.
...
### Phase 0 — The spike, and the phase that runs first
...
**The three numbers Phase 0 must produce, and Phase 1's shape hangs on them:**
...
### Phase 1 — The package and the loop

**Starts when:** Phase 0's spike document exists with its three numbers and both negative
controls observed red. If the state-survival number came back **no**, Phase 1 does not start on
the hot-reload promise — it either fixes that first or the PRD stops, and either outcome is
written down.
...
**Phase 0 result — 2026-08-12:** proceed. `GameState.playerX` survived the bounded Codex edit
within `0.000003` world units, 30-tick movement changed from `1.000002` to `1.999974` units,
and the still-running project's shipped playtest exited zero. The visible change arrived in
13.674 seconds and proof completed in 16.994 seconds. Disposable measurement glue was 220
lines. Without `acceptHotUpdate`, `playerX` reset from `-0.233320` to `-1.999998`; without the
playtest bridge, the runner exited `2` with `TN_PLAYTEST_BRIDGE_MISSING` and no assertion
results. These numbers support the small-package hypothesis; Phase 1 is authorised.

### Phase 2 — The evidence panel
...
### Phase 3 — Ownership, which is git
...
- [x] Phase 0's three numbers, with the commands that produced them
- [x] Both Phase 0 negative controls observed red
- [x] For any later phase: `pnpm typecheck && pnpm lint && pnpm test` green, `pnpm budgets`
      with the LOC trigger reported and justified in this PRD
- [x] A dated file in `docs/verification/` per executed phase

## 9. Acceptance criteria

Consumer-scoped. Phase 0's are the ones reachable before anything is built.

**Phase 0:**

- [x] **A document states whether play state survives an agent's edit**, with the command, and
      with the `acceptHotUpdate`-deleted control observed red.
- [x] **It states the sentence-to-visible-change time in seconds** and the glue LOC the spike
      needed.
- [x] **It states a recommendation, including "do not build this"**, and that recommendation is
      recorded in this file beneath the numbers that produced it.
- [x] **No package was created, no dependency added, and `git status` shows changes only under
      `docs/spikes/`.**

**Phases 1–3:**

- [x] **A user types a sentence, sees the game change, and does not lose where they were
      standing** — proved by a scenario, not a screenshot.
- [x] **No control in the Studio is enabled and inert, and no panel states a fact it did not
      observe.** Proved by an adversarial pass over every button and panel, in the shape of the
      dogfood that closed this question.
- [x] **Closing Studio leaves a normal ThreeNative project** on which `pnpm dev`, `pnpm test`
      and `git log` work. There is no Studio-only file format anywhere in it.
- [x] **`rm -rf packages/studio` leaves the repository green.**
- [x] **The framework LOC trigger is reported with Studio counted**, and if crossed, justified
      here with a kill-switch pass over what was added.

**What this PRD may not claim:** that any phase ran before its evidence file exists; that Studio
works before a user has driven the loop end to end; or that the sibling project's Studio works,
since its own dogfood recorded three major findings and this PRD is partly built on them.

---

## Owner decision — recorded 2026-08-12
...
| Amend or hold the CONFLICTS row 1 gate — *"Studio starts only after a stranger has played a game for five minutes"*? | **Amended.** Studio proceeds in parallel with PRD-080 (removed) instead of after it. The five-minute test remains the beta bar; it is no longer a predecessor of this work. | 2026-08-12 |
