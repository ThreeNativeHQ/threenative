...
**re-verified again 2026-09-11** after an un-filing (below).
Evidence in [doctor-2026-08-29](../../verification/doctor-2026-08-29.md);
`packages/create-threenative/__tests__/doctor.spec.ts` is now 61/61 green, and a live probe against a
...
**Un-filed and restored.** On 2026-09-08, `7d730126f` moved this PRD out of `done/` into
...
**Re-verified 2026-09-11:** `src/doctor.ts:331-348` still separates `malformed` from `missing`;
`:397` still emits the exact `reachable by npx only` wording with package@version; `:501` still
summarises per server. 61/61 green.
...
Every box above was re-checked against `main` at `7e6dffc1e` before this PRD was restored to
`done/`. Nothing was ticked on the strength of the 2026-08-29 record alone.

| Phase | Claim | Where it lives today | Result |
|---|---|---|---|
| 1 | per-server resolution, npx-only, malformed vs missing | `src/doctor.ts:331-348`, `:397`, `:501` | present |
| 2 | two download directories, compiled-asset settings vs targets | `src/doctor.ts:153`, `:591-594`, `:608-618` | present |
| 3 | playtest bin detected and folded, no runtime dependency | `src/doctor.ts:111`, `:666-680`; `__tests__/doctor.spec.ts:538` | present |
| 4 | JDK and Android SDK resolved, unsupported JDK named with its range | `src/doctor.ts:112`, `:638-660`, `:1083-1117` | present |
| acceptance | no runtime dependency on `@threenative/playtest` | `package.json` dependencies — absent, asserted at `doctor.spec.ts:538` | present |

```text
pnpm exec vitest run packages/create-threenative/__tests__/doctor.spec.ts
 ✓ packages/create-threenative/__tests__/doctor.spec.ts (61 tests) 265ms
 Test Files  1 passed (1)
      Tests  61 passed (61)
```

The 2026-09-08 readiness assessment recorded doctor exiting 1 on a published consumer install,
naming the missing Linux prebuilt and JDK 26 against supported JDK 17. That is this PRD's phase 4
behaving correctly against a real blocker, not a regression of it.
