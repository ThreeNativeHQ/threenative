...
**Status:** COMPLETE, 2026-08-15. Implemented in `36f79fe`, with removal-sensitive tests in
`6099a3b` and the post-integration census reconciled in `ae8032a`.
...
- [x] Add a repository-relative verification-record lookup that is optional for synthetic fixture
  roots and required when the record exists in the real workspace.
- [x] Parse every counted-area row and the record total; compare their sum with measured
  `nativeRuntimeLoc` and return an actionable budget error on any mismatch.
- [x] Route the new error through `enforceBudgets`, which is the `pnpm budgets` entry point, while
  preserving the 50,000 warning as a non-fatal review trigger.
- [x] Add tests that remove a row and alter the total, observe the exact command fail, restore the
  record, and observe the command pass.

**Wiring (the phase is not done without this):**

- [x] Caller edited: `scripts/check-budgets.ts:329` enforces census reconciliation before success.
- [x] Registration: the existing root `package.json` `budgets` script continues to invoke
  `tsx scripts/check-budgets.ts`; no new command is added.
- [x] Old path: the separate manual-only kill-switch proof is superseded by the budgets command;
...
- [x] Running `pnpm budgets` against the current repository record succeeds and reports the
  measured native runtime LOC.
- [x] Omitting any counted native area makes the exact `pnpm budgets` command exit non-zero before
  reporting `budgets ok`.
- [x] Changing the recorded total while leaving source files untouched makes the exact
  `pnpm budgets` command exit non-zero.
- [x] Synthetic budget fixtures without the production verification record remain usable in the
  existing unit suite.
- [x] The 50,000 native LOC trigger remains unchanged and visible; its warning stays non-fatal.
- [x] Caller census identifies `scripts/check-budgets.ts` as the live consumer, and the old
  source-only command path is gone.
- [x] Focused tests, `pnpm budgets`, and `pnpm typecheck && pnpm lint && pnpm test` pass after all
...
Completed evidence: `scripts/check-budgets.ts` now consumes the production census through
`enforceBudgets`; the focused budget contract suite passed 24/24, and the real `pnpm budgets`
command passed with 15,077 framework LOC and 71,012 native LOC while both review triggers stayed
visible and non-fatal. Synthetic roots without the production record remain covered by the same
suite. The restored repository gates are recorded on the integration branch.
