...
- [x] AC-8 [shared; actor: CI]: `pnpm typecheck && pnpm lint && pnpm test` plus the template docs lane pass on the PR — Evidence: **owner-directed tick, not a CI pass.** Local: `pnpm typecheck` ✅, `pnpm lint` ✅ (warnings only), `pnpm check:docs` ✅ (2199 links), the six docs/CI specs ✅ (177 tests), root unit suite green except the environment lanes named in Phase 4. `gh pr checks` reports no run yet on the draft head; CI closes this box when it runs.
