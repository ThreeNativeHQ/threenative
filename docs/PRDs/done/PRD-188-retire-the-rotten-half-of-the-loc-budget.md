...
**Status:** COMPLETE, 2026-08-22. Both phases landed; evidence and pasted mutations in
[the decommission record](../../verification/budgets-decommission-2026-08-22.md). One deviation
recorded there: the predicted no-unused revert-check mechanism does not exist in this repo's
config, so deletion is guarded by typecheck-on-callers plus the zero-survivor grep instead.
Deleting the snapshot also broke two historical inbound links; both repaired link-only.
...
- [x] Remove the symbols and the report field; let typecheck name every survivor
- [x] Update the framework trigger message to end after the kill-switch instruction
- [x] Delete the spec sections exercising removed symbols; run the suite
...
- [x] Update `LIMITS` and the fixture numbers together
- [x] Update the charter row; keep the charter's rule text untouched otherwise
...
- [x] `pnpm budgets` exits 0 printing no "review trigger" lines on the tree this PRD gated. On
      the landing tree (rebased onto the asset-pipeline series) the **native** trigger is silent
      at 79,887/100,000; the **framework** 15k trigger now fires because that series added
      `packages/assets` — a PRD-188 non-goal, owned by the crossing PRDs
- [x] Attribution grep (§ Phase 1, step 2) returns empty
- [x] Charter table and `LIMITS` agree on 100,000
- [x] Both phases' mutations were observed red and pasted into
      `docs/verification/budgets-decommission-2026-08-22.md`
- [x] `pnpm typecheck && pnpm lint && pnpm test` green at the closing commit
