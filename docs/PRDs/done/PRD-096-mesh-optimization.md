...
docs/verification/asset-pipeline-2026-08-22.md. Native decode of the output is PRD-097 (re-scoped).
**Parent:** [the series README](./asset-pipeline/README.md).
**Depends on:** [PRD-094](../done/PRD-094-asset-compile-step.md).
**Blocks:** [PRD-097](./PRD-097-native-decode-path.md),
[PRD-098](../done/PRD-098-lod-and-instancing.md).
...
#### Phase 1: The hardest real model compiles and still measures the same — not a cube

**Proof subject:** updated 2026-08-22 after a repo survey. No skinned or animated `.glb`
exists anywhere in the repository today: `native-proof.glb` is one flat mesh, one material, no
textures, no bones, no clips (`create-threenative inspect`), and `abyss-framework`'s
`terrain/chunk-*.glb` paths reference files that are not on disk. The originally-proposed
platformer character does not exist. So the phase ships its own hardest case: a checked-in
fixture **generated by a committed script** using `@gltf-transform` — skinned, animated,
multi-primitive, multi-material, textured — plus `starter`'s `native-proof.glb` as the real
shipped end-to-end subject through the actual template pipeline.
...
- [x] Run passes in the fixed order above; each is skippable by config, none is reorderable
- [x] Self-verify: reopen the output, compare triangles, vertices, joint count, and bounding box
      against the source; throw naming the drift when it exceeds tolerance
- [x] Never quantize joint indices or weights below the source declaration
- [x] Record counts and bytes in the manifest

**Wiring:**

- [x] Caller edited: `compile.ts` and `assets.ts`
- [x] Old path: identity pass removed for models
...
the pre-existing starter asset playtest from PRD-094 fails.
...
- `templates/starter/playtests/models.playtest.json` - NEW
...
- [x] The playtest asserts the character is **visible** and that its animation **advances** —
      a quantized skeleton that collapsed to a point is visible and wrong
- [x] A `visual` assertion against the pre-compression baseline within threshold
...
- [x] The fixture character renders and animates identically after compression, judged by
      a visual and a movement assertion against the pre-compression baseline
- [x] The build prints a smaller total model size and the number lands in the round ledger
- [x] A model that loses geometry in compression **fails the build**, naming the drift — it never
      ships a warped mesh
- [x] A user's existing Draco `.glb` dropped into `assets/` still loads and renders
- [x] Setting `assets.models` to `"none"` produces a game identical to today's

**Integration gates:**

- [x] Ledger has zero `TBD` cells
- [x] Caller census pasted
- [x] Revert check passed: removing the decoder wiring breaks a pre-existing playtest
- [x] No second model-compression output format is live
- [x] Every gate observed red once
- [x] Proved on a skinned, animated, multi-material model — the exercised/unexercised list is
