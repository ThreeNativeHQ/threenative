...
| ---: | --- | --- | --- | --- |
| 1 | [PRD-328 — Launch is measured on the engine that ships](PRD-328-launch-is-measured-on-the-engine-that-ships.md) | 🟢 1–2 days | Cheapest row; makes every later launch number honest; needs no phone for its desktop half | the V8 launch table is in the record and the code-cache decision is written |
| 2 | [PRD-327 — First-use pipeline compilation leaves the main loop](PRD-327-first-use-pipeline-compilation-leaves-the-main-loop.md) | 🟡 about a week | Biggest owned defect (launch 14 s → ≤ 8 s); closes PRD-218 criteria 1–2 | three cold launches ≤ 8 s median, `pipelineCompile ≤ 500 ms` on the first present |
...
| — | [PRD-218 — the loading screen hides a 12–14 s synchronous stall](PRD-218-fps-framework-native-load-fps-heat.md) | record | The finding PRD-327 executes; keep it open until 327 ticks its criteria 1 and 2 | moves to `done/` with PRD-327 |
