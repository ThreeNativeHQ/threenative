...
| ---: | --- | --- | --- | --- |
| 1 | [PRD-328 — Launch is measured on the engine that ships](PRD-328-launch-is-measured-on-the-engine-that-ships.md) | 🟢 done (desktop) 2026-09-03 | Cheapest row; makes every later launch number honest; needs no phone for its desktop half | **Desktop done.** V8 launch table in `runtime-perf-state.md` §5b: parse+compile 51 ms (9.7 %) of a 524 ms launch. Code-cache decision **deferred, not taken** — its pre-registered rule names the phone, and the phone lane has not run at ≥ 50 % battery. Phone half `UNVERIFIED` |
