...
- [ ] AC-6 [local; actor: implementation agent]: Existing lifecycle consumers preserve nested dispatch, add/remove/clear order, exception cleanup, pointer edges and hover behavior. Evidence: `loop.spec.ts` covers add-during-dispatch, removal-during-dispatch, clear, nested dispatch and recovery after a callback throws (the `finally` unwinds the depth); the full engine dev-instance playtest (`pnpm test:playtest`) passed all six scenarios after the change; the game before-render seam and pointer edge/hover behavior rest on the existing suites.
