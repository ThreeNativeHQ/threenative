...
- [ ] Linux section GC measured and kept or rejected (AC-8). — Kept (Linux-only block in `CMakeLists.txt`): stripped `mystral` 74,327,728 → **58,072,784 B** (−21.9%), raw 126,515,528 → 101,546,296; `ctest -L native-contract` 44/44 in both builds; `--version` exits 0. A game run on the GC'd executable is pending (the contract tests link their own executables, not `mystral`).
