...
**Execution in progress, 2026-09-06.** 32 of 40 rows are accepted with tracked evidence; eight remain open. Native transport, the Go reference server and protocol, the portable `@threenative/core/net` client and its caller, credential delivery, the authoritative simulation, the playtest resource waits, monotonic native time, the transport CPU meter, the qualification-matrix aggregator, and the whole authoring/discovery group (8a, 8b, 8c) have passed.

The eight open rows are: **1b-quiche-platforms**, **1b-quiche-pin**, **1b**, **3a**, **6b**, **7a**, **7metrics** and **7c**. Every one of them is blocked on the same two things — an owned nine-target quiche distribution that does not exist yet, and qualification hardware (physical Android, macOS, Windows, and the Safari/Firefox/Edge browser matrix) that this machine does not have. Row 7metrics is implemented in `scripts/run-networking-proof.mjs` but has no tracked evidence document, and row 7a has passed its browser half only. No row is blocked on missing design or unwritten code.
...
| [x] | **8c. Real-process MCP discovery.** E `packages/create-threenative/__tests__/scaffold-mcp.spec.ts`, `scripts/verify-golden-path.ts`, `scripts/__tests__/verify-golden-path.spec.ts` | Extend the existing spawned MCP harness with concrete multiplayer search and same-process capability detail; validate the installed networking import, constraints and overrides. Reuse the existing harness rather than adding a second MCP client. Its workspace symlinks are test setup, not cold-package evidence. The packed golden path must exercise the same search/detail and import the discovered export from installed dist. | Named networking discovery test must fail when networking metadata is removed. Execute search/detail from the packed core MCP launcher in the S3 sandbox, then build/import the discovered export from installed dist. Record tarball hashes and reject workspace/source/symlink resolution in that cold proof. Evidence: [task 8c](../../verification/prd-359-task8c-2026-09-06.md). |
