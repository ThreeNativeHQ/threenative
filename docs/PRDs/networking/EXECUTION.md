...
**Execution in progress, 2026-09-07.** 30 of 40 rows are accepted with tracked evidence; ten remain open. A fresh review on 2026-09-07 found four checkboxes flipped ahead of their evidence: **8c** and **4b** are unchecked again here, and **1b-win32** and **1b-quiche-ci** are kept with their evidence corrected to what actually ran. Native transport, the Go reference server and protocol, the portable `@threenative/core/net` client and its caller, credential delivery, the authoritative simulation, the playtest resource waits, monotonic native time, the transport CPU meter, the qualification-matrix aggregator, and the metadata/recall half of the authoring group (8a, 8b) have passed.

The ten open rows are: **1b-quiche-platforms**, **1b-quiche-pin**, **1b**, **3a**, **4b**, **6b**, **7a**, **7metrics**, **7c** and **8c**. The nine-target owned quiche distribution now exists and is published as `quiche-owned-v1`, so **1b-quiche-platforms** is closed by run 34087936051 and the rows behind it — **1b-quiche-pin**, **1b**, **3a** — are mechanical rather than blocked. What remains genuinely unavailable here is qualification hardware: physical Android, macOS, Windows and the Safari/Firefox/Edge browser matrix. Row 7metrics is implemented in `scripts/run-networking-proof.mjs` but has no tracked evidence document. Row 7a has now passed both halves' assertions — see [browser](../../verification/prd-359-task7a-browser-2026-09-06.md) and [native](../../verification/prd-359-task7a-native-2026-09-06.md) — and stays open only because the proof runner's whole-frame budget cannot be produced on the desktop target. Row 4b's six named tests exist and pass but it has no evidence document of its own. No row is blocked on missing design or unwritten code.
...
| [x] | **1b-win32. Windows header compatibility.** E `N/src/webtransport/webtransport.cpp`, `N/tests/webtransport_wire_test.cpp`, `N/docs/G1-desktop-host.md` | The PR Windows desktop core build fails because quiche includes Winsock before NOMINMAX is defined, expanding new std::min calls as a Windows macro. Establish the Windows header guard before the first Windows header without changing transport behavior. | Preserve the real MSVC compile failure; prove the corrected include boundary and rebuild existing native wire/surface/live tests. Rerun the Windows CI build; no Windows runtime qualification is claimed by compilation alone. Evidence: [task 1b-win32](../../verification/prd-359-task1b-win32-2026-09-06.md). |
...
| [x] | **1b-quiche-ci. Dependency artifact CI.** N `.github/workflows/build-quiche-owned.yml`, `N/scripts/validate-quiche-release.py`, `N/scripts/test-validate-quiche-release.py`, `N/scripts/test-quiche-workflow.py` | Invoke the owned producer on branch CI, validate the exact owned release tag, and publish only intended archive/manifest files after successful checks. Provision explicit target/toolchain inputs. No runtime-release workflow reuse. | Parse the workflow and reject missing tag trigger, missing validator checkout and unguarded publication. Execute Linux producer CI; do not publish an incomplete multi-platform dependency release. Evidence: [task 1b-quiche-ci](../../verification/prd-359-quiche-ci-2026-09-06.md). |
