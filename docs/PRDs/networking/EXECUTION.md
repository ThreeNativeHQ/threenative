...
**Execution in progress, 2026-09-07.** 27 of 40 rows are accepted with tracked evidence; thirteen remain open. Two reviews on 2026-09-07 found checkboxes flipped ahead of their evidence, and **8c**, **4b**, **7b**, **1b-win32** and **1b-quiche-ci** are all unchecked again as a result. A row is checked here only when its own evidence document says the row passed — not when the work looks done. Native transport, the Go reference server and protocol, the portable `@threenative/core/net` client and its caller, credential delivery, the authoritative simulation, the playtest resource waits, monotonic native time, the transport CPU meter, and the metadata/recall half of the authoring group (8a, 8b) have passed.

The thirteen open rows are: **1b-win32**, **1b-quiche-ci**, **1b-quiche-platforms**, **1b-quiche-pin**, **1b**, **3a**, **4b**, **6b**, **7a**, **7b**, **7metrics**, **7c** and **8c**. The nine-target owned quiche distribution now exists and is published as `quiche-owned-v1` by run 34089963144, so the rows behind it — **1b-quiche-pin**, **1b**, **3a** — are mechanical rather than blocked. **1b-quiche-platforms** stays open on its own acceptance, not on paperwork: nine archives that build and validate are not the row's bar, which also wants each target's runtime qualification recorded, and cross-compilation is not runtime proof. What remains genuinely unavailable here is qualification hardware: physical Android, macOS, Windows and the Safari/Firefox/Edge browser matrix. Row 7metrics is implemented in `scripts/run-networking-proof.mjs` but has no tracked evidence document. Row 7a's five networking assertions have passed on both halves — see [browser](../../verification/prd-359-task7a-browser-2026-09-06.md) and [native](../../verification/prd-359-task7a-native-2026-09-06.md) — but the native result is **not reproducible on the host this branch ships**, and two things keep the row open. The desktop target cannot express a diagnostics opt-out, so the example's deliberate empty-buffer decode fails the run; and the proof runner's whole-frame budget cannot be produced there at all. Row 4b's six named tests exist and pass but it has no evidence document of its own. No row is blocked on missing design or unwritten code.

Execute this file in table order.
...
| [x] | **4a. Go application adapter.** E `S/main.go`; N `S/protocol.go`, `S/protocol_test.go`, `P/protocol-v1-vectors.json` | Implement PROTOCOL.md envelope, HELLO/WELCOME and BIND/BOUND. Factor application adapter into protocol.go, using webtransport-go sessions; no custom reliability. Main exposes `/game` in addition to `/echo` and calls adapter. Add literal shared vectors. | Go tests `TestVectors`, `TestSplitFrames`, `TestMalformedFrame`, `TestDuplicateBind`, `TestChannelMismatch`, `TestNegotiatedLimits`. Corrupt a literal vector and observe test failure. A browser raw client completes HELLO/BIND against live `/game`. Evidence: [task 4a](../../verification/prd-359-task4a-2026-09-06.md). |
| [ ] | **4b. Shared client and real caller.** N `C/src/net.ts`, `C/__tests__/net.spec.ts`; E `C/package.json`, `C/tsup.config.ts`, `G/src/game.ts` | Implement exact API and framing in net.ts; add `./net` export and tsup entry. Add an internal opt-in networking proof method to NativeSmoke, initially disabled, that connects, polls in the existing returned frame-update callback and closes in scene exit. Later row 5a wires its configuration. | Tests `matches shared protocol vectors`, `honors typed array offsets`, `rejects invalid options`, `bounds queues`, `settles cancellation`, `polls disconnect once`. Test collection must show these names. Temporarily enable proof for browser/native echo; removal of export breaks game build. Do not call this row integrated until that opt-in flow executes. The six named tests exist and pass in `packages/core/__tests__/net.spec.ts`, but this row has no evidence document of its own; the link previously here pointed at task 4c's. |
| [x] | **4c-utf8. Strict portable UTF-8 decoding.** E `N/src/runtime-scripts/fetch-polyfill.js`, `N/src/runtime-scripts/streams-polyfill.js`, `N/tests/fetch-shim.test.mjs`, `N/tests/streams-shim.test.mjs`, `N/tests/webtransport_surface_test.cpp` | Fallback TextDecoder ignores fatal options, so malformed handshake UTF-8 cannot be rejected portably. Implement the standard UTF-8 fatal/error and view-offset behavior needed by the protocol; propagate decoder options through TextDecoderStream. Reject unsupported encodings/options honestly. | Malformed UTF-8 throws with fatal:true, valid split multibyte sequences and typed-array offsets decode correctly, nonfatal decoding uses replacement characters. Record red/green in Node shim tests and compiled native surface execution. |
...
| [x] | **5b. Authoritative server simulation.** E `S/main.go`, `S/protocol.go`, `G/src/networking-game.ts`, `G/src/game.ts`; N `S/game_test.go` | Implement the exact reference channel map and JSON schemas from PROTOCOL.md; 60 Hz simulation, 20 Hz snapshots, validated axes, server-owned positions. Game renders local/remote players from per-player snapshot datagrams, discards stale ticks per player and sends numbered actions. | `TestServerOwnsPosition`, `TestStaleInput`, `TestActionDeduplication`, `TestInvalidGameplayPayload`. Two actual clients join, move and receive server action acknowledgements. Dropping input at sender must prevent peer motion. Evidence: [task 5b](../../verification/prd-359-task5b-2026-09-06.md). |
...
| [ ] | **7a. Existing scenario reaches both clients.** E `G/src/networking-game.ts`, `G/src/game.ts`; N `G/playtests/networking.playtest.json`, `scripts/run-networking-proof.mjs`, `scripts/__tests__/run-networking-proof.spec.ts` | Implement the runner contract below. Start Go server, reuse Task 4e credential proxy and start partner client; invoke the existing playtest CLI for the subject client. Publish observations through ctx.state. Do not add a new scenario language, fake renderer or pixel-conformance registry row. | Unit tests reject missing server/partner, mismatched identities and zero assertions. Live scenario checks peer motion and action. Kill partner and observe failure. Resource assertions work on browser and native through existing bridge. Evidence: [browser half](../../verification/prd-359-task7a-browser-2026-09-06.md), [native half](../../verification/prd-359-task7a-native-2026-09-06.md), and the harness fix both needed, [desktop host args](../../verification/prd-359-desktop-host-args-2026-09-06.md). |
| [x] | **7wait-a. Bounded resource wait on browser.** E `packages/playtest/src/scenario/schema-base.ts`, `packages/playtest/src/scenario/schema-validate.ts`, `packages/playtest/src/runner/steps.ts`, `packages/playtest/__tests__/scenario.spec.ts`; N `packages/playtest/src/runner/wait-for-resource.ts` | Implement the portable waitForResource step specified below. Browser steps call the shared bounded helper using existing resource observation and tick-control APIs. | Reject missing timeout, malformed path/predicate and mixed action/wait steps. An asynchronously changing resource passes; a permanently false predicate times out with its last observation. Evidence: [task 7wait](../../verification/prd-359-task7wait-2026-09-06.md). |
| [x] | **7wait-b. Same wait on native.** E `packages/playtest/src/runner/androidRunner.ts`, `packages/playtest/src/runner/wait-for-resource.ts`, `packages/playtest/__tests__/device-playtest.spec.ts`, `packages/playtest/__tests__/ios-device-playtest.spec.ts`, `G/playtests/networking.playtest.json` | Wire the shared helper into the common native scenario path used by Android/desktop/iOS. Use the existing resource bridge, not CDP. If dispatch differs, document its actual caller before modifying another file. | Run same wait-positive and wait-timeout cases on browser and desktop; device transport tests cover missing observations. iOS mocked dispatch may run but live iOS remains deferred. Evidence: [task 7wait](../../verification/prd-359-task7wait-2026-09-06.md). |
| [x] | **7clock. Monotonic native performance time.** E `N/src/runtime.cpp`, `N/tests/timer-contract.test.mjs`, `N/tests/timer_delivery_test.cpp`, `N/docs/G1-desktop-host.md` | Native performance.now currently uses high_resolution_clock epoch time, which is not guaranteed monotonic. Use a runtime-relative steady_clock origin for the existing global so networking deadlines and clock probes use monotonic milliseconds. | Native contract asserts runtime-relative finite, nondecreasing time; old epoch-based implementation must fail. Source clock contract verifies the monotonic clock choice; run compiled timer contract and existing clock/timer tests. |
| [x] | **7native. Native transport CPU meter.** E `N/src/runtime.cpp`, `N/tests/host-gap-meter.test.mjs`, `scripts/run-networking-proof.mjs`, `scripts/__tests__/run-networking-proof.spec.ts` | Time the existing webtransport::processEvents call using the host monotonic clock and emit a separately named per-frame WebTransport duration through the existing meter path. Parse it with frame identity in proof runner; do not substitute the whole I/O segment. | Delay that call by 5 ms through a test seam: parsed networking CPU rises and budget fails. Missing or duplicated frame samples cannot pass. |
...
| [x] | **8a. Publish discovery metadata.** E `C/src/net.ts`, `scripts/not-owned-capabilities.ts`, `packages/core/capabilities.json`, `packages/create-threenative/capabilities.json`, `packages/create-threenative/agent-docs/references/capability-reference.md` | Add capability documentation to shipped export; narrow not-owned answer to replication/prediction. Run capabilities:sync; generated outputs land with their source. Search must return the real import and delivery limitations. | `pnpm capabilities:check`; manifest tests. Revert export metadata and intended query must no longer resolve, proving the test depends on new surface. Evidence: [task 8a](../../verification/prd-359-task8a-2026-09-06.md). |
| [x] | **8b. Recall and core instructions.** E `scripts/fixtures/capability-recall/corpus.json`, `scripts/fixtures/capability-recall/budget.json`, `scripts/__tests__/capability-recall.spec.ts`, `C/AGENTS.md`, `C/CLAUDE.md` | Update multiplayer transport queries to owned capability while retaining out-of-scope queries. Document actual subpath, defaults/overrides, no fallback, Go server and iOS unverified status. Regenerate mirror. | `pnpm caps:recall`; named recall test; sync:agents check. Old blanket answer must fail new positive query. Evidence: [task 8b](../../verification/prd-359-task8b-2026-09-06.md). |
| [ ] | **8c. Real-process MCP discovery.** E `packages/create-threenative/__tests__/scaffold-mcp.spec.ts`, `scripts/verify-golden-path.ts`, `scripts/__tests__/verify-golden-path.spec.ts` | Extend the existing spawned MCP harness with concrete multiplayer search and same-process capability detail; validate the installed networking import, constraints and overrides. Reuse the existing harness rather than adding a second MCP client. Its workspace symlinks are test setup, not cold-package evidence. The packed golden path must exercise the same search/detail and import the discovered export from installed dist. | Named networking discovery test must fail when networking metadata is removed. Execute search/detail from the packed core MCP launcher in the S3 sandbox, then build/import the discovered export from installed dist. Record tarball hashes and reject workspace/source/symlink resolution in that cold proof. Partial evidence: [task 8c](../../verification/prd-359-task8c-2026-09-06.md), which records that the row still needs an independent review checkpoint and that its cold scaffold is not the required external networking sandbox. |

Template instructions are five final rows, each owns five existing files: both AGENTS.md and its
generated CLAUDE.md for the named two templates, plus the shared template contract regression
`packages/create-threenative/__tests__/template.spec.ts`. Execute pairs in this order:
...
- Task 4c-utf8: [strict portable UTF-8 decoding](../../verification/prd-359-task4c-utf8-2026-09-06.md); the controlled fatal-decoder regression failed, the focused Node suites passed 73/73 with two documented skips, and compiled V8/QuickJS surface contracts passed.
...
- Task 4d: [authenticated session fixture](../../verification/prd-359-task4d-2026-09-06.md); the controlled room-binding regression failed, the required auth negatives and full Go package passed, and task 4e supplied the live authenticated join/redaction proof.
...
- Task 7a browser slice: [paired browser proof](../../verification/prd-359-task7a-browser-2026-09-06.md); process ownership and peer-loss timing first failed, then the real paired Chromium run passed 19 assertions including authenticated peer motion/action and a controlled partner loss. Native resource-bridge evidence remains open.
...
Template pair `action-rpg/defense` local evidence: [optional transport instructions](../../verification/prd-359-template-action-defense-2026-09-06.md); generated mirrors are green while the shared all-template regression remains open for the other pairs.
Template pair `minimal/platformer` local evidence: [optional transport instructions](../../verification/prd-359-template-minimal-platformer-2026-09-06.md); generated mirrors are green while the shared all-template regression remains open for the other pairs.
Template pair `puzzle/racing` local evidence: [optional transport instructions](../../verification/prd-359-template-puzzle-racing-2026-09-06.md); generated mirrors are green while the shared all-template regression remains open for the other pairs.
Template pair `runner/sailing` local evidence: [optional transport instructions](../../verification/prd-359-template-runner-sailing-2026-09-06.md); generated mirrors are green while the shared all-template regression remains open for the other pairs.
Template pair `shooter/starter` local evidence: [optional transport instructions](../../verification/prd-359-template-shooter-starter-2026-09-06.md); all ten template instruction contracts now pass the shared regression, pending independent review and the required external networking sandbox.
