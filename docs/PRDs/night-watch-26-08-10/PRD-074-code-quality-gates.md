...
**Status: SCOPING, 2026-08-10. NOTHING IN THIS PRD IS IMPLEMENTED.** Unlike most PRDs here,
every number in §2 was **measured on this working tree today**, not read off a file — the
commands are pasted in §2 and reproduce. The tree is commit `9c269f5` plus the uncommitted
`packages/core/src/collapse.ts`. What is *not* measured: whether acting on any of these
signals improves anything. That is the honest limit of this document, and §9 keeps it inside
it.
...
prefix, is enforced**, because it is not a judgement call: the repository is split 89-to-0 on
it along a line nobody drew. This PRD renames the 84 interfaces that lack the prefix, takes
the tree to zero violations, and only then turns the rule on as an error, so it holds a line
instead of nagging about a backlog.
...
rename: 84 interface declarations across `packages/{core,physics,ui,create-threenative,
...
Every measurement below was produced by running the pasted command against this tree.
Full JSON outputs are reproducible; nothing here is a code read.
...
| Fact | Measured |
| --- | --- |
| Unique interface names to rename | **84** — 67 exported, 17 module-local, plus 8 in `playtest` |
...
```
biome-ignore comments in packages/**/*.ts .............. 2
@ts-ignore / @ts-expect-error in packages/*/src ........ 0
as any in packages/*/src (excl. runtime-native) ........ 0
as unknown as in packages/*/src (excl. runtime-native) . 14
...
4. **It is machine-readable.** `pnpm quality --json` emits one object per finding with
...
| 4 | Suppressions | script | baseline 2 `biome-ignore`, 0 `@ts-*`, 14 `as unknown as` | §2.6 is near zero and cheap to hold | A new suppression needs a reason in the same commit |
...
signals 2–5, `pnpm quality` and `pnpm quality --json` wired in root `package.json`,
...
with `maxAllowedComplexity: 15`, plus an `overrides` entry that lints `packages/playtest/**`
at **warn severity only**, so §2.7's 89 errors surface without turning `pnpm lint` red.
...
**Phase 2 — the rename: 84 interfaces to `I*`, one package per commit.** This is the phase
that spends the §2.4 budget, and it is ordered so that a break is caught by the narrowest
possible gate.
...
build, that signal 3 does, and that the first move on a hard change is `pnpm quality --json`
...
2. `pnpm quality --json` emits one machine-readable record per finding with `file`, `line`,
...
10. `docs/verification/interface-rename-2026-08-10.md` lists all 84 names with old name, new
    name, declaring file, and — for anything struck from the list — the reason it was struck.
...
pnpm quality --json | head -40     # machine-readable shape
