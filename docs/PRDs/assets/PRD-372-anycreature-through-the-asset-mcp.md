...
`playtests/creature.playtest.json`; NEW engine evidence file
`E/docs/verification/prd-372-anycreature-implementation.md`. **Ledger:** C7.
...
## Verification evidence and release gate
...
| Phase | Required observation | Current result | Implementation artifact |
| --- | --- | --- | --- |
| 1 | Packed stdio discovery and guide, missing-payload red | UNVERIFIED | Package inventory, hashes, protocol transcript |
| 2 | Original wyvern compiles/revises; failures preserve prior output; cancellation completes | UNVERIFIED | Spec, raw GLB, receipts, process and red/green logs |
| 3 | Four silhouettes and hero; backend and missing-view controls; independent review | UNVERIFIED | Images, metrics, backend identity, reviewer record |
| 4 | Actual clip/skin/spec inspection; unknown/empty claims fail | UNVERIFIED | Claims, exact mutated artifacts and check responses |
| 5 | Fresh npm/pnpm scaffold reaches packaged creature compiler through core | UNVERIFIED | Installed dependency identities and stdio transcript |
| 6 | Cold agent discovers recipe and completes authoring calls | UNVERIFIED | Generated paths and bounded authoring transcript |
| 7 | Same produced asset cooks and animates on browser/native desktop | UNVERIFIED | Scenario, captures, adapter/host identity, actual gate output |

Record implementation evidence in `docs/verification/prd-372-anycreature-implementation.md` when
execution begins. That future path is not evidence today. Keep bounded logs and cited captures;
avoid committing the full toolchain cache or arbitrary generated game sources. Planning-document
checks are recorded separately in [the planning verification record](../../verification/prd-372-anycreature-planning-2026-09-09.md).
