...
🟢 a day or two · 🟡 days to a week · 🔴 weeks

### P0 — the instrument, before anything else

| # | Task | | Why first |
| --- | --- | --- | --- |
| 0.1 | **Measure GPU time per pass, on the phone** | 🟡 | Everything below P1.1 is a guess. Three sessions worked out GPU cost by rebuilding the app once per experiment and pushing a settings file through `run-as` — days per answer |
| 0.2 | **Confirm `gpuMs` reports on Android** | 🟢 | The meter is portable and reports *nothing* rather than zero when it cannot measure — but it has never been tried on Android |

**Stop-gate:** if 0.1 lands and the numbers above do not reproduce, **stop and re-plan.** Our model
of what is slow has been wrong twice (F12, PRD-227's second change).

### P1 — the biggest measured win

| # | Task | | Worth | Needs |
| --- | --- | --- | --- | --- |
| 1.1 | **Bake prefiltered reflections into `@threenative/assets`** | 🟡 | **~6.3 ms of an 18–19 ms frame.** Measured, not guessed. Needs no new instrument | — |
| 1.2 | **Ship `src/render/quality.ts` in all 8 templates**, platform-aware, real costs in comments | 🟢 | Free, on by default, deletable. **Allowed under §5b today and missing from all 8 templates** | 0.1 for honest numbers |
| 1.3 | **CI check that deletes every baked file and proves the game is identical** | 🟢 | Makes the delete-test a gate before there is a second pass to get wrong | 1.1 |

Start with reflections: the cheap alternative was tried and **made shaded faces visibly darker at
two attempts**. Baking changes *when* the work happens, not what it produces.

### P2 — the structural one

| # | Task | | Worth | Needs |
| --- | --- | --- | --- | --- |
| 2.1 | **Render thread over the frame buffer we already build** | 🔴 | Up to 9.3 ms of a ~28 ms phone frame — **and owed anyway**, `Worker` currently runs on the main thread | frame op stream (landed) |
| 2.2 | **Job system; make `Worker` a real worker** | 🔴 | Correctness first, speed second | 2.1 |

**Do not ship 2.1 without 1.1.**

### P3 — proven, incremental

| # | Task | | Worth |
| --- | --- | --- | --- |
| 3.1 | **LOD baking + LOD switching** | 🔴 | Drawing the town: 9–11 ms, our biggest unexplained GPU cost |
| 3.2 | **Scene projection that works on moving objects** | 🟡 | Extends something already worth 780 → 315 draws |
| 3.3 | **GPU cost per pass in `diagnostics`** | 🟡 | Lets an agent see 0.1's numbers without owning a phone |

### P4 — slow, outside our control, start anyway

| # | Task | | Worth |
| --- | --- | --- | --- |
| 4.1 | **Push fixes upstream to three.js** — per-object submission cost, indirect/multi-draw, render bundles | 🔴 | The 11.3 vs 5.3 µs gap is three.js's. Fixing it there speeds up web *and* native, forever. We have the best measurement rig of any three.js user — that is the contribution |
| 4.2 | **Timeboxed `shermes` AOT spike** | 🟡 | The only idea that stops iOS's no-JIT rule being permanent. Filed as *"a spike, not a plan"*, owned by nobody |
| 4.3 | **Android conformance running on every commit** | 🟡 | It last executed **0 of 74 rows** — stopped before Gradle on a stale dependency pin |
...
| Bevy / compile-to-IR / QuickJS-as-control-plane | Describes a codebase we do not have. Three of its ideas are already in the graveyard; four it proposes already shipped. Its one good idea — *a table of who owns each runtime property and what writing to it does* — is worth writing |
