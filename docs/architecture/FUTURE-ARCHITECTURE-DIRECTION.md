...
| 3 | ✅ **CI check: delete every baked file, prove the game is identical** — done, PRD-306, [`delete-test-passes-2026-09-01`](../verification/delete-test-passes-2026-09-01.md). Green and wired into the `golden-path-template` job. It landed **red** and the red was the finding: the loader's no-manifest fallback resolved the *base* path, not the *source* path, so no compiled game survived losing its bake | 🟢 | Makes the delete-test a gate before there is a second baking pass to get it wrong. Cheap now, expensive to retrofit | best landed with #5 |
