...
**Status:** analysis, 2026-08-02. **Design authority:** `DESIGN.md` §3 (kill switch),
...
|---|---|---|
| No game lifecycle, input, save, audio or release architecture | **Yes** | Shipped: `defineGame`, `Scene`, fixed-step loop, `InputMap`, asset loader, state store. This is `DESIGN.md` §3's 42% plumbing surface, and it is the entire product |
...
| Heavy systems exceed the JS CPU ceiling | **Yes, narrowly** | `@threenative/physics-native`: a JSI binding to Rapier's Rust, with **coarse bulk APIs**, never per-entity bridge calls. `DESIGN.md` §7 |
...
1. **Is it plumbing every game rewrites?** (`DESIGN.md` §3.) If not, stop.
