...
| 🟢 | **Cold start: the bundle is parsed as source every launch, and nothing can measure it.** The cold-start compile/execute markers exist only in QuickJS; V8, JSC and the desktop CLI emit none, so `measure-cold-start.mjs` fails closed on every shipped build. (SWC transpiles only `.ts` sources in `module_system.cpp`; a packaged bundle is `.js` and never goes through it) | [PRD-328](../PRDs/performance/critical/PRD-328-launch-is-measured-on-the-engine-that-ships.md), `src/js/v8_engine.cpp` | Filed 2026-09-02: mark, measure, then decide the code cache by number. Under QuickJS compile was 230 ms (8 %) of launch |
...
The next action is **step 1 of
[`docs/PRDs/performance/critical/README.md`](../PRDs/performance/critical/README.md)** — the
launch instrument (PRD-328), then the launch stall (PRD-327), then the matched-pixel GPU
comparison (PRD-329). Nothing 🔴 above starts before PRD-329's verdict.
