...
// PRD-449 adapter patch 1 of 5, and the only line in this file that is not the pinned source:
// upstream resolves the Kenney pack over the network on every run, and a benchmark whose inputs
// arrive unversioned and unhashed cannot be reproduced or locked. This is the same URL upstream
// names, served from the vendored copy `scripts/engine-load-test/fetch-city-assets.mjs` wrote into
// the pinned checkout's own `assets/kenney/`, whose per-file SHA-256s travel in the fixture. The
// relative paths, the 56 file names and every load below are the pinned ones, unchanged.
const BASE_URL: &str = "kenney";
