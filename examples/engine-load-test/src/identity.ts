...
// WebCrypto is fast where available; the owned native JS host has no crypto global. Keep one
// canonical byte stream and check the fallback against Node's SHA-256 at block boundaries.
const SHA256_K = [
...
function sha256WithoutWebCrypto(bytes: Uint8Array): string {
  ...
  const compress = (block: Uint8Array, offset: number): void => {
    for (let index = 0; index < 16; index += 1) {
      ...
      words[index] =
        ((block[start] as number) << 24) |
        ((block[start + 1] as number) << 16) |
        ((block[start + 2] as number) << 8) |
        (block[start + 3] as number);
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      const sum1 =
        (h as number) +
        (rotate(e as number, 6) ^ rotate(e as number, 11) ^ rotate(e as number, 25)) +
        (((e as number) & (f as number)) ^ (~(e as number) & (g as number))) +
        (SHA256_K[index] as number) +
        (words[index] as number);
      const sum2 =
        (rotate(a as number, 2) ^ rotate(a as number, 13) ^ rotate(a as number, 22)) +
    ...
    for (const [index, value] of [a, b, c, d, e, f, g, h].entries()) {
      state[index] = ((state[index] as number) + (value as number)) >>> 0;
    }
  };

  const fullLength = bytes.length - (bytes.length % 64);
  for (let offset = 0; offset < fullLength; offset += 64) compress(bytes, offset);
  ...
  lengthView.setUint32(tail.length - 8, Math.floor(bytes.length / 0x20000000));
  lengthView.setUint32(tail.length - 4, (bytes.length * 8) >>> 0);
  compress(tail, 0);
...
export async function sha256(bytes: Uint8Array): Promise<string> {
  if (globalThis.crypto?.subtle === undefined) return sha256WithoutWebCrypto(bytes);
