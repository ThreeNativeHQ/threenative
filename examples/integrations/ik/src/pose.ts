...
export interface IPoseTarget { readonly position: Point3; readonly quaternion?: Rotation4; }
...
export function uniformScaleOf(m: ArrayLike<number>): number {
  ...
  const x=Math.hypot(m[0],m[1],m[2]); const y=Math.hypot(m[4],m[5],m[6]); const z=Math.hypot(m[8],m[9],m[10]);
...
export function angularError(a: ArrayLike<number>, b: ArrayLike<number>): number {
  finite(a,4,'quaternion');finite(b,4,'quaternion');
  const an=Math.hypot(a[0],a[1],a[2],a[3]); const bn=Math.hypot(b[0],b[1],b[2],b[3]);
...
export function contactError(a: ArrayLike<number>, b: ArrayLike<number>): number {
  finite(a,3,'position');finite(b,3,'position');return Math.hypot(a[0]-b[0],a[1]-b[1],a[2]-b[2]);
}
export function validateTargets(targets: readonly IPoseTarget[], count: number): readonly IPoseTarget[] {
  if (count<1 || targets.length!==count) throw new Error(`IK requires exactly ${count} nonempty goals.`);
  return targets.map(target=>{
    ...
    const quaternion: Rotation4=[target.quaternion[0]/length,target.quaternion[1]/length,target.quaternion[2]/length,target.quaternion[3]/length];
