...
export interface IIKResidual {
  readonly bone: string;
  readonly metres: number;
  readonly radians: number | null;
}
export interface IIKReport {
  readonly converged: boolean;
  readonly solverStatus: readonly number[];
  readonly residuals: readonly IIKResidual[];
}
interface IRow {
  bone: Bone;
  joint: Joint;
...
export class ConstrainedIK {
  ...
  constructor(options: IIKOptions) {
    ...
    for (const tolerance of [options.positionTolerance, options.rotationTolerance])
      if (!Number.isFinite(tolerance) || tolerance <= 0)
        throw new Error("IK tolerances must be finite and positive.");
    if (!options.effectors.length || !options.joints.length)
      throw new Error("IK requires joints and effectors.");
    this.#positionTolerance = options.positionTolerance;
    ...
    for (const specification of options.joints) {
      if (specifications.has(specification.bone))
        throw new Error("IK joint appears more than once.");
      const axes = specification.axes;
      if (
        axes.length !== specification.min.length ||
        axes.length !== specification.max.length ||
        axes.length === 0
      )
        throw new Error("IK each axis requires a minimum and maximum offset.");
      if (
        new Set(axes).size !== axes.length ||
        axes.some((a, i) => !["x", "y", "z"].includes(a) || (i > 0 && a <= axes[i - 1]))
      )
        throw new Error("IK axes must be unique and in x/y/z order.");
      axes.forEach((_, i) => {
        if (
          !Number.isFinite(specification.min[i]) ||
          !Number.isFinite(specification.max[i]) ||
          specification.min[i] > specification.max[i]
        )
    ...
    const build = (bone: Bone, parent: Link): void => {
      ...
      if (specification) {
        ...
        joint.setMinLimits(...specification.min);
        joint.setMaxLimits(...specification.max);
      }
      ...
      this.#rows.push({
        ...
        zeros: new Array(specification?.axes.length ?? 0).fill(0),
      });
      for (const child of bone.children) if (child instanceof Bone) build(child, link);
    };
    build(this.root, this.#anchor);
    for (const bone of specifications.keys())
      if (!links.has(bone))
        throw new Error(`IK joint '${bone.name}' is outside the direct bone hierarchy.`);
    for (const effector of options.effectors) {
      const link = links.get(effector.bone);
      if (!link)
        throw new Error(`IK effector '${effector.bone.name}' is outside the bone hierarchy.`);
      if (this.#effectors.some((item) => item.bone === effector.bone))
        throw new Error("IK effector appears more than once.");
      const goal = new Goal();
      const orientation = effector.orientation ?? false;
      if (!orientation) goal.setFreeDoF(DOF.EX, DOF.EY, DOF.EZ);
      goal.makeClosure(link);
      this.#effectors.push({ bone: effector.bone, goal, orientation });
    }
  ...
  update(targets: readonly IPoseTarget[], blend = 1): IIKReport {
    ...
    if (!Number.isFinite(blend) || blend < 0 || blend > 1)
      throw new Error("IK blend must be in [0,1].");
    ...
    for (const row of this.#rows) {
      ...
      const p = row.bone.position;
      const q = row.bone.quaternion;
      angularError(q.toArray(), [0, 0, 0, 1]);
      row.before.copy(q);
    ...
    try {
      ...
      for (const row of this.#rows) {
        row.link.updateMatrixWorld();
        row.link.getWorldQuaternion(this.#worldQuaternion);
      ...
      const residuals = this.#effectors.map((effector, i) => {
        effector.bone.getWorldPosition(this.#position);
        effector.bone.getWorldQuaternion(this.#quaternion);
        const target = goals[i];
        return {
          bone: effector.bone.name,
          metres: contactError(this.#position.toArray(), target.position),
          radians: target.quaternion
            ? angularError(this.#quaternion.toArray(), target.quaternion)
            : null,
        };
      });
      return {
        solverStatus: status.map(Number),
        residuals,
        converged: residuals.every(
          (r) =>
            r.metres <= this.#positionTolerance &&
            (r.radians === null || r.radians <= this.#rotationTolerance),
        ),
      };
    } catch (error) {
