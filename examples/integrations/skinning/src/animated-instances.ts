...
import { type MeshStandardNodeMaterial, StorageBufferAttribute } from "three/webgpu";
import { FramePalette, type IFrameSnapshot, type IInstanceHandle, skinPoint } from "./palette.js";
...
function rigidScale(matrix: Matrix4): void {
  ...
  const tolerance = Math.max(length * length, 1e-8) * 1e-5;
  if (
    e.some((value) => !Number.isFinite(value)) ||
    length <= 1e-8 ||
    matrix.determinant() <= 0 ||
    Math.abs(y.length() - length) > length * 1e-5 ||
    Math.abs(z.length() - length) > length * 1e-5 ||
...
export class AnimatedInstances {
  ...
  constructor(options: IAnimatedInstanceOptions) {
    ...
    if (!source.isSkinnedMesh || !source.skeleton.bones.length)
      throw new Error("Animated instances require a bound skinned mesh.");
    if (Object.keys(geometry.morphAttributes).length || geometry.hasAttribute("tangent"))
      throw new Error("Morph targets and tangents are not supported in this admission fixture.");
    if (
      options.material.positionNode ||
      options.material.normalNode ||
      options.material.displacementMap
    )
    ...
    for (const [name, itemSize] of [
      ...
      if (
        !a ||
        a.itemSize !== itemSize ||
        "isInterleavedBufferAttribute" in a ||
        a.count !== geometry.getAttribute("position")?.count
      )
        throw new Error(
          `Animated instances need a non-interleaved '${name}' attribute with matching count.`,
        );
    }
    const bindingBytes = options.capacity * source.skeleton.bones.length * 64;
    if (
      !Number.isSafeInteger(options.maxStorageBufferBindingSize) ||
      options.maxStorageBufferBindingSize <= 0 ||
      bindingBytes > options.maxStorageBufferBindingSize
    )
      throw new Error("Animated instance palette exceeds the device storage binding limit.");
    this.#source = source;
    this.#pose = new Float32Array(source.skeleton.bones.length * 16);
    ...
    this.palette = new FramePalette(
      options.capacity,
      source.skeleton.bones.length,
      options.byteBudget,
    );
    const owned = new InstancedBufferGeometry();
    for (const [name, a] of Object.entries(geometry.attributes)) {
      if ("isInterleavedBufferAttribute" in a)
        throw new Error("Animated instances do not accept interleaved attributes.");
      owned.setAttribute(name, a.clone());
    }
    owned.setIndex(geometry.index?.clone() ?? null);
    owned.setDrawRange(geometry.drawRange.start, geometry.drawRange.count);
    for (const group of geometry.groups)
      owned.addGroup(group.start, group.count, group.materialIndex);
    owned.instanceCount = 0;
    const arrays = [
      this.palette.currentBones,
      this.palette.previousBones,
      this.palette.currentTransforms,
      this.palette.previousTransforms,
    ];
    this.#buffers = arrays.map((array, i) => {
      const buffer = new StorageBufferAttribute(array, 16);
      owned.setAttribute(`tnPalette${i}`, buffer);
      return buffer;
    });
    const current = storage(
      this.#buffers[0],
      "mat4",
      options.capacity * this.palette.bones,
    ).toReadOnly();
    ...
    const skin = (palette: typeof current) =>
      palette
    ...
    material.positionNode = Fn(() => {
      ...
      positionPrevious.assign(
        oldTransforms.element(instanceIndex).mul(skin(previous)).mul(vec4(positionGeometry, 1)).xyz,
      );
      return matrix.mul(vec4(positionGeometry, 1)).xyz;
    })();
    this.mesh = new Mesh(owned, material);
    this.mesh.frustumCulled = false;
    const pickGeometry = geometry.clone();
    pickGeometry.deleteAttribute("normal");
    this.#pick = new Mesh(pickGeometry, options.material);
    this.#pick.matrixAutoUpdate = false;
    this.mesh.raycast = (raycaster, hits) => this.#raycast(raycaster, hits);
  }
  #live(): void {
    if (this.#disposed) throw new Error("Animated instances are disposed.");
  }
  #readPose(source: SkinnedMesh): void {
    if (
      source.geometry !== this.#source.geometry ||
      source.skeleton.bones.length !== this.#source.skeleton.bones.length ||
    ...
    for (let i = 0; i < source.skeleton.bones.length; i++) {
      this.#matrix
        .fromArray(boneMatrices, i * 16)
        .premultiply(source.bindMatrixInverse)
        .multiply(source.bindMatrix);
      rigidScale(this.#matrix);
      this.#matrix.toArray(this.#pose, i * 16);
    }
  }
  add(source: SkinnedMesh, transform: Matrix4): IInstanceHandle {
    this.#live();
    rigidScale(transform);
    this.#readPose(source);
    return this.palette.allocate(this.#pose, transform.elements);
  }
  setPose(handle: IInstanceHandle, source: SkinnedMesh): void {
    this.#live();
    this.#readPose(source);
    this.palette.writePose(handle, this.#pose);
  }
  setTransform(handle: IInstanceHandle, transform: Matrix4): void {
    this.#live();
    rigidScale(transform);
    this.palette.writeTransform(handle, transform.elements);
  }
  remove(handle: IInstanceHandle): void {
    this.#live();
    this.palette.release(handle);
  }
  prepare(frame: number): IFrameSnapshot {
    this.#live();
    const snapshot = this.palette.prepare(frame);
    if (snapshot.frame !== this.#frame.frame) {
      for (const buffer of this.#buffers) buffer.needsUpdate = true;
    }
    this.#frame = snapshot;
    this.mesh.geometry.instanceCount = snapshot.count;
    return snapshot;
  }
  #raycast(raycaster: Raycaster, hits: Intersection<Object3D>[]): void {
    this.#live();
    this.mesh.updateWorldMatrix(true, false);
    this.#pick.matrixWorld.copy(this.mesh.matrixWorld);
    ...
    for (let draw = 0; draw < this.#frame.count; draw++) {
      const pose = this.palette.currentBones.subarray(
        draw * this.#pose.length,
        (draw + 1) * this.#pose.length,
      );
      this.#matrix.fromArray(this.palette.currentTransforms, draw * 16);
      for (let i = 0; i < position.count; i++) {
        for (let c = 0; c < 4; c++) {
          indices[c] = this.#indices.getComponent(i, c);
          weights[c] = this.#weights.getComponent(i, c);
        }
        for (let c = 0; c < 3; c++) point[c] = this.#positions.getComponent(i, c);
        skinPoint(pose, indices, weights, point, out);
        this.#point.fromArray(out).applyMatrix4(this.#matrix);
        position.setXYZ(i, this.#point.x, this.#point.y, this.#point.z);
      }
      this.#pick.geometry.computeBoundingBox();
      this.#pick.geometry.computeBoundingSphere();
      const found: Intersection[] = [];
      this.#pick.raycast(raycaster, found);
      for (const hit of found)
        hits.push(
          Object.assign(hit, {
            object: this.mesh,
            instanceId: this.palette.drawSlots[draw],
            instanceGeneration: this.palette.drawGenerations[draw],
          }),
        );
    }
  }
