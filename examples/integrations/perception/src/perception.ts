...
import { SearchController, validatePoint, type ISearchOptions, type ISearchDecision, type Point3 } from './search.js';
export interface IVisibilityRay { readonly origin: Point3; readonly direction: Point3; readonly distance: number; }
...
export interface IPerceptionTarget { readonly id: string; readonly object: Object3D; }
...
export class YukaPerception {
  ...
  constructor(options: IPerceptionOptions) {
    ...
    if (!Number.isFinite(options.fieldOfView) || options.fieldOfView <= 0 || options.fieldOfView > Math.PI * 2 ||
        !Number.isFinite(options.range) || options.range <= 0 || typeof options.raycast !== 'function')
      throw new Error('Perception requires a finite view cone, positive range and world ray query.');
    ...
    this.#forward.normalize(); this.#body = options.body; this.#raycast = options.raycast;
    ...
    this.#vision.fieldOfView = options.fieldOfView; this.#vision.range = options.range;
  }
  update(dt: number, target: IPerceptionTarget | null): ISearchDecision {
    ...
    this.#body.getWorldPosition(this.#position); this.#body.getWorldQuaternion(this.#rotation);
    const origin = this.#position.toArray() as [number, number, number]; validatePoint(origin);
    if (this.#rotation.toArray().some(value => !Number.isFinite(value)) || this.#rotation.lengthSq() < 1e-12)
    ...
    if (target) {
      ...
      const position = this.#target.toArray() as [number, number, number]; validatePoint(position);
      ...
      if (this.#vision.visible(this.#point)) {
        ...
        if (distance > 0) {
          ...
          const hit = this.#raycast({origin, direction: this.#direction.toArray() as [number, number, number], distance});
          if (hit !== null && (!Number.isFinite(hit) || hit < 0)) throw new Error('Perception ray query returned an invalid distance.');
  ...
  forget(): void { this.#brain.forget(); }
  dispose(): void { if (!this.#disposed) { this.#disposed = true; this.#brain.dispose(); } }
}
