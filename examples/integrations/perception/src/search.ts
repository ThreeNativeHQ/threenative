...
export class SearchController {
  ...
  constructor(options: ISearchOptions) {
    if (
      !Number.isFinite(options.memorySeconds) ||
      options.memorySeconds <= 0 ||
      !Number.isFinite(options.arrivalDistance) ||
      options.arrivalDistance < 0
    )
  ...
  step(dt: number, position: Point3, observation: IObservation | null): ISearchDecision {
    ...
    if (observation) {
      if (typeof observation.id !== "string" || !observation.id.trim())
        throw new Error("Perception target id is required.");
      validatePoint(observation.position);
    }
    if (observation) {
      this.#memory = {
        id: observation.id,
        position: Object.freeze([...observation.position]) as Point3,
      };
      this.#age = 0;
    } else if (this.#memory) {
      ...
      if (
        this.#age >= this.#memorySeconds ||
        Math.hypot(...position.map((v, i) => v - destination[i])) <= this.#arrivalDistance
      )
        this.#memory = null;
    }
