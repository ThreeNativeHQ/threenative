...
import type { GLTF, GLTFLoader, GLTFLoaderPlugin, GLTFParser } from 'three/addons/loaders/GLTFLoader.js';
...
function aborted(signal?: AbortSignal): void {
  if (signal?.aborted) { const error = new Error('VRM instantiation was aborted.'); error.name = 'AbortError'; throw error; }
}
...
export class VrmAvatar {
  ...
  constructor(vrm: VRM, clips: readonly AnimationClip[], release: (scene: Object3D) => void) {
    this.vrm = vrm; this.clips = clips; this.#release = release;
  }
  get scene(): Object3D { return this.vrm.scene; }
  ...
  setExpression(name: string, weight: number): void {
    ...
    if (!Number.isFinite(weight) || weight < 0 || weight > 1) throw new Error('VRM expression weight must be in [0,1].');
    manager.setValue(name, weight);
  }
  dispose(): void {
    if (this.#disposed) return;
    this.#disposed = true; this.scene.removeFromParent(); this.#release(this.scene);
  }
}
/** Supply model to createAssetLoader; core continues to own logical-path resolution and caching. */
export function createVrmModelReader(loader: GLTFLoader, options: IVrmReaderOptions = {}): IVrmModelReader {
  if (attachedLoaders.has(loader)) throw new Error('VRM reader is already registered on this loader.');
  const plugin = options.plugin ?? vrmWebGpuPlugin;
  const release = options.release ?? (scene => VRMUtils.deepDispose(scene));
  const readBytes = options.readBytes ?? (async url => {
    const response = await fetch(url);
    if (!response.ok) throw new Error(`VRM asset '${url}' failed with HTTP ${response.status}.`);
    return new Uint8Array(await response.arrayBuffer());
  });
  attachedLoaders.add(loader); loader.register(plugin);
  let disposed = false;
  const live = (): void => { if (disposed) throw new Error('VRM model reader is disposed.'); };
  return {
    async model(url) {
      live();
      const bytes = (await readBytes(url)).slice(); live();
      const document = inspectAvatarDocument(bytes);
      const clean = url.split(/[?#]/u)[0]; const resourcePath = clean.slice(0, clean.lastIndexOf('/') + 1);
      const parse = async (): Promise<GLTF> => {
        live(); const gltf = await loader.parseAsync(bytes.slice().buffer, resourcePath);
        if (disposed) { release(gltf.scene); live(); }
        return gltf;
      };
      ...
      return {
        ...
        async instantiate(signal) {
          ...
          try {
            ...
            if (!(vrm instanceof VRM)) throw new Error(`VRM '${url}' did not produce a valid humanoid; check retained extensions and resources.`);
            return new VrmAvatar(vrm, gltf.animations, release);
          } catch (error) { release(gltf.scene); throw error; }
        },
        dispose() { assetDisposed = true; },
      };
    },
    dispose() {
      if (disposed) return;
      disposed = true; loader.unregister(plugin); attachedLoaders.delete(loader);
    },
  };
}
