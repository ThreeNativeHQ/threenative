...
const geometry = (extra = {}) => ({
  attributes: {
    position: { array: new Float32Array([0, 0, 0, 1, 0, 0, 0, 1, 0, 9, 9, 9]), itemSize: 3 },
    uv: { array: new Float32Array([0, 0, 1, 0, 0, 1, 1, 1]), itemSize: 2 },
  },
  indices: new Uint32Array([3, 3, 3, 0, 1, 2]),
...
test("does not alias source arrays", () => {
  ...
  r.attributes.position.array[0] = 42;
...
test("supports non-indexed geometry and normalized integer attributes", () => {
  ...
  g.attributes.color = {
    array: new Uint8Array([255, 0, 0, 0, 255, 0, 0, 0, 255, 9, 9, 9]),
    itemSize: 3,
    normalized: true,
  };
  const r = compactTriangles(g);
  assert.ok(r.attributes.color.array instanceof Uint8Array);
  assert.equal(r.attributes.color.normalized, true);
...
test("handles an explicitly empty result", () => {
  ...
  assert.equal(r.attributes.position.array.length, 0);
});
...
test("rejects NaN and mismatched attribute counts", () => {
  const g = geometry();
  g.attributes.position.array[0] = Number.NaN;
  assert.throws(() => compactTriangles(g), /finite/i);
  g.attributes.position.array[0] = 0;
  g.attributes.uv.array = new Float32Array(2);
...
test("rejects overlapping groups and uncovered active triangles", () => {
  ...
  assert.throws(
    () => compactTriangles(geometry({ groups: [{ start: 0, count: 3, materialIndex: 0 }] })),
    /cover/i,
  );
});
test("uses uint32 when compacted indices exceed uint16", () => {
  ...
  const r = compactTriangles({
    attributes: { position: { array: p, itemSize: 3 } },
    indices: null,
    range: { start: 0, count: n },
    groups: [],
  });
  assert.ok(r.indices instanceof Uint32Array);
  assert.equal(r.indices[n - 1], n - 1);
});
test("Infinity means the remaining backing range, not extra triangles", () => {
  assert.equal(
    compactTriangles(geometry({ range: { start: 3, count: Number.POSITIVE_INFINITY } })).indices
      .length,
    3,
  );
});
