import { BufferAttribute, BufferGeometry, Matrix4, Mesh, type Material } from 'three';
import { ADDITION, Brush, Evaluator, INTERSECTION, SUBTRACTION } from 'three-bvh-csg';
import { compactTriangles, type IAttribute, type NumericArray } from './active-geometry.js';

export type SolidOperation = 'union' | 'subtract' | 'intersect';
export interface ISolidResult { readonly mesh: Mesh; dispose(): void; }
...
export function exportGeometry(source: BufferGeometry): BufferGeometry {
  if (Object.keys(source.morphAttributes).length) throw new Error('CSG export does not support morph attributes.');
  const attributes: Record<string, IAttribute> = Object.create(null);
  for (const [name, attribute] of Object.entries(source.attributes)) {
    if ('isInterleavedBufferAttribute' in attribute)
      throw new Error(`CSG interleaved attribute '${name}' is not supported; deinterleave it before authoring.`);
    attributes[name] = { array: attribute.array as NumericArray, itemSize: attribute.itemSize, normalized: attribute.normalized };
  }
  ...
  for (const group of compact.groups) geometry.addGroup(group.start, group.count, group.materialIndex);
  if (compact.indices.length) { geometry.computeBoundingBox(); geometry.computeBoundingSphere(); }
  return geometry;
}
function brushFrom(mesh: Mesh): Brush {
  mesh.updateWorldMatrix(true, false);
  const matrix = mesh.matrixWorld;
  if (matrix.elements.some(value => !Number.isFinite(value)) || Math.abs(matrix.determinant()) < 1e-12)
...
export function evaluateSolid(left: Mesh, right: Mesh, operation: SolidOperation): ISolidResult {
  ...
  let a: Brush | undefined; let b: Brush | undefined; let result: Brush | undefined;
  try {
    a = brushFrom(left); b = brushFrom(right);
    ...
    if (JSON.stringify(names) !== JSON.stringify(Object.keys(b.geometry.attributes).sort()))
      throw new Error('CSG operands must expose the same attributes; no attribute is silently dropped.');
    for (const name of names) {
      const aa = a.geometry.getAttribute(name); const bb = b.geometry.getAttribute(name);
      if (aa.itemSize !== bb.itemSize || aa.normalized !== bb.normalized)
        throw new Error(`CSG attribute '${name}' layouts differ.`);
    }
    const evaluator = new Evaluator();
    evaluator.attributes = names; evaluator.useGroups = true;
    ...
    let disposed = false;
    return { mesh, dispose() { if (!disposed) { disposed = true; geometry.dispose(); } } };
  } finally {
    for (const brush of [result, b, a]) if (brush) { brush.disposeCacheData(); brush.geometry.dispose(); }
  }
}
