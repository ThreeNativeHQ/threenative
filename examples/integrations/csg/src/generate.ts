...
try {
  ...
  console.info(`Wrote ${bytes.byteLength} bytes to ${output}; run the ordinary ThreeNative asset cook next.`);
} finally { result.dispose(); wall.geometry.dispose(); cutter.geometry.dispose(); material.dispose(); }
