...
test("roots do not move; tips stay within the authored amplitude", () => {
  assert.equal(windSample(-1, 0, settings).offset, 0);
  assert.equal(windSample(0, 0, settings).slope, 0);
  assert.equal(windSample(8, 0, settings).offset, 2);
});
test("analytic slope agrees with finite differences", () => {
  const h = 1e-5;
  for (const y of [0.1, 1, 2, 3.9]) {
    const f = windSample(y, 0, settings);
    const derivative =
      (windSample(y + h, 0, settings).offset - windSample(y - h, 0, settings).offset) / (2 * h);
    assert.ok(Math.abs(f.slope - derivative) < 1e-8);
  }
});
test("wind is deterministic and bounded over a time series", () => {
  for (let i = 0; i < 500; i++) {
    const a = windSample(3, i / 60, settings);
    assert.deepEqual(a, windSample(3, i / 60, settings));
