...
const game = defineGame<GameState, PhysicsContext>({
  initialState,
  input: {
    // Gamepad button 0 is A and 7 is RT, which is what the HUD prompts say.
    dash: { buttons: [7], down: ["ShiftLeft", "ShiftRight"] },
    jump: { buttons: [0], down: ["Space"] },
  },
  // Gravity is the character controller's, not the world's: the fox is
  // kinematic and integrates its own. Rapier only moves the crate and the lift.
