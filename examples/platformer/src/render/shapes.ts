...
//
// Rounded, cached primitives. A sharp BoxGeometry reads as Minecraft; the same
// box with a 0.14 corner radius reads as a toy, and that soft corner-wrap is
// most of the storybook look the reference has and a box stack does not.
//
// Nothing here is textured. Surface variety comes from alternating palette
// entries in a loop, never from a bitmap — `CanvasTexture` samples black under
// `WebGPURenderer`, and colour-per-face is both cheaper and closer to the
// reference anyway.
import {
  BoxGeometry,
  type BufferGeometry,
