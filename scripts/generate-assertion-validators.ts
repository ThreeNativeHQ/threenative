...
function typeName(entry: IPlaytestAssertionSchemaEntry): string {
  return `IPlaytest${pascalCase(entry.kind)}Assertion`;
}

function typeFromConstraint(constraint: IPlaytestAssertionSchemaConstraint): string {
  switch (constraint.kind) {
    ...
    case "json":
      return "unknown";
    case "literal":
      return constraint.values.map((value) => JSON.stringify(value)).join(" | ");
    case "number":
      return "number";
    case "string":
      return "string";
    case "array":
      return `Array<${typeFromConstraint(constraint.items)}>`;
    case "tuple":
      return `[${constraint.items.map(typeFromConstraint).join(", ")}]`;
    case "union":
      return constraint.variants.map(typeFromConstraint).join(" | ");
    case "record":
      return renderTypeRecord(constraint.fields);
  }
}

function renderTypeRecord(
  fields: readonly IPlaytestAssertionSchemaField[],
  excludedNames: readonly string[] = [],
): string {
  const entries = fields.map(
    (field) =>
      `${field.name}${field.required === true ? "" : "?"}: ${typeFromConstraint(field.constraints)}`,
  );
  entries.push(...excludedNames.map((name) => `${name}?: never`));
  return entries.length === 0 ? "Record<string, never>" : `{ ${entries.join("; ")} }`;
}

function renderGeneratedAssertionTypes(
  ...
  for (const entry of registry) {
    ...
    if (entry.variants === undefined) {
      const base = renderTypeRecord(entry.fields);
      lines.push(
        entry.kind === "reachability"
          ? `export type ${name} = ${base} & { envelope?: { fallDistanceToGround: number; forwardReach: number; maxRise: number } };`
          : `export type ${name} = ${base};`,
      );
      continue;
    }
    const variantNames = entry.variants.map((variant, index) => {
      ...
      const excluded = entry.fields
        .map((field) => field.name)
        .filter((fieldName) => !fields.some((field) => field.name === fieldName));
      const variantName = `${name}Variant${index}`;
      lines.push(`type ${variantName} = ${renderTypeRecord(fields, excluded)};`);
      return variantName;
    });
    lines.push(`export type ${name} = ${variantNames.join(" | ")};`);
  }
  const resources = registry.find((entry) => entry.kind === "resources");
  if (resources?.variants !== undefined) {
    const resourceName = typeName(resources);
    lines.push(
      `export type IPlaytestResourcePathAlternative = ${resourceName}Variant0["anyOf"][number];`,
      `export type IPlaytestResourceAnyOfAssertion = ${resourceName}Variant0;`,
      `export type IPlaytestResourcePathAssertion = ${resourceName}Variant1;`,
      `export type IPlaytestResourceAssertion = ${resourceName};`,
      "export type IPlaytestPathAssertion = IPlaytestResourcePathAssertion;",
    );
  }
  const world = registry.find((entry) => entry.kind === "world");
  if (world !== undefined) {
    lines.push(
  ...
  lines.push("", "export interface IPlaytestScenarioAssertions {");
  for (const entry of registry) {
    lines.push(`  ${entry.kind}?: ${typeName(entry)}${entry.cardinality === "array" ? "[]" : ""};`);
  }
  lines.push("}", "");
  return lines.join("\n");
}
...
export { renderGeneratedAssertionTypes };

async function main(): Promise<void> {
  const source = renderGeneratedAssertionValidators();
  const typesSource = renderGeneratedAssertionTypes();
  if (process.argv.includes("--check")) {
    let current: string;
    let currentTypes: string;
    try {
      current = await readFile(target, "utf8");
      currentTypes = await readFile(typesTarget, "utf8");
    } catch {
      console.error("assertion validators or assertion types are stale or missing");
      process.exitCode = 1;
      return;
    }
    if (current !== source || currentTypes !== typesSource) {
      console.error(
        "assertion validators or assertion types are stale: regenerate generated artifacts",
      );
      process.exitCode = 1;
      return;
    }
  ...
  await writeFile(typesTarget, typesSource);
  console.log(
    `assertion validators and types: ${PLAYTEST_ASSERTION_REGISTRY.length} kinds -> ${path.relative(repoRoot, target)}, ${path.relative(repoRoot, typesTarget)}`,
  );
}
