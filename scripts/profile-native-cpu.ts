...
export function parseProfileArgs(args: readonly string[]): IProfileArgs {
  ...
  return {
    ...
    renderModes: enumList(
      value("--render-mode"),
      ["independent"],
      ["independent", "distinct-materials", "instanced", "merged", "scene-collapse"],
      "--render-mode",
    ),
...
const PRESENTATION_GUIDANCE =
  "Canonical visual evidence command: xvfb-run -a -s '-screen 0 1600x900x24' pnpm profile:native-cpu -- --headed --verify-presentation --scenario fox-scale";
