...
describe("quality JSON command", () => {
  it("should emit only parseable finding records through the documented silent form", () => {
    ...
    expect(records.some((record) => record.threshold === 14 || record.threshold === 2)).toBe(false);
  });
});
