...
const recommended = [
  "ambientcg_list_files", "ambientcg_search_assets", "asset_download_file", "asset_search_sources",
  "audio_download_asset", "audio_search_assets", "polyhaven_list_files", "polyhaven_search_assets",
  "asset_inspect_rig", "asset_auto_rig", "asset_retarget_animations", "asset_preview_animation",
];
...
function fixture() {
  ...
  return {
    ...
    close() { lines.close(); stdin.destroy(); stdout.destroy(); },
  };
}

describe("published asset MCP capture", () => {
  for (const [name, response, pattern] of [
    ["JSON-RPC error", { error: { code: -32601, message: "method unavailable" } }, /method unavailable/],
  ...
  it("ignores nonresponses without losing a later matching response", async () => {
    ...
    try {
      ...
      for (const value of [null, [], 42, { id: 99, result: {} }, { method: "notification" }]) f.reply(value);
  ...
  it("captures every page, validates the recommended surface, and sorts names", async () => {
    ...
    f.child.stdin.on("data", (data: Buffer) => {
      ...
      const result = sent.params.cursor === undefined
  ...
  for (const [name, result, pattern] of [
    ...
    ["duplicate name", { tools: [{ name: "asset_inspect_rig" }, { name: "asset_inspect_rig" }] }, /invalid or duplicate/],
  ...
  it("drives a real child over newline-delimited stdio", async () => {
    ...
    try {
      assert.deepEqual(await request(child, lines, { value: 1 }, "initialize", {}, 2_000), { method: "initialize" });
