...
function operationDoc(file: string, symbol: string): string {
  ...
  for (const statement of source.statements) {
    ...
    assert.equal(
      ...
      1,
      `${symbol} shares another callable's metadata`,
    );
...
test("browser recipe is explicit and does not contaminate pointer metadata", () => {
  assert.match(
    operationDoc("runner/index.ts", "resolveBrowserArguments"),
    /resolveBrowserArguments\(WEBGPU_BROWSER_ARGS\)/,
  );
  assert.doesNotMatch(
    operationDoc("runner/index.ts", "reconcileBrowserPointers"),
    /Vulkan|SwiftShader|Chromium/,
  );
});

test("scenario error construction does not advertise loading or deterministic stepping", () => {
  assert.doesNotMatch(
    operationDoc("index.ts", "invalidScenario"),
    /@situation.*(?:load|deterministic|ticks)/,
  );
});
