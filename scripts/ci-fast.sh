#!/usr/bin/env bash
# The pre-push tier: bounded drift checks, and nothing that needs a full workspace build, GPU or a
# clock. Full local verification remains `pnpm ci:local`.
#
...
# `test-playtest`, `golden-path`, `visuals`. They are minutes to tens of minutes, and a hook people
