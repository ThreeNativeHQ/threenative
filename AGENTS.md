...
1. **Search the manifest before you write, and let a rule decide when — not your judgement.**
   Run `engine_search_capabilities`, then `engine_capability_detail` on every hit, before you add a
   file under `src/` or `packages/`, before you write a helper longer than a screen, and before you
   turn a render stage on or off. Not "before you write a system": that is a judgement every agent
   makes in its own favour, and this file used to say it. Measured here on 2026-09-03 — a session
   added 880 colliders, rewrote a loading gate and tuned four render stages without one search,
   because none of it felt like *a system*; `ClusteredBatch`, which exists precisely to make
   thousands of scanned props affordable, went unread through the whole density pass. The templates
   have said `before writing any entity` for months. This repository was instructing its users
   better than itself.

   The manifest (`packages/create-threenative/capabilities.json`, regenerated by `pnpm build`) is
   the only complete list of the public surface, searchable by plain-words situation — *"enemy walks
   around a wall"*, *"put a weapon in a character's hand"*. Several exports are subpath imports
   (`@threenative/physics/navigation`) that grepping will never reveal, and its constraints are
   binding. A game once hand-wrote 446 lines that were already installed, and ran at 9 FPS.
   Both tools come from the shipped `threenative-engine-mcp` server. Installing `@threenative/core`
   writes the `.mcp.json` that wires it up, so a project that added ThreeNative by hand has it too —
   but it is read from the directory the session was launched in, so a session started in a parent
   of the game directory registers nothing. Launch from the game-project root, or write a
   `.mcp.json` there.
...
this repository. `docs/benchmark/sweeps/` is the same hazard and is now untracked for it: 2,963
generated arm sources every grep used to cross. The archives stay on disk and every `sweep:*`
command still reads them.
