...
describe("SceneRenderProjection batches across differing geometries", () => {
  ...
  it("invalidates a cached material bundle only when its draw structure changes", () => {
    const scene = new Scene();
    const material = new MeshStandardMaterial({ color: 0x335577 });
    const meshes = town(scene, material, 40);
    const projection = projected(scene, 600);
    const bundle = projection.root.children.find(
      (object) => (object as { isBundleGroup?: boolean }).isBundleGroup === true,
    ) as (Group & { version: number }) | undefined;
    const batch = bundle?.children[0] as
      | (BatchedMesh & {
    ...
    // Model the first render-bundle recording. BatchedMesh builds its multidraw list only from
    // onBeforeRender; Three r185 skips that hook while replaying a cached bundle.
    batch?.onBeforeRender(
      undefined as never,
      projection.root as Scene,
      new PerspectiveCamera(),
      batch.geometry,
      material,
      null,
    );
    expect(batch?._multiDrawCount).toBe(40);
    const settledVersion = bundle?.version as number;

    // Texture-backed transforms and live material uniforms do not change recorded draw commands,
    // so invalidating these would throw away the bundle's intended steady-state win.
    (meshes[5] as Mesh).position.x = 500;
    material.color.setHex(0xdd8844);
    projection.reconcile();
    expect(bundle?.version).toBe(settledVersion);

    // Visibility does change which multidraw commands exist. The version bump makes r185 record
    // the bundle again, which runs onBeforeRender and drops the hidden instance from that list.
    (meshes[9] as Mesh).visible = false;
    projection.reconcile();
    expect(bundle?.version).toBe(settledVersion + 1);
    batch?.onBeforeRender(
      undefined as never,
      projection.root as Scene,
      new PerspectiveCamera(),
    ...
    expect(batch?._multiDrawCount).toBe(39);

    const hiddenVersion = bundle?.version as number;
    scene.remove(meshes[8] as Mesh);
    projection.reconcile();
    expect(bundle?.version).toBe(hiddenVersion + 1);
    batch?.onBeforeRender(
      undefined as never,
      projection.root as Scene,
      new PerspectiveCamera(),
    ...
    expect(batch?._multiDrawCount).toBe(38);
  });
