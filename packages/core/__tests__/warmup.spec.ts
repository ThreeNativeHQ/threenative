...
/**
 * The gap these close, measured rather than reasoned about.
 *
 * `compileAsync` walks the main render list and nothing else (three 0.185.1,
 * `build/three.webgpu.js:60185`-`:60245`). The shadow map is rendered from the light's own node
 * update inside `render()` (`:45416`, `:45521`), and the tone-mapping output pipeline is built
 * when the frame resolves to the canvas. Neither is reachable from a compile walk, so a warm-up
 * that returns `abandoned: 0, timedOut: false` still leaves them to the first `render()`.
 *
 * The desktop probe counted it on an RTX 2080 with the real game's render shape: both warm-up
 * granularities compiled 4 pipelines and the first frame still built 2 synchronously — the
 * `ShadowMaterial` pipeline at 1024x1024 and `outputColorTransform` at the canvas
 * (`docs/verification/prd-360-startup-cost-2026-09-08/`). A green report hid them both.
 *
 * The seam is the caller's because only the caller knows whether the frame it is about to draw is
 * hidden. The engine passes it while an opaque startup layer covers the canvas; a game whose
 * loading surface is a transparent HUD gets no early world frame, because that would be a look
 * change bought for a compile.
 */
describe("scene warm-up first-use coverage", () => {
  test("should render first-use work inside the warm-up window, after the compile walk", async () => {
    const order: string[] = [];
    const renderer = {
      compileAsync: (object: unknown) => {
        order.push(`compile:${(object as IFakeObject).name}`);
        return Promise.resolve();
      },
    };
    const firstUseRender = vi.fn(() => {
      order.push("firstUseRender");
    });
    const yieldFrame = vi.fn(() => {
      order.push("yield");
      return Promise.resolve();
    });

    const report = await warmUpScene(renderer as never, sceneOf(3) as never, {} as never, {
      firstUseRender,
      yieldFrame,
    });

    expect(firstUseRender).toHaveBeenCalledTimes(1);
    // A frame between the compile walk and the render, so the loading surface presents once more
    // before the work that used to freeze it.
    expect(order).toEqual(["compile:scene", "yield", "firstUseRender"]);
    expect(report.firstUseRendered).toBe(true);
  });

  test("should render first-use work at object granularity too", async () => {
    const renderer = fakeRenderer();
    const firstUseRender = vi.fn();

    const report = await run(renderer, sceneOf(2), { firstUseRender });

    expect(renderer.compiled).toEqual(["mesh-0", "mesh-1"]);
    expect(firstUseRender).toHaveBeenCalledTimes(1);
    expect(report.firstUseRendered).toBe(true);
  });

  test("should say so rather than claim coverage when the budget ran out first", async () => {
    const renderer = { compileAsync: () => new Promise<void>(() => undefined) };
    const firstUseRender = vi.fn();
    ...
    // The launch must never be worse for having tried to optimize it: an exhausted budget releases
    // the game rather than spending more of it on a render.
    expect(firstUseRender).not.toHaveBeenCalled();
    expect(report).toMatchObject({ firstUseRendered: false, timedOut: true });
  }, 10_000);

  test("should report a failed first-use render instead of throwing into the launch", async () => {
    const renderer = fakeRenderer();
    const firstUseRender = vi.fn(() => {
      throw new Error("device lost");
    });

    const report = await run(renderer, sceneOf(1), { firstUseRender });

    expect(report.firstUseRendered).toBe(false);
    expect(report.firstUseFailure).toMatch(/device lost/u);
  });

  test("should not store a persistent hint while first-use work stayed uncovered", async () => {
    ...
    try {
      const renderer = fakeRenderer();
      const report = await run(renderer, sceneOf(1), {
        cache: { key: "bayview-1" },
        firstUseRender: () => {
          throw new Error("device lost");
        },
      });
      // A hit skips the whole compile walk on the next launch. Storing one for a warm-up that
      // left the first frame to compile shadows would make the next launch worse and silent.
      expect(report.cache).not.toBe("stored");
  ...
  test("should leave the report unchanged when no seam is supplied", async () => {
    const renderer = fakeRenderer();
    const report = await run(renderer, sceneOf(1));
    expect(report.firstUseRendered).toBeUndefined();
    expect(report.firstUseFailure).toBeUndefined();
  });
});

/**
 * Why this option exists, measured rather than assumed: the native host runs a **two-thread**
 * pipeline compile pool (`packages/runtime-native/src/webgpu/bindings_pipelines.cpp:229`), and
 * three's own `compileAsync` awaits each object's pipeline promises before starting the next
 * (`three.webgpu.js:60259`, `:60278`, `:60285`). One job is ever queued, so one worker ever runs and
 * the other idles for the whole launch. On a Pixel 8 that walk took 15–21 s against 8.5 s for
 * letting the first frame compile the same pipelines synchronously.
 */
describe("scene warm-up compile concurrency", () => {
  test("should keep several compiles in flight when asked", async () => {
    let inFlight = 0;
    let peak = 0;
    ...
    const running = run(renderer, sceneOf(6), { compileConcurrency: 3, sliceSize: 3 });
    // Drain whatever the walk has queued, letting each group start before releasing the next.
    for (let round = 0; round < 20 && (release.length > 0 || round < 4); round += 1) {
      await new Promise<void>((resolve) => setTimeout(resolve, 0));
      while (release.length > 0) (release.shift() as () => void)();
    }
    expect(peak).toBe(3);
    const report = await running;
    expect(report.compiled).toBe(6);
    expect(report.abandoned).toBe(0);
  }, 10_000);

  test("should default to the serial walk", async () => {
    let peak = 0;
    let inFlight = 0;
    const renderer = {
      compileAsync: () => {
    ...
    const report = await run(renderer, sceneOf(4));
    expect(peak).toBe(1);
    expect(report.compiled).toBe(4);
  });

  test("should fail closed on a concurrency that cannot progress", async () => {
    const renderer = fakeRenderer();
    for (const compileConcurrency of [0, -2, 1.5]) {
      await expect(run(renderer, sceneOf(2), { compileConcurrency })).rejects.toThrow(
        /TN_WARMUP_CONCURRENCY_INVALID/u,
      );
    }
  });
});
