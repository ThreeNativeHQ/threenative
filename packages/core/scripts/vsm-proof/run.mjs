...
const luminance = (png, x0, y0, x1, y1) => {
  let sum = 0, n = 0;
  for (let y = y0; y < y1; y += 1) for (let x = x0; x < x1; x += 1) { const i = (y * png.width + x) * 4; sum += 0.2126 * png.data[i] + 0.7152 * png.data[i + 1] + 0.0722 * png.data[i + 2]; n += 1; }
  return sum / n;
};
const results = {};
const shots = {};
try {
  ...
  for (const [mode, query] of [["stock", "mode=stock"], ["virtual", "mode=virtual"], ["one", "mode=virtual&clip=60"], ["one", "mode=virtual&clip=60"]]) {
    ...
    try {
      await page.waitForFunction(() => window.__PROOF__ !== undefined, undefined, { timeout: 60_000 });
    } catch (error) {
    ...
    if (mode === "stock") {
      let best = { x: 0, y: 256, value: Infinity };
      for (let y = 256; y < png.height - 32; y += 8) for (let x = 0; x < png.width - 32; x += 8) {
    ...
    results[mode] = { adapter: proof.adapter, under: Math.round(under), beside: Math.round(beside), stats: proof.stats, counts: proof.counts, marker: logs.filter((l) => l.startsWith("TN_VIRTUAL_SHADOW")).slice(-1)[0] };
    await page.close();
  }
  await browser.close();
  let changed = 0;
  const a = shots.stock, b = shots.virtual;
  for (let i = 0; i < a.data.length; i += 4) { if (Math.abs(a.data[i] - b.data[i]) + Math.abs(a.data[i + 1] - b.data[i + 1]) + Math.abs(a.data[i + 2] - b.data[i + 2]) > 60) changed += 1; }
