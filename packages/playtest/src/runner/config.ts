...
export const PLAYTEST_FLAGS = {
  ...
  "--target": { default: "browser", summary: "execution target (browser or android)", takesValue: true },
...
export function parseStandalonePlaytestArgs(argv: readonly string[], cwd = process.cwd()): IStandalonePlaytestConfig {
  ...
  if (target !== "browser" && target !== "android") {
    throw new PlaytestCliUsageError(`Unknown target '${target}'. Expected 'browser' or 'android'.`);
  }
