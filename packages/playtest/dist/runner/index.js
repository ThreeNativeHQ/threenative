...
new Set(PLAYTEST_CAPABILITY_REGISTRY.map(({ name }) => name));
...
function tiltDegrees(rotation) {
  ...
  const length2 = Math.hypot(x, y, z, w);
  if (length2 <= Number.EPSILON) return void 0;
  const upDot = 1 - 2 * ((x / length2) ** 2 + (z / length2) ** 2);
...
function validateStep(value, scenarioPath, index) {
  ...
  const waitFrames2 = positiveInteger(value.waitFrames);
  ...
  if (press === void 0 && overlayMessage === void 0 && pointerPosition === void 0 && window === void 0 && waitFrames2 === void 0 && waitTicks === void 0) {
  ...
  if (value.waitFrames !== void 0 && waitFrames2 === void 0) {
  ...
  if (waitTicks !== void 0 && waitFrames2 !== void 0) {
  ...
  return {
    ...
    ...waitFrames2 === void 0 ? {} : { waitFrames: waitFrames2 },
...
function validateOptionalNumberTuple(value, key, length2, scenarioPath, index) {
  if (!hasKey(value, key)) {
    return void 0;
  }
  const tuple = length2 === 3 ? validateNumberTuple(value[key], 3) : validateNumberTuple(value[key], 4);
  if (tuple === void 0) {
    throw invalidScenario(scenarioPath, `Scenario setup.entities[${index}].${key} must be a ${length2}-number tuple.`);
  }
  return tuple;
}
function validateNumberTuple(value, length2) {
...
var PlaytestBridgeError = class extends Error {
  constructor(diagnostic) {
    super(diagnostic.message);
    this.diagnostic = diagnostic;
  }
...
async function connectPlaytestBridge(page, scenario) {
  const required = requiredPlaytestCapabilities(scenario);
  const semanticRequired = required.filter((capability2) => !capability2.startsWith("browser."));
  ...
  const description = await bridgeCall(page, "describe");
  ...
  const ready = await bridgeCall(page, "ready");
  if (!ready.ready) {
    throw new PlaytestBridgeError(playtestDiagnostic(
      "TN_PLAYTEST_BRIDGE_NOT_READY",
      `Playtest bridge is not ready${ready.reason === void 0 ? "." : `: ${ready.reason}`}`,
  ...
  return {
    ...
    sample: async (request) => {
      assertBoundedPayload(request);
      const snapshot = await bridgeCall(page, "sample", request);
...
async function bridgeCall(page, method, argument) {
  let timeout;
  try {
    return await Promise.race([
      page.evaluate(
        async ({ argument: argument2, globalName, method: method2 }) => {
          const bridge = globalThis[globalName];
          const operation = bridge?.[method2];
          if (typeof operation !== "function") {
            throw new Error(`Bridge operation '${method2}' is unavailable.`);
          }
          return operation.call(bridge, argument2);
        },
...
function parseStandalonePlaytestArgs(argv, cwd = process.cwd()) {
  const scenarioPath = readFlag(argv, "--scenario") ?? positional(argv);
  if (scenarioPath === void 0) {
  ...
  return {
    ...
    projectPath,
    scenarioPath,
    ...serverCommand === void 0 ? {} : { server: { command: serverCommand, cwd: projectPath, timeoutMs: readPositiveInteger(readFlag(argv, "--server-timeout"), 15e3) } },
    timeoutMs: readPositiveInteger(readFlag(argv, "--timeout"), 15e3),
...
async function initStandalonePlaytest(projectPath = process.cwd()) {
  const files = {
...
var ManagedServerError = class extends Error {
  constructor(diagnostic) {
    super(diagnostic.message);
    this.diagnostic = diagnostic;
  }
...
async function runStandalonePlaytest(config) {
  ...
  const server = config.server === void 0 ? void 0 : startManagedServer(config);
  ...
  try {
    ...
    if (bridge !== void 0 && scenario.setup !== void 0) {
...
function buildReport(config, scenario, beforeSnapshot, afterSnapshot, consoleEntries, networkEntries) {
  ...
  const movementDelta = beforePosition === void 0 || afterPosition === void 0 ? void 0 : subtract(afterPosition, beforePosition);
  const distance = movementDelta === void 0 ? 0 : length(movementDelta);
  const diagnostics = [];
  const base = {
    ...afterPosition === void 0 ? {} : { after: { frame: scenario.steps.length, position: afterPosition, ...afterRotation === void 0 ? {} : { rotation: afterRotation }, tick: afterSnapshot?.clock.tick ?? 0 } },
    ...beforePosition === void 0 ? {} : { before: { frame: 0, position: beforePosition, ...beforeRotation === void 0 ? {} : { rotation: beforeRotation }, tick: beforeSnapshot?.clock.tick ?? 0 } },
    diagnostics,
    distance,
    entity,
    expectMoved: scenario.assert?.movement?.minDistance !== void 0,
    frames: scenario.steps.reduce((total, step) => total + (step.holdFrames ?? step.waitFrames ?? step.holdTicks ?? step.waitTicks ?? 1), 0),
    ...movementDelta === void 0 ? {} : { movementDelta },
    observations: {
  ...
  if (scenario.assert?.camera !== void 0) {
  ...
  const assertionResults = [...evaluated.assertions, ...cameraResult === void 0 ? [] : [cameraResult]];
...
function failureReport(config, scenario, diagnostic) {
  return {
    artifactDirectory: config.artifactDirectory,
...
function setupRequest(scenario) {
  return {
    entities: scenario.setup?.entities?.map(({ entity, position, rotation, scale }) => ({
      entity,
      transform: { position, rotation, scale }
    })),
    resources: scenario.setup?.resources?.map(({ id, path, value }) => ({ id, path, value }))
  };
}
async function runStep(page, bridge, step) {
  if (step.pointerPosition !== void 0) {
    await page.mouse.move(step.pointerPosition.x, step.pointerPosition.y);
  }
  if (step.press !== void 0) {
  ...
  if (step.press !== void 0 && step.release) {
    await page.keyboard.up(step.press);
  }
}
async function waitFrames(page, frames) {
  if (frames <= 0) return;
  await page.evaluate((count) => new Promise((resolveFrame) => {
...
function observedEntityIds(scenario) {
  return [...new Set([
    scenario.subject,
...
function observedResourceIds(scenario) {
  return [.../* @__PURE__ */ new Set([
    ...(scenario.assert?.resources ?? []).map(({ id }) => id),
    ...(scenario.setup?.resources ?? []).map(({ id }) => id)
  ])];
}
function entityPosition(snapshot, id) {
  return snapshot?.entities?.find((entity) => entity.id === id)?.transform?.position;
}
function entityRotation(snapshot, id) {
  return snapshot?.entities?.find((entity) => entity.id === id)?.transform?.rotation;
}
function resourceObservations(before, after) {
  const ids = /* @__PURE__ */ new Set([...Object.keys(before?.resources ?? {}), ...Object.keys(after?.resources ?? {})]);
  return Object.fromEntries([...ids].map((id) => [id, { before: before?.resources?.[id], after: after?.resources?.[id] }]));
}
function normalizedRuntimeDiagnostics(snapshot, scenario) {
  return {
    recentRuntimeErrors: snapshot?.diagnostics ?? [],
    scene: {
      renderedEntities: (snapshot?.entities ?? []).map((entity) => ({
        id: entity.id,
        projectedBounds: entity.bounds === void 0 ? void 0 : pixelBoundsToNdc(entity.bounds, scenario.viewport)
      }))
    }
  };
}
function pixelBoundsToNdc(bounds, viewport) {
  return {
...
function cameraReport(scenario, before, after) {
  ...
  return {
    ...afterCamera === void 0 ? {} : { after: { frame: scenario.steps.length, position: afterCamera, tick: after?.clock.tick ?? 0 } },
    ...beforeCamera === void 0 ? {} : { before: { frame: 0, position: beforeCamera, tick: before?.clock.tick ?? 0 } },
    entity: cameraId,
    separation: afterCamera === void 0 || target === void 0 ? void 0 : length(subtract(afterCamera, target)),
    within: assertion?.within ?? Number.POSITIVE_INFINITY
  };
}
function evaluateCamera(scenario, snapshot) {
  const assertion = scenario.assert?.camera;
  ...
  const separation = camera === void 0 || target === void 0 ? void 0 : length(subtract(camera, target));
  return {
    details: { camera: cameraId, separation, target: targetId, visible: targetEntity?.visible },
    id: "camera",
    pass: (assertion.within === void 0 || separation !== void 0 && separation <= assertion.within) && (assertion.targetInViewport !== true || targetEntity?.visible === true)
  };
}
...
async function waitForUrl(url, timeoutMs, server) {
...
function stopManagedServer(server) {
  if (server?.pid === void 0 || server.exitCode !== null) return;
  if (process.platform === "win32") server.kill();
  else process.kill(-server.pid, "SIGTERM");
}
function managedServerError(message, url, timeoutMs, output) {
  return new ManagedServerError(playtestDiagnostic(
    "TN_PLAYTEST_SERVER_FAILED",
    `${message} URL: ${url}. Timeout: ${timeoutMs}ms. Output: ${output.join("").slice(-4e3)}`,
    "Run the server command directly, fix its first error, then rerun the same playtest command."
  ));
}

export { PlaytestBridgeError, connectPlaytestBridge, initStandalonePlaytest, parseStandalonePlaytestArgs, runStandalonePlaytest };
