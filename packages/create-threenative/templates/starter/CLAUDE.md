...
When a frame costs more than it should, find the object before changing anything: press backtick, open the overlay's **Geometry** tab and press **Capture**, which ranks the objects that submitted this frame's triangles beside their projected size on screen. An agent gets the same report by asserting `geometry` in a scenario (`"assert": { "geometry": { "limit": 25 } }`, requiring `runtime.geometry`). The numbers are measured submissions reconciled against the frame's own pass totals, so what the rows do not account for is reported as an unattributed remainder, and anything that cannot be measured — a packed batch's per-member draws, bounds that are unknown — says so with a reason instead of reading as zero.
