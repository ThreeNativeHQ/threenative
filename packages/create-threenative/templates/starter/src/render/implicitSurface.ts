...
function createSurfaceFunctions() {
  ...
  // biome-ignore format: compact generated helper keeps the render source under its smell budget.
  function auditImplicitSurface(indices: Uint32Array, positions: Float32Array) {
  ...
  function buildImplicitSurface(options: SurfaceOptions) {
    ...
    let report = auditImplicitSurface(outputIndices, outputPositions);
    if (report.signedVolume < 0) {
      ...
      report = auditImplicitSurface(outputIndices, outputPositions);
    }
  ...
  return { auditImplicitSurface, buildImplicitSurface };
}
export const { auditImplicitSurface, buildImplicitSurface } = createSurfaceFunctions();
...
export function createImplicitSurfaceWorkerSource(): string {
  ...
  return `const { auditImplicitSurface, buildImplicitSurface } = (${createSurfaceFunctions.toString()})();`;
}
