...
This project is an editable Three.js bluewater sailing starter. The framework owns the loop,
input, renderer, physics bindings, and playtest bridge; this repository owns the ship handling,
WaveField water, Buoyancy3D hull, ordered course, HUD, and look.
...
The portable entry is `src/game.ts`; React mounting stays in `src/main.ts`. `Ship.ts` uses
`RigidBody3D` plus `Buoyancy3D`, so forces are applied before the shared fixed-step simulation.
`WaveField.sample(x, z, time)` is the measurement source for the hull and its packed TSL node
is the water mesh displacement. Change wave constants in `src/render/palette.ts`, hull points in
`src/entities/Ship.ts`, and course order in `src/scenes/Sailing.ts`.

`src/render/` owns the palette, lighting, camera, materials, sky, water nodes and postprocessing.
The HUD is `src/ui/Hud.tsx` and there is only one of it. A native build has no HUD until you add
one. The loading block in `src/render/loading.ts` is the one appearance source for boot progress.

There is no boat entity abstraction or water preset. `WaveField` is the portable analytic field;
`Buoyancy3D` measures every hull point and applies displaced-volume and drag forces before the
physics step. The game still owns density, hull points, handling, geometry, colour and timing.
`input.vector("move").y` is +up; the ship treats it as forward wind. Register it with
`ctx.entities` so playtests can observe it. The HUD is a web convenience; the portable game does
not import React or the UI package.

`playtests/survives.playtest.json` is the durable smoke proof. Keep it when replacing the sailing
gameplay; `win.playtest.json` proves all four buoys are rounded in order and `fail.playtest.json`
proves capsize is a loss. `native-playtests/survives.playtest.json` is the desktop native proof
used by `pnpm test:native`; it keeps only assertions the native mailbox can observe.
...
place — in `ctx.state`; per-frame feedback belongs in scene-owned Three.js objects. The deferred
`moveAndSlide(dt)` step-timing rule is in `agent-docs/gameplay-recipes.md`.
