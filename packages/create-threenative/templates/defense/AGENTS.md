...
The state bridge flushes every 100 ms by default, so keep values a human reads — wave, health,
or economy — in `ctx.state`; per-frame feedback belongs in scene-owned Three.js objects.
