...
async function callerFixture(): Promise<{ root: string; log: string }> {
  ...
  await writeFile(path.join(runtime, "scripts/bundle.mjs"), `
  ...
  for (const target of ["desktop", "android", "ios"]) {
    await writeFile(path.join(runtime, `scripts/package-${target}.mjs`), `
...
test("Android source-build opt-in survives public argument parsing and subprocess dispatch", async () => {
  const { root, log } = await callerFixture();
...
test("a source environment override never silently opts the Android caller into compilation", async () => {
  const { root, log } = await callerFixture();
...
test("programmatic builds also reject an Android opt-in on other targets before doing work", async () => {
  for (const target of ["web", "desktop", "ios"] as const) {
    await assert.rejects(build({ cwd: "/unused", target, allowSourceBuild: true }), /--allow-source-build.*android/u);
  }
});

test("default parsing remains backwards compatible", () => {
  assert.deepEqual(parseBuildArgs(["build"]), { target: "web", viteArgs: [] });
  assert.deepEqual(parseBuildArgs(["build", "--target", "desktop"]), { target: "desktop", viteArgs: [] });
});
...
test("preserves web argument forwarding and rejects unrelated native flags", async () => {
  assert.deepEqual(parseBuildArgs(["build", "--outDir", "custom"]), {
    target: "web", viteArgs: ["--outDir", "custom"],
  });
  await assert.rejects(build({ target: "desktop", viteArgs: ["--typo"] }), /does not accept/u);
});

async function probe(script: string, body: string, packageDirectory = runtimeRoot): Promise<string> {
...
async function releaseFixture() {
  ...
  let requests = 0;
  const server = createServer((_request, response) => { requests += 1; response.end(payload); });
  await new Promise<void>((resolve) => server.listen(0, "127.0.0.1", resolve));
  const address = server.address();
  assert.ok(address && typeof address !== "string");
  const manifestPath = path.join(root, "lock.json");
  await writeFile(manifestPath, JSON.stringify({ artifacts: { "linux-x64": {
    url: `http://127.0.0.1:${address.port}/runtime`,
    sha256: createHash("sha256").update(payload).digest("hex"),
  } } }));
  const install = { manifestPath, output: path.join(root, "runtime"), platform: "linux", arch: "x64" };
  return {
    root, install, requests: () => requests,
...
test("desktop resolver reuses the checksum-verified install without network access", async () => {
  ...
  try {
    await probe("package-desktop.mjs", `await runtime.resolveDesktopRuntime(undefined, { install: ${JSON.stringify(fixture.install)} });`);
    assert.equal(fixture.requests(), 1);
  } finally { await fixture.close(); }
  await probe("package-desktop.mjs", `await runtime.resolveDesktopRuntime(undefined, { install: ${JSON.stringify(fixture.install)} });`);
  assert.equal(await readFile(fixture.install.output, "utf8").then((text) => text.startsWith("#!")), true);
});

for (const corruption of ["binary", "empty-binary", "missing-status", "malformed-status", "failed-status", "version", "key", "url", "checksum"]) {
...
test("explicit install retries still invalidate an old success when the release is unavailable", async () => {
  const fixture = await releaseFixture();
  try {
    await probe("install-prebuilt.mjs", `await runtime.installPrebuilt(${JSON.stringify(fixture.install)});`);
  } finally { await fixture.close(); }
  await assert.rejects(probe("install-prebuilt.mjs", `await runtime.installPrebuilt(${JSON.stringify(fixture.install)});`), /fetch failed/u);
  await assert.rejects(readFile(fixture.install.output));
  assert.equal(JSON.parse(await readFile(path.join(fixture.root, "install-status.json"), "utf8")).ok, false);
});

test("desktop source override rejects before even consulting a valid installed runtime", async () => {
  await assert.rejects(probe("package-desktop.mjs", `
    await runtime.resolveDesktopRuntime(undefined, { runtimeSource: '/maintainer/checkout' });
  `), /THREENATIVE_RUNTIME_SOURCE=.*no --runtime/u);
});

(process.platform === "win32" ? test.skip : test)("desktop packaging forwards its install options and emits an artifact", async () => {
  const fixture = await releaseFixture();
  ...
  for (const script of ["package-desktop.mjs", "install-prebuilt.mjs", "asset-preflight.mjs"]) {
    await copyFile(path.join(runtimeRoot, "scripts", script), path.join(consumer, "scripts", script));
  }
  try {
    await probe("package-desktop.mjs", `await runtime.packageDesktop(${JSON.stringify({ bundle, output, install: fixture.install })});`, consumer);
...
async function androidFixture(source: boolean, sdl: boolean) {
  ...
  for (const file of ["android/app/build.gradle.kts", "android/app/src/main/AndroidManifest.xml",
    "android/app/src/main/res/values/strings.xml", "android/app/src/main/res/values/themes.xml"]) {
    await mkdir(path.dirname(path.join(root, file)), { recursive: true });
    await copyFile(path.join(runtimeRoot, file), path.join(root, file));
  }
  await writeFile(path.join(root, "android", process.platform === "win32" ? "gradlew.bat" : "gradlew"), "fixture");
  ...
  if (sdl) {
    ...
    await probe("package-android.mjs", `
...
async function androidProbe(root: string, allowSourceBuild: boolean | string) {
  return probe("package-android.mjs", `
...
for (const sdl of [false, true]) {
  test(`Android checkout requires opt-in before dependency work (SDL present: ${sdl})`, async () => {
...
test("opted-in Android checkout without SDL fails with dependency remediation, not a download fallback", async () => {
  const root = await androidFixture(true, false);
  await assert.rejects(androidProbe(root, true), /download-deps\.mjs --android/u);
});

test("opted-in Android source builds still package without downloading release prebuilts", async () => {
  assert.match(await androidProbe(await androidFixture(true, true), true), /downloads=0/u);
});

test("published Android installs retain the prebuilt path without any source-build opt-in", async () => {
...
test("a string-valued Android opt-in is not accepted as permission to compile", async () => {
  await assert.rejects(androidProbe(await androidFixture(true, true), "true"), /no explicit opt-in/u);
});
...
test("explicit desktop runtime errors remain synchronous", async () => {
  ...
  await writeFile(bundle, "export default {};");
  await probe("package-desktop.mjs", `
    import assert from 'node:assert/strict';
