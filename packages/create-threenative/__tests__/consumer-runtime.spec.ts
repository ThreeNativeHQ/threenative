...
const envKeys = ["THREENATIVE_RUNTIME_SOURCE", "THREENATIVE_PREBUILT_MANIFEST", "THREENATIVE_ALLOW_INSECURE_PREBUILT"];
...
async function installedFixture() {
  ...
  const install = { output: path.join(root, "runtime"), statusPath: path.join(root, "status.json"), manifestPath };
...
test("desktop reuses a checksum-verified install when the release is unavailable", async () => {
  ...
  assert.equal(await desktop.resolveDesktopRuntime(undefined, { install: fixture.install }), fixture.install.output);
...
for (const mutation of ["bytes", "status-missing", "status-json", "ok", "key", "version", "url", "sha256"]) {
...
test("desktop refuses an unverified cached runtime when reinstall fails", async () => {
  ...
  await new Promise<void>((resolve) => fixture.server.close(() => resolve()));
  await assert.rejects(desktop.resolveDesktopRuntime(undefined, { install: fixture.install }), /fetch failed/u);
...
test("desktop source override is rejected even when a verified consumer runtime is cached", async () => {
  ...
  await assert.rejects(desktop.resolveDesktopRuntime(undefined, { install: fixture.install }), /THREENATIVE_RUNTIME_SOURCE.*no --runtime/u);
...
test("desktop explicit runtime override remains offline and takes precedence over source preflight", async () => {
  ...
  await assert.rejects(desktop.resolveDesktopRuntime(path.join(root, "missing")), /Missing prebuilt runtime/u);
});
...
test("desktop explicit packaging still reports a missing bundle synchronously", () => {
  const root = temporary();
  assert.throws(() => desktop.packageDesktop({ bundle: path.join(root, "absent"), runtime: path.join(root, "runtime"), output: path.join(root, "game") }), /Missing native bundle/u);
});
...
for (const aar of [undefined, "SDL3-0.0.0.aar", `SDL3-${android.SDL3_ANDROID_VERSION}.aar`]) {
  test(`Android refuses a source checkout before wrapper/network work (${aar ?? "no SDL archive"})`, async () => {
    ...
    const calls: string[] = [];
    await assert.rejects(android.packageAndroid("absent.js", undefined, undefined, undefined, undefined, {
      runtimeRoot: root,
      ensureGradleWrapper: async () => { calls.push("wrapper"); },
      prepareAndroidPrebuilts: async () => { calls.push("download"); },
      spawnSync: () => { calls.push("compile"); },
...
function androidFixture(source: boolean) {
  ...
  writeFileSync(path.join(root, "android", process.platform === "win32" ? "gradlew.bat" : "gradlew"), "fixture");
  if (source) writeFileSync(path.join(root, "CMakeLists.txt"), "# source\n");
...
test("Android explicit opt-in uses the source route without downloading prebuilts", async () => {
  ...
  assert.equal(await android.packageAndroid(fixture.bundle, output, undefined, undefined, undefined, {
    ...fixture.options, allowSourceBuild: true,
  }), output);
...
test("Android installed consumers still download prebuilts without opting into source builds", async () => {
  ...
  assert.equal(await android.packageAndroid(fixture.bundle, undefined, undefined, undefined, undefined, fixture.options), fixture.apk);
...
test("Android download failure names the usable public CLI source-build opt-in", async () => {
  const fixture = androidFixture(false);
  await assert.rejects(android.packageAndroid(fixture.bundle, undefined, undefined, undefined, undefined, {
    ...fixture.options,
    prepareAndroidPrebuilts: async () => { throw new Error("HTTP 404"); },
...
test("desktop cache cannot override an explicit manifest changed in place", async () => {
  ...
  await assert.rejects(desktop.resolveDesktopRuntime(undefined, { install: fixture.install }), /Checksum verification failed/u);
  assert.equal(fixture.requests(), 2);
  assert.equal(existsSync(fixture.install.output), false);
});
...
(process.platform === "win32" ? test.skip : test)("desktop package CLI cold-installs once and rebuilds offline", async () => {
  const root = temporary();
  ...
  writeFileSync(manifestPath, JSON.stringify({ artifacts: {
    [`${process.platform}-${process.arch}`]: {
      ...
      sha256: installer.sha256(payload), size: payload.length,
    },
  } }));
  const bundle = path.join(root, "game.js");
  const output = path.join(root, "game");
  writeFileSync(bundle, "export default {};\n");
  const args = [path.join(consumer, "scripts", "package-desktop.mjs"), "--bundle", bundle, "--output", output];
  const env = { ...process.env, THREENATIVE_PREBUILT_MANIFEST: manifestPath, THREENATIVE_ALLOW_INSECURE_PREBUILT: "1" };
  await promisify(execFile)(process.execPath, args, { env });
  assert.equal(readFileSync(output, "utf8"), "compiled-fixture");
  assert.equal(JSON.parse(readFileSync(path.join(consumer, "prebuilt", "install-status.json"), "utf8")).ok, true);
  rmSync(output);
  await new Promise<void>((resolve) => server.close(() => resolve()));
  await promisify(execFile)(process.execPath, args, { env });
  assert.equal(readFileSync(output, "utf8"), "compiled-fixture");
