...
const roots: string[] = [];
const savedBinary = process.env.THREENATIVE_RUNTIME_BINARY;
const savedSource = process.env.THREENATIVE_RUNTIME_SOURCE;

afterEach(async () => {
  await Promise.all(roots.splice(0).map((root) => rm(root, { force: true, recursive: true })));
  for (const [key, value] of [
    ["THREENATIVE_RUNTIME_BINARY", savedBinary],
    ["THREENATIVE_RUNTIME_SOURCE", savedSource],
  ]) {
...
async function gameFixture() {
  ...
  roots.push(root);
  delete process.env.THREENATIVE_RUNTIME_BINARY;
  delete process.env.THREENATIVE_RUNTIME_SOURCE;
  await mkdir(path.join(root, "src"), { recursive: true });
  await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "consumer-game", type: "module" }));
  ...
  await writeFile(path.join(runtime, "package.json"), JSON.stringify({ name: "@threenative/runtime-native", type: "module" }));
...
async function outputArgs(root: string, suffix = "") {
  return JSON.parse(await readFile(path.join(root, "dist-native", `consumer-game${suffix}`), "utf8")) as string[];
}
...
test("Android public CLI parses and forwards explicit source-build opt-in", async () => {
  ...
  assert.equal((await outputArgs(root, ".apk")).filter((arg) => arg === "--allow-source-build").length, 1);
});
...
for (const target of ["web", "desktop", "ios"] as const) {
  test(`source-build flag cannot silently change a ${target} build`, async () => {
    assert.throws(() => parseBuildArgs(["build", "--target", target, "--allow-source-build"]), /--allow-source-build.*android/u);
    await assert.rejects(build({ cwd: "/does-not-exist", target, allowSourceBuild: true }), /--allow-source-build.*android/u);
  });
}

test("source-build opt-in does not swallow other invalid native flags", async () => {
  const options = parseBuildArgs(["build", "--target", "android", "--allow-source-build", "--unexpected"]);
...
test("ordinary web argument forwarding and default parse result remain unchanged", () => {
  ...
  assert.deepEqual(parseBuildArgs(["build", "--outDir", "web-dist"]), { target: "web", viteArgs: ["--outDir", "web-dist"] });
});
