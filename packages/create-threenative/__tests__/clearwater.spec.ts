...
import {
  beerLambert,
  dielectricFresnel,
} from "../template-assets/clearwater/src/render/clearwaterOptics.js";
import {
  type IClearwaterOptions,
  resolveClearwaterOptions,
} from "../template-assets/clearwater/src/render/clearwaterOptions.js";
...
class Scalar {
  ...
  read(x: Scalar | number): number {
    return x instanceof Scalar ? x.value : x;
  }
  add(x: Scalar | number): Scalar {
    return new Scalar(this.value + this.read(x));
  }
  sub(x: Scalar | number): Scalar {
    return new Scalar(this.value - this.read(x));
  }
  mul(x: Scalar | number): Scalar {
    return new Scalar(this.value * this.read(x));
  }
  div(x: Scalar | number): Scalar {
    return new Scalar(this.value / this.read(x));
  }
  max(x: Scalar | number): Scalar {
    return new Scalar(Math.max(this.value, this.read(x)));
  }
  clamp(a: number, b: number): Scalar {
    return new Scalar(Math.max(a, Math.min(b, this.value)));
  }
  sqrt(): Scalar {
...
const invalid: IClearwaterOptions[] = [
  ...
  { absorption: [-1, 0, 0] },
  { ior: 1 },
  { causticsResolution: 16384 },
  { causticsSegments: 1024 },
  { reflectionScale: 2 },
];
...
test("Fresnel is exact at normal incidence and grazing", () => {
  const ior = 1.3335;
  assert.ok(
    Math.abs(dielectricFresnel(new Scalar(1), ior).value - ((ior - 1) / (ior + 1)) ** 2) < 1e-14,
  );
...
test("auxiliary pass restores target, mip, MRT, clear and XR state after failure", () => {
  const state = {
    target: "main" as string | null,
    face: 2,
    mip: 3,
    mrt: "velocity" as string | null,
    autoClear: true,
    color: "blue",
    alpha: 0.4,
    xr: { enabled: true },
  ...
  const renderer = {
    ...
    getMRT: () => state.mrt,
    setMRT: (value: string | null) => {
      state.mrt = value;
    },
    getClearColor: (out: { value: string }) => {
      out.value = state.color;
      return out;
    },
    getClearAlpha: () => state.alpha,
    setClearColor: (color: { value: string }, alpha: number) => {
  ...
  assert.throws(
    () =>
      withClearwaterTarget(renderer, "caustics", { value: "black" }, { value: "" }, () => {
