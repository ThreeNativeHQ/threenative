...
it("installs source and license, then refuses to overwrite an edited game", async () => {
  ...
  try {
    ...
    expect(await readFile(path.join(target, "src/clearwater.ts"), "utf8")).toContain("createClearwater");
    expect(await readFile(path.join(target, "src/render/CLEARWATER-LICENSE.txt"), "utf8")).toContain("Copyright (c) 2026 Lumaris");
    await writeFile(path.join(target, "src/clearwater.ts"), "// game-owned edits\n");
    await expect(run(process.execPath, [path.join(bundle, "install.mjs"), target])).rejects.toThrow("Refusing to overwrite");
    expect(await readFile(path.join(target, "src/clearwater.ts"), "utf8")).toBe("// game-owned edits\n");
