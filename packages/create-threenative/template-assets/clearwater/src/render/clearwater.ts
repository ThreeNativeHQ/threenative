...
export function createClearwaterAppearance(
  ocean: IClearwaterOceanSource,
  ripples: IClearwaterRippleSource,
  surface: IClearwaterSurfaceSource,
  ...
  try {
    ...
    const rippleTexture = new DataTexture(
      pixels,
      ripples.resolution,
      ripples.resolution,
      RGBAFormat,
    ...
    function rippleAt(xz: Node<"vec2">): Node<"vec4"> {
      ...
      const sampleUV = uv
        .mul((ripples.resolution - 1) / ripples.resolution)
        .add(0.5 / ripples.resolution);
    ...
    function waveHeight(xz: Node<"vec2">): Node<"float"> {
      ...
      for (let index = 0; index < ocean.cascades.length; index += 1) {
        ...
        const read = (x: Node<"float">, z: Node<"float">) =>
          buffer.element(wrap(z).mul(grid).add(wrap(x)).toUint()).y;
    ...
    const field = {
      heightAt(xz: Node<"vec2">): Node<"float"> {
        return waveHeight(xz).add(rippleAt(xz).x);
      },
      normalAt(xz: Node<"vec2">): Node<"vec3"> {
        const ripple = rippleAt(xz);
        const dx = waveHeight(xz.add(vec2(step, 0)))
          .sub(waveHeight(xz.sub(vec2(step, 0))))
          .div(2 * step)
          .add(ripple.y);
        const dz = waveHeight(xz.add(vec2(0, step)))
          .sub(waveHeight(xz.sub(vec2(0, step))))
          .div(2 * step)
          .add(ripple.z);
    ...
    const caustics = options.caustics
      ? createClearwaterCaustics(field, level, sunDirection, options)
      : undefined;
    ...
    material.positionNode = vec3(
      positionLocal.x,
      level.add(field.heightAt(positionLocal.xz)),
      positionLocal.z,
    );
    ...
    const offset = select(
      surface.thicknessAt(proposedOffset).greaterThan(0),
      proposedOffset,
      vec2(0),
    );
    ...
    const pathLength = receiver
      .sub(positionWorld)
      .length()
      .min(options.depth * 12);
    ...
    if (caustics !== undefined) {
      ...
      const depthConfidence = float(1)
        .sub(verticalDepth.sub(options.depth).abs().div(options.depth))
        .clamp(0, 1);
      floorLight = floorLight.mul(
        mix(vec3(1), caustics.at(receiver.xz), depthConfidence.mul(options.causticsStrength)),
      );
    }
    const singleScatter = vec3(
      ...
      .mul(transmittance.oneMinus())
      .mul(vec3(...options.sunColor))
      .mul(0.08);
    ...
    const reflected = options.reflection
      ? surface.reflectionAt(normalView.xy.mul(options.distortion))
      : vec3(0.11, 0.27, 0.62);
    ...
    const distribution = tan2
      .negate()
      .div(a2)
      .exp()
      .div(a2.mul(Math.PI).mul(cos2.mul(cos2)));
    const visibility = float(0.5).div(
      nl
        .mul(facing.mul(facing).mul(a2.oneMinus()).add(a2).sqrt())
        .add(facing.mul(nl.mul(nl).mul(a2.oneMinus()).add(a2).sqrt()))
        .add(0.00001),
    );
    const specular = distribution
      .mul(visibility)
      .mul(dielectricFresnel(half.dot(view), options.ior))
      .mul(nl)
      .min(12000);
    material.colorNode = mix(under, reflected, fresnel).add(
      vec3(...options.sunColor).mul(specular),
    );
