...
export function attributeLaunchStall(text, { minShare = 0.8, label = 'run' } = {}) {
  ...
  const total = frame.toFirstFrameMs;
  ...
  const lines = [];
  lines.push(`| phase | cost | calls | share of launch |`);
  lines.push(`| --- | --- | --- | --- |`);
  ...
  lines.push(
    `| **total to first frame** | ${formatMs(total)} | — | 100 % |`,
  );

  const report = {
    label,
    toFirstFrameMs: total,
  ...
  if (!(share >= minShare)) {
    const error = new Error(
      `TN_STALL_ATTRIBUTION_UNDER_MIN: named segments account for ${(share * 100).toFixed(1)} % of ` +
        `the ${formatMs(total)} launch, below the required ${(minShare * 100).toFixed(0)} %. ` +
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    console.log('');
    console.log(
      `attributed ${(report.attributedShare * 100).toFixed(1)} % of ${formatMs(report.toFirstFrameMs)} ` +
        `(boot enterTotal ${report.bootEnterTotalMs ?? '?'} ms, hitch gap ${report.hitchGapMs ?? '?'} ms)`,
    );
  } catch (error) {
