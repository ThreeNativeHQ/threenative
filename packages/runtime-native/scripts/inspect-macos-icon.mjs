...
function iconTool(run, command, args) {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(
      `${INVALID}: ${command} could not inspect the icon (${result.error?.message ?? result.status ?? 'no exit status'}). Run the brand verifier on macOS with iconutil and sips available; metadata alone is not icon proof.`,
    );
  }
}
...
// Compare at the same conversion boundary. iconutil can normalize even visible colour values
// while encoding/decoding ICNS. Resized source PNG pixels alone are therefore not its output.
// Build the reference independently from app.icon with the packager's existing recipe, then
// decode both ICNS files. Exact visible RGBA comparison remains unchanged; no tolerance is added.
function referenceIconset(source, scratch, run) {
  const inputs = join(scratch, 'reference.iconset');
  mkdirSync(inputs);
  for (const name of GENERATED_NAMES) {
    const match = /^icon_(\d+)x\1(@2x)?\.png$/u.exec(name);
    const pixels = String(Number(match[1]) * (match[2] ? 2 : 1));
    const target = join(inputs, name);
    iconTool(run, 'sips', ['-z', pixels, pixels, source, '--out', target]);
    pngImage(target, Number(pixels));
  }
  const reference = join(scratch, 'reference.icns');
  iconTool(run, 'iconutil', ['-c', 'icns', inputs, '-o', reference]);
  assertIcns(readFileSync(reference));
  const decoded = join(scratch, 'reference-decoded.iconset');
  iconTool(run, 'iconutil', ['-c', 'iconset', '-o', decoded, reference]);
  if (!lstatSync(decoded).isDirectory() || lstatSync(decoded).isSymbolicLink()) {
    throw new Error(`${INVALID}: iconutil did not produce a reference iconset directory.`);
  }
  return decoded;
}
 ...
 * iconutil round trip is independently repeated for the authored source, then compared at decoded
 * RGBA. Compression/metadata differences cannot pass for artwork or reject identical artwork. Authored .icns inputs must remain byte-identical and still
...
export function inspectMacosIcon(icon, source, { run = spawnSync } = {}) {
  ...
  try {
    ...
    const references = authoredIcns ? undefined : referenceIconset(source, scratch, run);
    ...
    for (const entry of entries) {
      ...
      if (references !== undefined) {
        const reference = join(references, entry.name);
        const info = lstatSync(reference);
        if (!info.isFile() || info.isSymbolicLink()) {
          throw new Error(`${INVALID}: unexpected reference output ${entry.name}.`);
        }
