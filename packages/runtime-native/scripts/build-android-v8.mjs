...
import { ANDROID_16KB_ABIS, assertAndroid16KbAlignment } from './check-android-16kb-alignment.mjs';
...
export function androidV8GnArgs(abi) {
  if (!Object.hasOwn(TARGETS, abi)) throw new Error(`Unsupported Android V8 ABI: ${abi}`);
  return [
...
function payloadFiles(root, directory = '') {
  ...
  for (const entry of readdirSync(join(root, directory)).sort()) {
    ...
    if (stats.isSymbolicLink()) throw new Error(`V8 payload must not contain symlinks: ${relative}`);
    if (stats.isDirectory()) result.push(...payloadFiles(root, relative));
    else if (stats.isFile() && stats.size > 0) result.push(relative);
    else throw new Error(`V8 payload has an empty or non-regular file: ${relative}`);
  }
  return result;
}
...
export function createAndroidV8Receipt(root, options = {}) {
  ...
  const required = ['include/v8.h', 'include/v8-version.h'];
  for (const abi of ANDROID_16KB_ABIS) {
    required.push(`lib/${abi}/libv8android.so`, `lib/${abi}/libc++_shared.so`,
      `snapshot_blob/${abi}/snapshot_blob.bin`);
  }
  for (const path of required) {
    if (!files.includes(path)) throw new Error(`Android V8 is missing ${path}`);
  }
  const versionHeader = readFileSync(join(root, 'include/v8-version.h'), 'utf8');
  const parts = ['MAJOR_VERSION', 'MINOR_VERSION', 'BUILD_NUMBER', 'PATCH_LEVEL'].map((part) =>
    new RegExp(`^#define V8_${part} (\\d+)\\s*$`, 'mu').exec(versionHeader)?.[1]);
  if (parts.join('.') !== ANDROID_V8_BUILD.version) {
    throw new Error(`Android V8 headers do not match ${ANDROID_V8_BUILD.version}`);
  }
  const libraries = files.filter((path) => path.endsWith('.so'));
  for (const path of libraries) {
    const [, abi] = path.split('/');
    const bytes = readFileSync(join(root, path));
    if (!path.startsWith('lib/') || !Object.hasOwn(TARGETS, abi) || bytes.length < 64 ||
        !bytes.subarray(0, 7).equals(Buffer.from([0x7f, 0x45, 0x4c, 0x46, 2, 1, 1])) ||
        bytes.readUInt16LE(16) !== 3 || bytes.readUInt16LE(18) !== TARGETS[abi].machine) {
      throw new Error(`Android V8 ${path} has an invalid ELF class/type/machine for its ABI`);
    }
  }
  const inspected = assertAndroid16KbAlignment(libraries.map((path) => join(root, path)), options);
  return {
    schemaVersion: 1,
    build: { ...ANDROID_V8_BUILD },
    args: Object.fromEntries(ANDROID_16KB_ABIS.map((abi) => [abi, androidV8GnArgs(abi)])),
    files: Object.fromEntries(files.map((path) => [path, sha256(join(root, path))])),
    libraries: inspected.map(({ alignments }, index) => ({ path: libraries[index], alignments })),
  };
}

export function verifyAndroidV8Installation(root, options = {}) {
  const path = join(root, RECEIPT);
  if (!existsSync(path)) throw new Error(`Android V8 build receipt is missing: ${path}`);
  const receipt = JSON.parse(readFileSync(path, 'utf8'));
  const args = Object.fromEntries(ANDROID_16KB_ABIS.map((abi) => [abi, androidV8GnArgs(abi)]));
  if (receipt.schemaVersion !== 1 || JSON.stringify(receipt.build) !== JSON.stringify(ANDROID_V8_BUILD) ||
      JSON.stringify(receipt.args) !== JSON.stringify(args)) {
    throw new Error('Android V8 cache has a different build recipe; rebuild the dependency');
  }
  const actual = createAndroidV8Receipt(root, options);
  if (!receipt.files || Object.keys(receipt.files).length !== Object.keys(actual.files).length) {
    throw new Error('Android V8 receipt does not cover the complete payload');
  }
  for (const [file, hash] of Object.entries(actual.files)) {
    if (receipt.files[file] !== hash) throw new Error(`Android V8 ${file} checksum mismatch`);
  }
  return actual;
}
...
export function resolveAndroidV8Ndk(env = process.env) {
  ...
  const root = env.ANDROID_NDK_HOME ?? env.ANDROID_NDK_ROOT ?? env.ANDROID_NDK ??
    (sdk ? join(sdk, 'ndk', ANDROID_V8_BUILD.ndk) : undefined);
  const properties = root && join(root, 'source.properties');
  if (!properties || !existsSync(properties) ||
      !readFileSync(properties, 'utf8').includes(`Pkg.Revision = ${ANDROID_V8_BUILD.ndk}`)) {
    throw new Error(`Android V8 requires NDK ${ANDROID_V8_BUILD.ndk}; install it with sdkmanager ` +
...
function replaceOnce(path, before, after) {
  ...
  if (text.split(before).length !== 2) throw new Error(`Pinned V8 patch context changed in ${path}`);
  writeFileSync(path, text.replace(before, after));
}

/** Linux x86_64 source-build lane. Consumers continue to use engine-qualified runtime assets. */
export function provisionAndroidV8(destination, { force = false, env = process.env } = {}) {
  if (!force && existsSync(destination)) {
    try {
      const receipt = verifyAndroidV8Installation(destination);
      console.log(`Verified Android V8 ${receipt.build.version}: both ABI/snapshot/STL payloads`);
      return receipt;
    } catch (error) {
  ...
  if (process.platform !== 'linux' || process.arch !== 'x64') {
    throw new Error('Rebuilding Android V8 requires a Linux x86_64 source-build host. ' +
  ...
  if (!Number.isInteger(jobs) || jobs < 1 || jobs > 32) {
    throw new Error('THREENATIVE_V8_BUILD_JOBS must be an integer between 1 and 32');
  }
  ...
  const checkout = (directory, url, revision) => {
    ...
    if (actual !== revision) throw new Error(`V8 source revision mismatch: ${actual} != ${revision}`);
  };
  try {
    console.log(`Building Android V8 in ${work}`);
    checkout(upstream, 'https://github.com/Kudo/v8-android-buildscripts.git', ANDROID_V8_BUILD.patches);
    checkout(depot, 'https://chromium.googlesource.com/chromium/tools/depot_tools.git', ANDROID_V8_BUILD.depotTools);
    const depotEnv = { ...baseEnv, PATH: `${depot}:${baseEnv.PATH ?? ''}` };
    run(join(depot, 'gclient'), ['config', '--name', 'v8', '--unmanaged',
      'https://chromium.googlesource.com/v8/v8.git'], upstream, depotEnv);
    run(join(depot, 'gclient'), ['sync', '--revision', `v8@${ANDROID_V8_BUILD.source}`,
      '--deps=android', '--no-history', '--nohooks'], upstream, depotEnv);
    run(join(depot, 'gclient'), ['runhooks'], upstream, depotEnv);
    symlinkSync(ndk, join(source, 'android-ndk-r28c'), 'dir');
    replaceOnce(join(upstream, 'scripts/env.sh'), 'NDK_VERSION="r23c"', 'NDK_VERSION="r28c"');
    run('bash', ['scripts/patch.sh', 'android']);
    const clangVersions = readdirSync(join(tools, 'lib/clang'));
    if (clangVersions.length !== 1) throw new Error('Pinned NDK has an ambiguous Clang resource version');
    replaceOnce(join(source, 'build/config/android/BUILD.gn'),
      '/clang/12.0.9/lib/linux/$arch_dir', `/clang/${clangVersions[0]}/lib/linux/$arch_dir`);
    replaceOnce(join(source, 'BUILD.gn'), 'v8_loadable_module("libv8android") {\n',
    ...
    replaceOnce(join(source, 'src/heap/cppgc/stats-collector.h'),
    ...
    for (const abi of ANDROID_16KB_ABIS) {
      ...
      run(join(source, 'buildtools/linux64/gn'), ['gen', output, `--args=${androidV8GnArgs(abi)}`], source);
      run('ninja', [`-j${jobs}`, '-C', output, 'libv8android', 'run_mksnapshot_default'], source);
      mkdirSync(join(stage, 'lib', abi), { recursive: true });
      mkdirSync(join(stage, 'snapshot_blob', abi), { recursive: true });
      copyFileSync(join(source, output, 'libv8android.so'), join(stage, 'lib', abi, 'libv8android.so'));
      copyFileSync(join(tools, 'sysroot/usr/lib', target.triple, 'libc++_shared.so'),
        join(stage, 'lib', abi, 'libc++_shared.so'));
      copyFileSync(join(source, output, 'snapshot_blob.bin'), join(stage, 'snapshot_blob', abi, 'snapshot_blob.bin'));
    }
    cpSync(join(source, 'include'), join(stage, 'include'), { recursive: true });
    mkdirSync(join(stage, 'licenses'), { recursive: true });
    copyFileSync(join(source, 'LICENSE'), join(stage, 'licenses/V8-LICENSE'));
    copyFileSync(join(upstream, 'LICENSE'), join(stage, 'licenses/BUILD-SCRIPTS-LICENSE'));
    copyFileSync(join(source, 'third_party/icu/LICENSE'), join(stage, 'licenses/ICU-LICENSE'));
    ...
    throw new Error(`Android V8 source build failed; previous install is unchanged. ` +
      `Inspect ${work}. ${error.message}`, { cause: error });
  }
}

if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    provisionAndroidV8(resolve(dirname(fileURLToPath(import.meta.url)), '../third_party/v8-android'), {
