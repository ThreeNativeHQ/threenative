...
/// `PictOpOver`, from `Xrender.h`. The one operator that honours a source window's alpha.
const PICT_OP_OVER: c_int = 3;
/// `CompositeRedirectManual`, from `composite.h` — the server stops drawing redirected windows
/// itself and hands their pixels to the compositor.
const COMPOSITE_REDIRECT_MANUAL: c_int = 1;

/// The compositing manager this process runs for itself when the display has none.
///
/// Nothing on X11 blends a top-level ARGB window by itself: the server copies the window's pixels
/// as they are, alpha included, so the transparent parts arrive opaque. Automatic redirection does
/// not change that — measured on Xvfb, an ARGB window under `CompositeRedirectAutomatic` reads
/// back identical to one with no redirection at all, and under `CompositeRedirectManual` with
/// nobody drawing it the root is black. What blends is manual redirection *plus* a loop that names
/// each redirected window's pixmap and draws it with `XRenderComposite`; that loop is this.
///
/// It exists for the headless lane. A session that already has a compositor returns before this is
/// ever built, so the user's desktop is never redirected. The whole thing runs on the game-loop
/// thread from `compose_frame`, called out of `tn_ui_overlay_pump`, so there is no second X client
/// and no `XInitThreads` question.
struct SelfCompositor {
    xlib: xlib::Xlib,
    composite: libloading::Library,
    render: libloading::Library,
    display: *mut xlib::Display,
    root: c_ulong,
    root_picture: c_ulong,
    /// The unmapped window holding `_NET_WM_CM_S<screen>`. Destroying it releases the selection.
    owner: c_ulong,
}

impl Drop for SelfCompositor {
    fn drop(&mut self) {
        unsafe {
            if let Ok(unredirect) = self
                .composite
                .get::<unsafe extern "C" fn(*mut xlib::Display, c_ulong, c_int)>(
                    b"XCompositeUnredirectSubwindows\0",
                )
            {
                unredirect(self.display, self.root, COMPOSITE_REDIRECT_MANUAL);
            }
            if let Ok(free_picture) = self
                .render
                .get::<unsafe extern "C" fn(*mut xlib::Display, c_ulong)>(b"XRenderFreePicture\0")
            {
                free_picture(self.display, self.root_picture);
            }
            (self.xlib.XDestroyWindow)(self.display, self.owner);
...
thread_local! {
    /// Main-thread only, like every other call into this crate; `RefCell` is what keeps it honest.
    static SELF_COMPOSITOR: RefCell<Option<SelfCompositor>> = const { RefCell::new(None) };
}

/// Guarantee a compositing manager, taking the role ourselves when the server can composite but
/// nobody has claimed the selection.
///
/// The attach guard is right on a user's machine — a transparent top-level window with nothing to
/// blend it is a black plate over the game. It is fatal in a headless test lane: `Xvfb` has the
/// Composite extension but ships no compositor, so the starter template's own
/// `native-playtests/react-hud.playtest.json` failed on a fresh project. This is the mechanism
/// in-process, so every consumer gets it from the runtime with no new dependency and no compiled
/// fixture to ship.
pub fn ensure_compositor() -> bool {
    if compositor_present() {
        return true;
    }
    SELF_COMPOSITOR.with(|cell| {
        let mut slot = cell.borrow_mut();
        if slot.is_none() {
            *slot = try_self_composite();
        }
        slot.is_some()
    })
}

/// Undo `ensure_compositor`. Called when an attach that redirected the display then failed, so a
/// half-attached overlay cannot leave the player's screen black; and on detach, which restores the
/// server's own drawing. A display that already had a compositor is left alone.
pub fn release_compositor() {
    SELF_COMPOSITOR.with(|cell| {
        cell.borrow_mut().take();
    });
}

/// Draw the current window stack onto the root, once a frame.
///
/// A no-op unless this process is the compositor, which is why the pump can call it unconditionally.
pub fn compose_frame() {
    SELF_COMPOSITOR.with(|cell| {
        if let Some(compositor) = cell.borrow().as_ref() {
            unsafe { compositor.draw_root() };
        } else if std::env::var("TN_UI_OVERLAY_TRACE").is_ok() {
            eprintln!("TN_COMPOSE: no self-compositor on this thread");
        }
    });
}

fn try_self_composite() -> Option<SelfCompositor> {
    let composite = unsafe { libloading::Library::new("libXcomposite.so.1") }.ok()?;
    let render = unsafe { libloading::Library::new("libXrender.so.1") }.ok()?;
    let xlib = xlib::Xlib::open().ok()?;
    unsafe {
        let query: libloading::Symbol<
            unsafe extern "C" fn(*mut xlib::Display, *mut c_int, *mut c_int) -> c_int,
        > = composite.get(b"XCompositeQueryExtension\0").ok()?;
        let redirect: libloading::Symbol<
            unsafe extern "C" fn(*mut xlib::Display, c_ulong, c_int),
        > = composite.get(b"XCompositeRedirectSubwindows\0").ok()?;
        let find_format: libloading::Symbol<
            unsafe extern "C" fn(*mut xlib::Display, *mut xlib::Visual) -> *mut c_void,
        > = render.get(b"XRenderFindVisualFormat\0").ok()?;
        let create_picture: libloading::Symbol<
            unsafe extern "C" fn(*mut xlib::Display, c_ulong, *mut c_void, c_ulong, *mut c_void) -> c_ulong,
        > = render.get(b"XRenderCreatePicture\0").ok()?;
        ...
        if query(display, &mut event_base, &mut error_base) == 0 {
            (xlib.XCloseDisplay)(display);
            return None;
        }
        let root = (xlib.XRootWindow)(display, screen);
        let mut root_attributes: xlib::XWindowAttributes = std::mem::zeroed();
        if (xlib.XGetWindowAttributes)(display, root, &mut root_attributes) == 0 {
            (xlib.XCloseDisplay)(display);
            return None;
        }
        let format = find_format(display, root_attributes.visual);
        let root_picture = if format.is_null() {
            0
        } else {
            create_picture(display, root, format, 0, std::ptr::null_mut())
        };
        if root_picture == 0 {
            (xlib.XCloseDisplay)(display);
            return None;
        ...
        let name = std::ffi::CString::new(format!("_NET_WM_CM_S{screen}")).expect("selection name");
        let atom = (xlib.XInternAtom)(display, name.as_ptr(), 0);
        let owner = (xlib.XCreateSimpleWindow)(display, root, -1, -1, 1, 1, 0, 0, 0);
        (xlib.XSetSelectionOwner)(display, atom, owner, 0);
        if (xlib.XGetSelectionOwner)(display, atom) != owner {
            (xlib.XDestroyWindow)(display, owner);
            (xlib.XCloseDisplay)(display);
            return None;
        }
        redirect(display, root, COMPOSITE_REDIRECT_MANUAL);
...
impl SelfCompositor {
    /// Composite every viewable top-level window onto the root, bottom to top.
    ///
    /// Reading the root itself needs no scratch surface: `XRenderComposite` draws straight into
    /// it. Windows are the root's direct children only — a nested child is already part of its
    /// parent's redirected pixmap, which is exactly why only top levels are redirected.
    unsafe fn draw_root(&self) {
        static TRACE_FRAMES: std::sync::atomic::AtomicU32 = std::sync::atomic::AtomicU32::new(0);
        let trace = std::env::var("TN_UI_OVERLAY_TRACE").is_ok()
            && TRACE_FRAMES.fetch_add(1, std::sync::atomic::Ordering::Relaxed) < 3;
        let Ok(name_pixmap) = self
            .composite
            .get::<unsafe extern "C" fn(*mut xlib::Display, c_ulong) -> c_ulong>(
                b"XCompositeNameWindowPixmap\0",
            )
        else {
            return;
        };
        let Ok(find_format) = self
            .render
            .get::<unsafe extern "C" fn(*mut xlib::Display, *mut xlib::Visual) -> *mut c_void>(
                b"XRenderFindVisualFormat\0",
            )
        else {
            return;
        };
        let Ok(create_picture) = self
            .render
            .get::<unsafe extern "C" fn(*mut xlib::Display, c_ulong, *mut c_void, c_ulong, *mut c_void) -> c_ulong>(
                b"XRenderCreatePicture\0",
            )
        else {
            return;
        };
        let Ok(composite) = self.render.get::<
            unsafe extern "C" fn(
                *mut xlib::Display,
                c_int,
                c_ulong,
        ...
        >(b"XRenderComposite\0") else {
            return;
        };
        let Ok(free_picture) = self
            .render
            .get::<unsafe extern "C" fn(*mut xlib::Display, c_ulong)>(b"XRenderFreePicture\0")
        else {
            return;
        ...
        if trace {
            eprintln!("TN_COMPOSE_TREE:{{\"count\":{count},\"root_picture\":{}}}", self.root_picture);
        }
        for index in 0..count as usize {
            let window = *children.add(index);
            let mut attributes: xlib::XWindowAttributes = std::mem::zeroed();
            if (self.xlib.XGetWindowAttributes)(self.display, window, &mut attributes) == 0 {
                continue;
            }
            if attributes.map_state != xlib::IsViewable || attributes.depth == 0 {
                continue;
            }
            let pixmap = name_pixmap(self.display, window);
            if trace {
                eprintln!(
                    "TN_COMPOSE_WIN:{{\"win\":{window},\"depth\":{},\"map\":{},\"geom\":[{},{},{},{}],\"pixmap\":{pixmap}}}",
                    attributes.depth,
            ...
            if trace {
                let sample = |x: c_int, y: c_int| -> Option<c_ulong> {
                    let probe = (self.xlib.XGetImage)(
                        self.display,
                        pixmap,
                        x,
                        y,
                    ...
                    let v = (*probe).funcs.get_pixel.map(|f| f(probe, 0, 0));
                    (self.xlib.XDestroyImage)(probe);
                    v
                };
                eprintln!(
                    "TN_COMPOSE_PIXEL:{{\"win\":{window},\"hud\":{:08x?},\"centre\":{:08x?},\"low\":{:08x?}}}",
                    sample(100, 100),
                    sample((attributes.width / 2) as c_int, (attributes.height / 2) as c_int),
                    sample(200, 600),
                );
            }
            let format = find_format(self.display, attributes.visual);
            if format.is_null() && trace {
                eprintln!("TN_COMPOSE_NOFORMAT:{{\"win\":{window}}}");
            }
            if !format.is_null() {
                let picture = create_picture(self.display, pixmap, format, 0, std::ptr::null_mut());
                if trace {
                    eprintln!("TN_COMPOSE_PIC:{{\"win\":{window},\"picture\":{picture}}}");
                }
                if picture != 0 {
                    // A depth-32 window's alpha blends; a depth-24 one has none and composites
                    // opaque. `attributes.x/y` are already relative to the root, which is the
                    // destination picture's coordinate space.
                    composite(
                        self.display,
                        PICT_OP_OVER,
                        picture,
                    ...
                    free_picture(self.display, picture);
                }
            }
        ...
        let frame = TRACE_FRAMES.load(std::sync::atomic::Ordering::Relaxed);
        if std::env::var("TN_UI_OVERLAY_TRACE").is_ok() && frame % 120 == 0 {
            ...
            if image.is_null() {
                eprintln!("TN_COMPOSE_READBACK:null");
            } else {
                let pixel = (*image).funcs.get_pixel.map(|f| f(image, 0, 0));
                eprintln!("TN_COMPOSE_READBACK:{:?}", pixel);
