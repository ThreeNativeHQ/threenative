...
/// Dispatch one synthetic DOM keyboard event into the page. See `key_injection_script`.
///
/// `ctrl`/`alt`/`shift`/`meta` are the modifier state, which the page sees as `ctrlKey` and
/// friends. `text` is the character the key would insert; empty for every key that types nothing.
#[no_mangle]
pub extern "C" fn tn_ui_overlay_inject_key(
    ...
    if kind.is_null() || key.is_null() || code.is_null() || text.is_null() {
        return -5;
    }
    let read = |value: *const c_char| -> Option<String> {
        (unsafe { CStr::from_ptr(value) }).to_str().map(str::to_string).ok()
    };
    let (Some(kind), Some(key), Some(code), Some(text)) =
        (read(kind), read(key), read(code), read(text))
    else {
        return -5;
    };
    OVERLAY.with(|slot| match slot.borrow().as_ref() {
