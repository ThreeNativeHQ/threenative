...
test("converted wait sites have no fixed sleep poll and retain a wake primitive", () => {
  ...
  for (const [path, waitPattern] of sites) {
    ...
    assert.doesNotMatch(
      source,
      /sleep_for\(std::chrono::milliseconds\((?:1|10)\)\)/u,
      `${path} still has a fixed poll sleep`,
    );
    assert.match(source, waitPattern, `${path} has no wait/fence replacement`);
  }

  const webgpuWait = nativeDefinition("waitForWebGpuCallback").text;
  assert.doesNotMatch(webgpuWait, /sleep_for\(std::chrono::milliseconds\((?:1|10)\)\)/u);
  assert.match(webgpuWait, /waitCondition\.wait_for\(/u);
});
...
test("retained delays document why polling cannot yet become a condition wait", () => {
  ...
  assert.match(
    read("src/video/screen_capture_kit.mm"),
    /Retry finding the window[\s\S]*usleep\(100000\)/u,
  );
});
