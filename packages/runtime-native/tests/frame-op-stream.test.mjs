...
function harness() {
  const device = { createCommandEncoder() {} };
...
describe("packed frame op stream", () => {
  ...
  it("serializes every steady-state command family without a native fallback", () => {
    ...
    const encoder = device.createCommandEncoder();
    const render = encoder.beginRenderPass({ colorAttachments: [{ view, loadOp: "clear", storeOp: "store", clearValue: [0, 0, 0, 1] }] });
    render.setPipeline(pipeline); render.setBindGroup(0, group, [4]); render.setVertexBuffer(0, buffer);
    render.setIndexBuffer(buffer, "uint32"); render.draw(3); render.drawIndexed(3, 1, 0, -1, 0);
    render.drawIndirect(buffer, 0); render.drawIndexedIndirect(buffer, 0); render.setViewport(0, 0, 1, 1, 0, 1);
    render.setScissorRect(0, 0, 1, 1); render.setBlendConstant([0, 0, 0, 1]); render.setStencilReference(1);
    render.executeBundles([bundle]); render.end();
    ...
    encoder.copyTextureToTexture({ texture }, { texture }, [1, 1, 1]); encoder.clearBuffer(buffer);
    ...
    expect(opcodes).toEqual(expect.arrayContaining(Array.from({ length: 30 }, (_, index) => index + 2)));
