...
static uint64_t g_currentSurfaceTextureId = 0;
                ...
                g_engine->newFunction("requestDevice", [](void* ctx, const std::vector<js::JSValueHandle>& args) {
                    ...
                    g_engine->setProperty(queue, "writeBuffer",
                        g_engine->newFunction("writeBuffer", [](void* ctx, const std::vector<js::JSValueHandle>& args) {
                            ...
                            const double offsetValue = g_engine->toNumber(args[1]);
                            if (!std::isfinite(offsetValue) || offsetValue < 0 || std::floor(offsetValue) != offsetValue ||
                                static_cast<uint64_t>(offsetValue) % 4 != 0) {
                                g_engine->throwException("writeBuffer: buffer offset must be a non-negative multiple of 4");
                                return g_engine->newUndefined();
                            }
                            ...
                            auto bytesPerElementValue = g_engine->getProperty(args[2], "BYTES_PER_ELEMENT");
                            if (!g_engine->isUndefined(bytesPerElementValue)) {
                                const double value = g_engine->toNumber(bytesPerElementValue);
                                if (value > 0) bytesPerElement = static_cast<size_t>(value);
                            }

                            const double dataOffsetValue = args.size() > 3 ? g_engine->toNumber(args[3]) : 0;
                            const double sizeValue = args.size() > 4 ? g_engine->toNumber(args[4]) : -1;
                            if (!std::isfinite(dataOffsetValue) || dataOffsetValue < 0 ||
                                ...
                                g_engine->throwException("writeBuffer: data offset and size must be non-negative");
                                return g_engine->newUndefined();
                            }
                            ...
                            if (dataOffset > dataSize || writeSize > dataSize - dataOffset) {
                                g_engine->throwException("writeBuffer: source range exceeds the supplied buffer view");
                                return g_engine->newUndefined();
                            }
                            ...
                            if (buffer && g_queue) {
                                ...
                                if (alignedWriteSize == writeSize) {
                                    wgpuQueueWriteBuffer(g_queue, buffer, offset, source, writeSize);
                                } else {
                                    ...
                                    wgpuQueueWriteBuffer(g_queue, buffer, offset, alignedData.data(), alignedData.size());
                                }
                            }
                    ...
                    g_engine->setProperty(device, "createShaderModule",
                        g_engine->newFunction("createShaderModule", [](void* ctx, const std::vector<js::JSValueHandle>& args) {
                            ...
                            g_engine->setProperty(jsShader, "_tnVertexEntryPoint",
                                g_engine->newString(singleWgslEntryPoint(code, "vertex").c_str()));
                            g_engine->setProperty(jsShader, "_tnFragmentEntryPoint",
                                g_engine->newString(singleWgslEntryPoint(code, "fragment").c_str()));
                    ...
                    g_engine->setProperty(device, "createRenderPipeline",
                        g_engine->newFunction("createRenderPipeline", [](void* ctx, const std::vector<js::JSValueHandle>& args) {
                            ...
                            const bool hasVertexEntry = !g_engine->isUndefined(vertexEntryProp);
                            std::string vertexEntry = hasVertexEntry
                                ? g_engine->toString(vertexEntryProp)
                                : g_engine->toString(
                                    g_engine->getProperty(vertexModule, "_tnVertexEntryPoint"));
                            ...
                            if (hasFragment) {
                                ...
                                if (!g_engine->isUndefined(fragEntryProp)) {
                                    ...
                                    fragmentEntry = g_engine->toString(g_engine->getProperty(
