...
function Pager({ path }: { readonly path: string }) {
  ...
  return (
    <nav
      ...
      className="mt-12 grid gap-3 border-t border-tn-border pt-8 sm:grid-cols-2"
    >
      {previous ? (
        <a
          className="rounded-xl border border-tn-border bg-tn-surface/60 p-4 hover:border-white/20"
          href={docsHref(previous.path)}
        >
      ...
      {next ? (
        <a
          className="rounded-xl border border-tn-border bg-tn-surface/60 p-4 text-right hover:border-white/20"
    ...
    >
      <aside className="min-w-0 lg:sticky lg:top-[96px] lg:max-h-[calc(100dvh_-_120px)] lg:self-start lg:overflow-y-auto">
        ...
        <div className="hidden lg:block">
          <DocsNavigation path={page.path} />
        </div>
        <details
          className="rounded-lg border border-tn-border p-3 lg:hidden"
          data-testid="mobile-docs-navigation"
        >
          <summary className="cursor-pointer text-[14px] font-medium text-tn-fg">
            Browse docs · {page.label}
          </summary>
          <div className="mt-5">
            <DocsNavigation path={page.path} />
          </div>
        ...
        <div className="mx-auto max-w-[820px] xl:mx-0">
          <nav aria-label="Breadcrumb" className="mb-4 text-[13px] text-tn-fg-subtle">
            <ol className="flex items-center gap-2">
              <li>
                <a className="hover:text-tn-fg" href="/docs/">
                  Docs
                </a>
              </li>
          ...
          <h1 className="mt-4 max-w-[760px] text-[36px] font-bold leading-[1.08] tracking-[-0.035em] text-tn-fg sm:text-[48px]">
            {page.title}
          </h1>
          ...
          {sourceHref ? (
            <a
              className="mt-5 inline-flex text-[13px] text-tn-fg-subtle underline underline-offset-4 hover:text-tn-fg"
              href={sourceHref}
              rel="noreferrer"
              target="_blank"
            >
          ...
          {toc.length ? (
            <details className="mt-7 rounded-lg border border-tn-border p-4 xl:hidden">
              <summary className="cursor-pointer text-[13px] font-medium text-tn-fg">
                On this page
              </summary>
              <div className="mt-4">
          ...
          <a
            className="mt-8 inline-flex text-[13px] text-tn-fg-subtle underline underline-offset-4 hover:text-tn-fg"
            href={`https://github.com/ThreeNativeHQ/threenative/edit/develop/${page.sourceFile}`}
            rel="noreferrer"
            target="_blank"
          >
    ...
    >
      <h2 className="mb-4 text-[26px] font-semibold tracking-[-0.02em] text-tn-fg">
        <a className="group" href={`#${id}`}>
          ...
          <span
            aria-hidden="true"
            className="ml-2 text-tn-accent opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100"
          >
            #
          </span>
        </a>
