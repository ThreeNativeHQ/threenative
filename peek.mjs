...
const out = process.argv[3] ?? "/tmp/claude-1000/-home-joao-projects-threejs-webgpu/ab4f7baa-23b5-430d-9147-5484b238617c/scratchpad/shot.png";
const wait = Number(process.argv[4] ?? 2500);
const keys = (process.argv[5] ?? "").split(",").filter(Boolean);
const hold = Number(process.argv[6] ?? 1200);
const browser = await chromium.launch({ args: ["--enable-unsafe-webgpu", "--disable-gpu-sandbox", "--ignore-gpu-blocklist"] });
